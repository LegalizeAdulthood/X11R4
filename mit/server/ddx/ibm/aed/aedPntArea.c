...
/* $Header: /afs/testers.athena.mit.edu/src/x11r4/src/mit/server/ddx/ibm/aed/RCS/aedPntArea.c,v 1.2 89/11/19 20:37:10 jfc Exp $ */
/* $Source: /afs/testers.athena.mit.edu/src/x11r4/src/mit/server/ddx/ibm/aed/RCS/aedPntArea.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/testers.athena.mit.edu/src/x11r4/src/mit/server/ddx/ibm/aed/RCS/aedPntArea.c,v 1.2 89/11/19 20:37:10 jfc Exp $";
#endif
    ...
    PixmapPtr pstipple;
{
    ...
    bits = (unsigned short *)(pstipple->devPrivate.ptr);

    tileWidth = pstipple->drawable.width;
	...
	{
	    MOVE(bits, vikint+vikoff, tilelen<<1 );
	    bits = bits + tilelen + 1; /* skip the last 16 pad bits */
	    vikoff += tilelen;
	}
    }
    else
    {
	MOVE(bits, vikint+vikoff, ( ( tilelen<<1 ) * tileHeight ) );
	vikoff += ( tilelen * tileHeight );
    }
...
{
    int i;
    int tilelen, tileHeight, tileWidth, j;
    unsigned short *bits;
    int chunkHeight, copyHeight, tileYPos, fillHeight, tilewords;
    int chunkYPos;
    int k, h, y;

    TRACE(("aedBigStippleFillArea(%#x, %d, %#x, %d, %#x)",pDraw, nbox, pbox, merge, pstipple));
	...
	while ( h > 0 )
	{
	    ...
	    bits = (unsigned short *)(pstipple->devPrivate.ptr) + chunkYPos*tilewords*2;
	    if ( ( chunkYPos + chunkHeight ) > tileHeight )
		copyHeight = tileHeight - chunkYPos;
	    else
		copyHeight = chunkHeight;

	    if ( ( ( y % tileHeight ) % chunkHeight ) != 0 )
		{
		if ( ( fillHeight = chunkHeight - ( ( y % tileHeight ) % chunkHeight ) ) > copyHeight )
	    ...
	    {
		MOVE(bits, &vikint[vikoff], copyHeight*tilelen*2);
