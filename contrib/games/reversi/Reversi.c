#ifndef lint
static char Xrcsid[] = "$XConsortium: Reversi.c,v 1.1 89/12/14 16:00:49 keith Exp $";
#endif /* lint */
...
#define offset(field) XtOffset(ReversiBoardWidget, field)
...
static void Initialize(request, new)
    Widget request, new;
{
    ReversiBoardWidget rw = (ReversiBoardWidget) new;
    int		x, y;
    ...
    rw->reversi.animate.state = AnimateNone;
} /* Initialize */
    ...
    int		    x, y;
{
    ...
    if (rw->reversi.animate.x == x &&
	rw->reversi.animate.y == y)
    {
    	switch (rw->reversi.animate.state)
    	{
    	case AnimateA:
	    value = rw->reversi.animate.A;
	    break;
    	case AnimateB:
	    value = rw->reversi.animate.B;
	    break;
    	}
    }
    switch (value)
...
static void Redisplay(w, event, region)
    ...
    Region region;
{
    ReversiBoardWidget rw = (ReversiBoardWidget) w;
    GC gc;
    int	x, y;
...
XawReversiBoardSetSpot (w, x, y, value)
    Widget	    w;
    int		    x, y;
    ReversiStone    value;
{
    ...
    if (rw->reversi.animate.state != AnimateNone &&
	rw->reversi.animate.x == x &&
	rw->reversi.animate.y == y)
    {
	XtRemoveTimeOut (rw->reversi.animate.timeout);
	rw->reversi.animate.state = AnimateNone;
	PaintEntry (rw, x, y);
	if (rw->reversi.animate.callback)
	    (*rw->reversi.animate.callback) (x, y);
    }
    else if (rw->reversi.board[x][y] != value)
    {
...
XawReversiBoardAnimateSpot (w, x, y, A, B, delay, repeat, callback)
    Widget	    w;
    int		    x, y;
    ReversiStone    A, B;
    unsigned long   delay;
    ...
    Cardinal *num_params;	/* unused */
{
	...
	XtCallCallbackList (w, rw->reversi.callbacks, (XtPointer) &move);
}
...
ReversiBoardClassRec reversiBoardClassRec = {
  {
/* core_class fields */	
    /* superclass	  	*/	(WidgetClass) superclass,
    /* class_name	  	*/	"ReversiBoard",
    /* widget_size	  	*/	sizeof(ReversiBoardRec),
...
WidgetClass reversiBoardWidgetClass = (WidgetClass)&reversiBoardClassRec;
