...
/*
 * Moved some local variables outside of DrawLocalWindow  to allow reuse
 * by DrawShips, DrawPlanets, DrawTorps
 */
 ...
 * and return in locations pointed to by x_ref and y_ref.
 */
  ...
  struct player  *p;
{
  ...
  for (torp = &torps[0]; torp < &torps[MAXPLAYER * MAXTORP]; torp++)
  {
	...
	if (cl_scale >= 1600)
	  ...
	  cl_scale = 3;
	else if (cl_scale >= 1100)
	  cl_scale = 2;
	else if (cl_scale >= 850)
	  cl_scale = 1;
	else
	  cl_scale = 0;
...
#define LCLM (8000 * 8000)
	...
	if (ISPLOCK (p) && player->p_no == p->p_playerl)
	  ...
	  max = CLM;
	if (distance_squared > max || (random () % max) < distance_squared)
	  continue;
      }
    }
	...
	if (pix)
	{				/* found a pixmap so use it */
	  if (p->mono)
	    /* mono, so just OR in pix */
	    XCopyArea (display, pix, p->w, p->monogc, 0, 0, ship_width, ship_height, x - ship_hwidth, y - ship_hheight);
	  else
	  {
	    XCopyArea (display, pix, p->w, p->monogc, 0, 0, ship_width, ship_height, x - ship_hwidth, y - ship_hheight);
#if 0
	    XSetClipMask (display, p->monogc, pix);
	    XSetClipOrigin (display, p->monogc, x - ship_hwidth, y - ship_hheight);
	    XFillRectangle (display, p->w, p->monogc, x - ship_hwidth, y - ship_hheight, ship_width, ship_height);
#endif
	  }
	}				/* no pixmap, use font */
	else
	{
	  int             nglyph = oempire >= 4 ? 0 : oempire;
	  char            glyph = SHIP_BASE + SHIP_MULT * nglyph + r;

	  XDrawString (display, p->w, p->xfgc, x - ship_hwidth, y - ship_hheight, &glyph, 1);
	}
      }
	...
	if (player->p_damage > p->ship.maxdamage / 2)
	  ...
	  glyph = Shield_GLYPH;
	/* if !self then glyph = Shield_GLYPH; old else for enemy shield disp */
	XDrawString (display, p->w, p->xfgc, x - shield_hwidth, y - shield_hheight, &glyph, 1);
      }
      /* NOTE: move XTextWidth calc into player structure */

      CLEARZONE (p, clearzone, x + ship_hwidth, y - ship_hheight, XTextWidth (font, shipnos + player->p_no, 1), fontHeight (font));
      CLEARZONE (p, clearzone, x - shield_hwidth, y - shield_hheight, shield_width, shield_height);
      break;
    case PExplode:
      {
	char            glyph;
	...
	glyph = EXP_GLYPHS_LEFT + ((10 - player->p_explode) >> 1);
	XDrawString (display, p->w, p->xfgc, expl_x, expl_y, &glyph, 1);
	glyph = EXP_GLYPHS_RIGHT + ((10 - player->p_explode) >> 1);
  ...
  register struct player *p;
{
  ...
  font_hwidth = font_width >> 1;
  font_hheight = font_height >> 1;
    ...
    /* top edge */
  {
    ...
    /* Adding in GWIDTH and subtracting it back out may seem stupid, but it makes it so that the dividend is always positive. */
    left = (GWIDTH - origin_x) / SCALE - GWIDTH / SCALE - BORDER_WIDTH;
    top = (GWIDTH - origin_y) / SCALE - GWIDTH / SCALE - BORDER_WIDTH;
    XFillRectangle (display, p->w, p->xfgc, left, top, GWIDTH / SCALE + BORDER_WIDTH * 2, BORDER_HEIGHT);
    CLEARZONE (p, clearzone, left, top, GWIDTH / SCALE + BORDER_WIDTH * 2, BORDER_HEIGHT);
  }
  if (origin_x <= -SCALE)
    /* left edge */
  {
    ...
    left = (GWIDTH - origin_x) / SCALE - GWIDTH / SCALE - BORDER_WIDTH;
    top = (GWIDTH - origin_y) / SCALE - GWIDTH / SCALE;
    XFillRectangle (display, p->w, p->xfgc, left, top, BORDER_WIDTH, GWIDTH / SCALE);
  ...
  register struct player *p;
{
  ...
  /* Draw Planets */
  {
	...
	continue;
      ...
      XDrawString (display, window, p->xfgc, x - mplanet_hwidth, y - mplanet_hheight, &glyph, 1);
      XDrawImageString (display, window, p->dfgc, x - (font_width * 3 >> 1), y + mplanet_hheight + font->ascent, planet->pl_name, 3);
    }
  }
  p->redrawall = 0;
  ...
  register struct player *j;
{
  ...
  if (udcounter % 4 == 0)		/* update every 4 window updt */
  {
    ...
    if (ISPLOCK (j) && !(ISCLOAK (p)))
      ...
      XDrawImageString (j->display, j->tstatw, j->dfgc, j->ts_offset, 31 + j->dfont->ascent, buf, STATLINESIZE);
  }
  if (ISSHOWSTATS (j))
