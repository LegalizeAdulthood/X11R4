...
LOCAL int       getcourse (p, ww, x, y)
  register struct player *p;
  Window          ww;
  int             x, y;
{
  return ww != p->mapw ? POLAR_DIRECTION (x - WINSIDE / 2, y - WINSIDE / 2) : POLAR_DIRECTION (x * GWIDTH / WINSIDE - p->p_x, y * GWIDTH / WINSIDE - p->p_y);
}
  ...
  int             old_num_players;
{
  ...
  if (np != old_num_players)
    fprintf (stderr, "Nplayers %d, old %d\n", np, old_num_players);
  ...
  for (;;)
  {
	...
	else
	{
	  ...
	  if (si >= 2 && sbuf[si - 2] == '\015' && sbuf[si - 1] == '\012')
	  {
	    ...
	    sbuf[si - 2] = '\0';
	    if (sscanf (sbuf, "Display: %s Login: %s", display_name, login) != 2)
	    {
		...
		if (player->p_status == PFree)
		{
		  ...
		  strcpy (player->p_monitor, display_name);
	...
	X Protocol error:  BadValue, integer parameter out of range for operation
	  ...
	  Serial number of failed request:  6649
	  Current serial number in output stream:  6666
	*/
