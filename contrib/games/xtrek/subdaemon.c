...
int             tm_robots[MAXEMPIRES + 1];	/* To limit the number of robots */
...
exitgame (p, status)
  ...
  int             status;
{
  /* commented out to compile under 4.2 level systems -- dl2n */
  ...
  register struct player *j;
{
  ...
  case PExplode:
	...
	j->p_stats.st_maxkills = j->p_kills;
      if (empstats[j->p_empire].empire_maxkills < j->p_kills)
	empstats[j->p_empire].empire_maxkills = j->p_kills;
      empstats[j->p_empire].empire_kills += j->p_kills;
