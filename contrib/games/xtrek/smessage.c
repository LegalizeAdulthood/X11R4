...
/* returns -1 if bad, empire number + EMPIRE_OFFSET, 
   player number +  PLAYER_OFFSET, or 0 for bcast */
  ...
  *(icharbuf+1) = '\0';
  ...
  case '\177':
    ...
    XFillRectangle(display, window, p->cleargc, 
                 fw5 * p->p_umsg.m_lcount, 5,font_width,font_height);
    break;
  case '\027':				/* CTRL-w */
    {
      int i = 0;
...
!isspace (p->p_umsg.m_buf[p->p_umsg.m_lcount - UMSGADDRLEN]))
  ...
  default:
    ...
    XDrawImageString (display, window, p->dfgc, fw5 * p->p_umsg.m_lcount, fa5, icharbuf, 1);
