...
double *pimpact_time, *pimpact_dist;
{
  ...
  impact_time = (POLAR_SINE (alpha) * impact_dist) / (POLAR_SINE (delta) * (enemy_speed + 0.01));
  ...
  difang=(difang > 128)?difang-128:difang;

  if( (difang < MINANGLE) || (difang > 128 - MINANGLE) || (enemy_speed < MINSPEED)){
 ...
 struct player *p;
{
    ...
    struct torp *t;
    struct planet *pl;
    int tcourse,impact_dist,impact_x,impact_y,torp_can_hit,thits=0,future_x,future_y;
    double impact_time;
    uchar ldir;

    for (i=MAXEMPIRES*MAXTORP,t = &torps[MAXEMPIRES * MAXTORP -1]; i; i--,t--) {
	tx = t->t_x;
	ty = t->t_y;
	mx = p->p_x;
	my = p->p_y;
	...
	if ( (t->t_status == TMine) || (t->t_mask & p->p_mask)) continue;
	torp_can_hit = calculate_hit
	    (t->t_speed*WARP1, t->t_x,t->t_y, 
	     p->p_speed*WARP1, p->p_dir, p->p_x, p->p_y,
	...
	if ((pl->pl_owner == p->p_empire) && !ISRHOSTILE(p)) continue;
	tx = pl->pl_x;
 	ty= pl->pl_y;
	mx = p->p_x;
	my = p->p_y;
	dx = tx - mx;
	dx = ABS(dx);
	if (dx > AVOIDDISTP) continue;
	dy = ty - my;
	dy = ABS(dy);
	if (dy > AVOIDDISTP) continue;
	if ((dx*dx + dy*dy) > AVOIDDISTP*AVOIDDISTP) continue;
	courseto = newcourse(p,tx,ty);
	coursedif = courseto - p->p_dir;
	if (coursedif > 128){
	    coursedif -=256;
 ...
 int flags,x,y;
{
    ...
    if (cnt < MAXPLAYER / 2) return;
    printf("starting robot for empire: %d\n",empire);
    p = &players[rpno];
    bzero(p->p_stats, sizeof(struct stats));
    nplayers++;
    enter(empire, "", rpno);
    SETROBOT(p);
    SETCOPILOT(p);
    if(random()%100 < p->ship.sneaky){
	SETSNEAKY(p);
    }
    else{
	CLRSNEAKY(p);
...
LOCAL void shoot(p,ecourse,tcourse,torp_can_hit,edist,edamage,lv,imptime)
 struct player *p;
 int ecourse,tcourse,torp_can_hit,edist,edamage,lv,imptime;
{
    int burst;
    if (imptime < 5) imptime= -1;
    if ((p->p_wtemp > 900) && random()&1) return;

    /** level 1 **/
    if (edist <= (p->ship.phaserdist *.75) && 
            p->p_fuel>=p->ship.cost[(int)Phaser]){
	if (ISCLOAK(p))
	    cloak_off(p);
	phaser(p, ecourse);
    }
    if ((13000 < edist) || torp_tubes[p->p_no])return; 
    /** level 1 **/
    if (torp_can_hit && p->p_ntorp < MAXTORP &&	
         p->p_fuel >= p->ship.cost[(int)Torp]) {
	if (ISCLOAK(p)) cloak_off(p);
	if (ISKILLER(p)) {
	    burst=p->ship.burst;
	    switch (burst){
	      case 4 :
		...
		case 0 :
		  default:
            ...
            ntorp(p,NORMALIZE(tcourse + (random()%(edist/1000 +1) - 
                  (edist/2000))),TMove,imptime+(edist/2000));
	}
    }
}
 ...
 u_char *ecourse;
{
    ...
    int torp_can_hit=0;
    double dx, dy, impact_time;
    int tdist;
    int p_nonhostile = !ISRHOSTILE(p);
    int tmpx,tmpy,tmpdir,tmpspeed,tmpimptime=10,tmpdist;
    int  impact_dist;
    ...
    enemy = p;
    *edist = (GWIDTH * GWIDTH) + 1;
    /* Find an enemy */
    for (i = MAXPLAYER, j = &players[MAXPLAYER - 1]; i--; j--) {
	if (j->p_status != PAlive || j == p) continue; 
	if (p_agressive & j->p_mask || (j->p_swar|j->p_hostile) & p->p_mask) {
	    ...
	    if (ISCLOAK(j)) {
		tmpdist = (dx * dx + dy *dy);
		if (tmpdist > 64000000 || (random() % 64000000) < tmpdist){
		    j->p_lv++;
		}
		else{
		    j->p_lv=0;
    ...
    if (ISSNEAKY(p)){
	if(*edist < p->ship.maxcloak && !ISCLOAK(p) && !ISWEPREL(p) && !(*edist < p->ship.mincloak)){
	    cloak_on(p);
	}
	else{
	...
	if (*edist < p->ship.engage) SETWEPREL(p);
    }
    if (*edist > p->ship.disengage) CLRWEPREL(p); 
    ...
    if (enemy->p_lv < SHOOTTIME){
	torp_can_hit = calculate_hit
	    (p->ship.torpspeed*WARP1, p->p_x, p->p_y, 
	     tmpspeed*WARP1, tmpdir, tmpx, tmpy,
	     &tcourse, &impact_time, &impact_dist, 
	     &impact_x, &impact_y);
...
LOCAL void runaway(p,edist,ecourse)
 ...
 u_char ecourse;
{ 
    if (((p->p_damage > 40 && p->p_shield < 40) || p->p_fuel < 600
	 || FAILED(p, Torp) || FAILED(p, Shield) || FAILED(p, Phaser) 
	 || p->p_wtemp > 900)
	&& edist < 4000*WARP1 ) {
	/* Run away!! Run away!! */
	if (!ISCLOAK(p) && p->p_fuel > 1000 && cloak_on(p) && ISCLOAK(p))
	    set_speed(p, p->ship.cloaked);
	else set_speed(p, p->ship.flee);
	if (ISSHIELD(p) && (p->p_etemp + p->p_wtemp) >= 99)
	    shield_down(p);
	p->p_desdir = ecourse - 128;
	if (edist <MINEDIST) ntorp(p,0,TMine,-1);
	}
    if ((p->p_damage>p->ship.maxdamage*.8)&& !ISCALLEDHELP(p)) {
