  ...
  struct player  *p;
{
  ...
  display = p->display;
  p->war = XCreateSimpleWindow (display, p->baseWin, WINSIDE + 10, -BORDER + 10, WARWIDTH, WARHEIGHT * (numempires + EXTRAWAR), WARBORDER, p->borderColor, p->backColor);

  for (i = 0; i < numempires + 2; i++)
  {
    p->waremp[i] = XCreateSimpleWindow (display, p->war, 0, i * WARHEIGHT, WARWIDTH, WARHEIGHT, WARBORDER, p->borderColor, p->backColor);
  ...
  else
  {
    ...
    tile = p->aTile[(int) Green];
    XSetWindowBorderPixmap (display, p->baseWin, tile);
    XSetWindowBorder (display, p->iconWin, tile);
  }
  p->p_status = POutfit;
}
  ...
  register struct player *p;
{
  ...
  {
    window = XCreateSimpleWindow (display, p->w, i * dx, 400, (unsigned) BOXSIDE, (unsigned) BOXSIDE, 1, p->shipCol[i], p->backColor);
    if (CANCHOOSE (i, p))
      XSelectInput (display, window, KeyPressMask | ButtonPressMask | ButtonReleaseMask | ExposureMask);
    else
    {
  ...
  window = XCreateSimpleWindow (display, p->w, numempires * dx, 400, (unsigned) BOXSIDE, (unsigned) BOXSIDE, 1, p->textColor, p->backColor);
  ...
  register struct player *p;
{
  ...
  for (i = 0, j = &players[0]; i < MAXPLAYER; i++, j++)
    if (j->p_status != PFree && (j != p) && !ISROBOT (j) && !strcmp (j->p_monitor, monitor))
    {
...

  {
    ...
    base_window = XCreateSimpleWindow (display, root_window, rootX, rootY, (unsigned) WINSIDE * 2 + 1 * BORDER, (unsigned) WINSIDE + 2 * BORDER + 2 * MSGSIZE, BORDER, p->borderColor, p->backColor);
