...
LOCAL int       s_warpcost[DEFEMPIRES] = {6, 4, 4, 2};
  ...
  char           *name;
{
  ...
  {
    team = findempire (name);
    if (team < 0)
      return NULL;
    if (!shipinit[team])
    {					/* first time so copy defaults */
      bcopy (&defaultShip, &ships[team], sizeof (aShip));
      shipinit[team] = 1;
    }
    return &ships[team];
  }
  ...
  int             empire;
{
  ...
  sl->label_length = 6;
  sl->min = 0;
  sl->max = sl->diff = MAXTEMP;
  sl->low_red = 0;
  sl->high_red = MAXTEMP * 3 / 4;
...
GLOBAL void     convertships ()
{
  ...
  for (empire = 0; empire < numempires; empire++)
  {
    ...
    ship = &ships[empire];
    maxdmg = ship->maxdamage;
    ship->cost[(int) Phaser] = ship->phaserdamage * 16;
    ship->cost[(int) Torp] = ship->torpdamage * 16;
    ship->cost[(int) Mine] = ship->minedamage * 32;
    for (i = 0; i < NSUBSYSTEMS; i++)
      ship->fail[i] = ship->fail[i] * maxdmg / 100;
    initsliders (empire);
  }
}
