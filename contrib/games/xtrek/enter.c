/*
 * 1989 Jon Bennett, Mike Bolotski, David Gagne, Dan Lovinger
...
#define BOGUS -1
  ...
  /* find a planet and put him near it */
  {
    ...
    do
    {
      struct planet  *planet = &planets[random () % numplanets];

      if (planet->pl_owner == empire)
      {
	x = planet->pl_x;
	y = planet->pl_y;
	goto found;
    ...
    /* probably empire does not own any planets */
    x = GWIDTH / 2;
    y = GWIDTH / 2;

  found:
    rand = random ();
    x += (rand & 0x1fff) - 0x1000;
    if (x < 0)
      x = 0;
    else if (x >= GWIDTH)
      x = GWIDTH - 1;
    p->p_x = x;
    y += (rand >> 13 & 0x1fff) - 0x1000;
    if (y < 0)
      y = 0;
    else if (y >= GWIDTH)
      y = GWIDTH - 1;
