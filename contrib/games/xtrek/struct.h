...
typedef 
enum { PFree, POutfit, PAlive, PExplode, PDead, PSetup } PlayerStatus;
...
typedef 
enum { KQuit, KTorp, KPhaser, KPlanet, KShip, KDaemon, KWinner }  DeathReason;
...
typedef 
enum { Green, Yellow, Red } AlertLevel;
...
typedef  enum {
   ...
   Detonate, Teleport, Sentinel} System;
...
typedef struct player {
    ...
    uchar p_dir;	/* Real direction */
    uchar p_lvdir;	/* last visible dir */
    uchar p_desdir;	/* desired direction */
    ulong p_hostile;		/* Who my torps will hurt */
    ulong p_swar;		/* Who am I at sticky war with */
    ushort p_subdir;	/* fraction direction change */
    ...
    ulong p_mask;	  	/* Team bit mask */
    short p_damage;		/* Current damage */
    short p_subdamage;		/* Fractional damage repair */
    short p_subshield;		/* Fractional shield recharge */
    ...
    Window	w,				/* big window */
		...
		planetw, 
		playerw, qwin,
                waremp[MAXEMPIRES+2],		/* wargo, warno hanging off */
                win[MAXEMPIRES];
    GC		gc, 				/* normal gc */
		bmgc,				/* base window background gc */
		cleargc, 
		dfgc, 				/* default font gc */
		bfgc, 				/* bold font gc */
		ifgc, 				/* italic font gc */
		bFgc, 				/* big font gc */
		xfgc,				/* xtrek font gc */
    ...
    short	fracwarp;   /*next digit is fractional warp desired*/
...
typedef enum { TFree, TMove, TExplode, TDet, TOff, TMine } TorpState;
...
typedef struct torp {
    ...
    uchar t_dir;	/* direction */
    ulong t_war;			/* enemies */
    ulong t_mask;			/* launching empire */
...
typedef struct planet {
    int pl_no;
    ulong pl_flags;		/* State information */
    int pl_owner;		/* Current owner of planet. */
    int pl_orig_owner;		/* Original owner of planet (for coup). */
    int pl_x;
    int pl_y;
    char pl_name[16];
