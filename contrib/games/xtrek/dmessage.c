  ...
  register struct player *p;
{
  ...
  if (mctl->mc_current == p->lastm)
  {
    if (!p->mdisplayed)
      return;
    XFillRectangle (p->display, p->messagew, p->cleargc, p->dfont->ascent, 5, fontWidth (p->dfont) * p->lastcount, fontHeight (p->dfont));
    p->mdisplayed = 0;
    return;
  }
  do
  {
    ...
    if (!(cur->m_flags & MVALID))
      continue;
    if (!((cur->m_flags & MALL) || (cur->m_flags & MEMPIRE) && (cur->m_recpt & p->p_mask) || (cur->m_flags & MINDIV) && (cur->m_recpt == p->p_no)))
      continue;
    if (p->mdisplayed)
    {
      XFillRectangle (p->display, p->messagew, p->cleargc, p->dfont->ascent, 5, fontWidth (p->dfont) * p->lastcount, fontHeight (p->dfont));
    ...
    if (p->lastcount > 80)
      p->lastcount = 80;
    XDrawImageString (p->display, p->messagew, p->dfgc, p->dfont->ascent, 5 + p->dfont->ascent, buf, p->lastcount);
    if (p->do_bell)
