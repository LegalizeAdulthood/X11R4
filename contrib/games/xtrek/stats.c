  ...
  Window          window;
{
  ...
  for (i = 0, slider_ptr = sliders[p->p_empire]; i < NUM_SLIDERS; i++, slider_ptr++, box_top += increment)
  {
    ...
    case 5:
      ...
      break;
    default:
      fprintf (stderr, "Somebody fucked up the sliders!\n");
      value = 0;
      old_value_ptr = 0;
      break;
    }
    ...
    XSetForeground (display, gc, value < slider_ptr->low_red || value > slider_ptr->high_red ? warning_color : normal_color);
    if (new_x > 0)
      XFillRectangle (display, window, gc, box_left, box_top, new_x, box_height);
    if (new_x < box_width)
      XClearArea (display, window, box_left + new_x, box_top, box_width - new_x, box_height, False);
    *old_value_ptr = value;
  }
}
  ...
  char           *prog;
{
  ...
  p->uspec = uspec;
  p->statX = (uspec & (XValue | XNegative)) == (XValue | XNegative) ? -stat_x : stat_x;
  p->statY = (uspec & (YValue | YNegative)) == (YValue | YNegative) ? -stat_y : stat_y;
}
  ...
  struct player  *p;
{
  ...
  window = XCreateWindow (display, RootWindow (display, p->screen), p->statX, p->statY, (unsigned) STAT_WIDTH, (unsigned) stat_height, 2, p->depth, InputOutput, (Visual *) CopyFromParent, 0L, (XSetWindowAttributes *) 0);
  ...
  XDefineCursor (display, window, (Cursor) XCreateFontCursor (display, XC_crosshair));
  ...
  Window          window;
{
  ...
  slider_ptr = sliders[player->p_empire];
  font_width = fontWidth (player->dfont);
  text_left = MAX_LABEL * font_width + STAT_SPACE;
  text_top = STAT_SPACE + player->dfont->ascent;
  box_left = text_left + font_width - 1;
  box_top = STAT_SPACE - 1;
  box_width = STAT_WIDTH - STAT_SPACE - box_left;
  font_height = fontHeight (player->dfont);
  box_height = font_height + 1;
  counter = NUM_SLIDERS - 1;
  increment = font_height + STAT_SPACE;

  for (;;)
  {
    int             length;

    length = slider_ptr->label_length;
    XDrawImageString (display, window, gc, text_left - length * font_width, text_top, slider_ptr->label, length);
    XDrawRectangle (display, window, gc, box_left, box_top, box_width, box_height);
    if (--counter < 0)
      break;
    slider_ptr++;
    text_top += increment;
