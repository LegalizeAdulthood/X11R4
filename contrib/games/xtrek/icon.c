  ...
  register struct player *p;
{
  ...
  XCopyPlane (display, p->ibm, p->iconWin, p->bmgc, 0, 0, icon_width, icon_height, 0, 0, 1);
}
  ...
  int             uspec;
{
  ...
  XWMHints        winhints;
  Window          icon_window;
  int             rootX, rootY, dummy1, dummy2;
  char           *str;

  display = p->display;
  base_window = p->baseWin;
  p->ibm = XCreateBitmapFromData (display, base_window, icon_bits,
  ...
  wininfo.flags = uspec & (XValue | YValue | XNegative | YNegative) ? USPosition | PSize | PMinSize | PMaxSize : PPosition | PSize | PMinSize | PMaxSize;
  XSetStandardProperties (display, base_window, PROGRAM_NAME, PROGRAM_NAME, p->ibm, (char **) NULL, 0, &wininfo);

  icon_window = XCreateSimpleWindow (display, RootWin, rootX, rootY, icon_width, icon_height, BORDER, p->borderColor, p->backColor);
  ...
  wininfo.x = rootX;
  wininfo.y = rootY;
  wininfo.flags = uspec & (XValue | YValue | XNegative | YNegative) ? USPosition : PPosition;
