  ...
  int imptime;
{
  ...
  *owned += reload;
  ...
  if (status == TMine)
  {
    ...
    torp->t_fuse = 1;
    p->p_nmine++;
    torp->t_fuse = (random () % MFIREVAR) + MFIREMIN;
  ...
  else
  {
    ...
    if (imptime >= 0)
      ...
      torp->t_fuse = (random () % TFIREVAR) + TFIREMIN;
    torp->t_damage = p->ship.torpdamage;
    torp->t_range = p->ship.torpdrange;
    p->p_stats.st_torps++;
    empstats[p->p_empire].empire_torps++;
  }
  ...
  register struct player *p;
{
  ...
  while (--h >= 0)
  {
	...
	continue;
      ...
      if (delta_x >= DETDIST || delta_x <= -DETDIST)
	continue;
      delta_y = torp->t_y - p->p_y;
      if (delta_y >= DETDIST || delta_y <= -DETDIST)
	continue;
      if (delta_x * delta_x + delta_y * delta_y < DETDIST * DETDIST)
	torp->t_status = TDet;
    }
  }
}
...
GLOBAL void udtorps ()
{
  ...
  for (torp = &torps[0]; torp < &torps[MAXPLAYER * MAXTORP]; torp++)
  {
    ...
    case TFree:
      continue;
    case TMine:
      torp->t_x += random () % 100 - 50;
      torp->t_y += random () % 100 - 50;
      if (nearest (torp))
	do_explode = 1;
      break;
    case TMove:
      torp->t_x += POLAR_X (torp->t_speed * WARP1, torp->t_dir);
      if (torp->t_x < 0)
      {
	torp->t_x = 0;
	do_explode = 1;
	...
	break;
      ...
      torp->t_y += POLAR_Y (torp->t_speed * WARP1, torp->t_dir);
      if (torp->t_y < 0)
      {
	torp->t_y = 0;
	do_explode = 1;
