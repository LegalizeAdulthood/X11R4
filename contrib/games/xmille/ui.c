...
GC		cheap_gc;
Region		null, mine;
unsigned long	foreground, background, border, distance;
    ...
    char **argv;
{
	...
	if (iscolor)
	{
		...
		foreground = colorMap[BLACK_COLOR].pixel;
		distance = colorMap[BLUE_COLOR].pixel;
	}
	else
	{
		border = colorMap[BLACK_COLOR].pixel;
		background = colorMap[WHITE_COLOR].pixel;
		foreground = colorMap[BLACK_COLOR].pixel;
		distance = colorMap[BLACK_COLOR].pixel;
	}
	
	gcv.font = XLoadFont(dpy, font_name);
