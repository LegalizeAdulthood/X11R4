...
static	char	sccsid[] = "@(#) olwm.c 4.1 89/12/06 Crucible";
#include <errno.h>
#include <stdio.h>
#include <signal.h>
#include <memory.h>
...
Button FrameLimButtons[] = {
	{ "Dismiss", True, Enabled, {(FuncPtr) 0, &DismissMenu} },
        { "Back", False, Enabled, {FrameLimAction, (Menu *)0} },
        { "Refresh", False, Enabled, {FrameLimAction, (Menu *)0} },
        { "Owner?", False, Enabled, {FrameLimAction, (Menu *)0} },
	};
...
int idx;
{
	...
	if ( DialogueBox( dpy, screen,
			  "Please confirm exit from window manager",
		     	  "Exit", "Cancel", 1, -1, -1 ) == 0 )
		Exit(dpy);
}
