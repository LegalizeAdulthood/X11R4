 ...
 *
 *	Original Source: ./client/kterm/main.c
 *
...
#include <sys/termio.h>
#ifndef mips
#if defined(SX9100) || defined(EWS) || defined(FACOM)
#include <sys/wait.h>
#ifndef FACOM
#include <sys/pty.h>
#endif
...
#ifdef XJP
	XJp_attributes	jp_att;
	int	arrange_flag = 0;
#endif /* XJP */
#ifdef EWS
struct  jtermio jtmio;
#endif
...
#ifdef	KANJI	/* sano */
	offset(xterm_name), XtRString, "xjpterm"},
#else	KANJI
...
#ifdef	XJP
{"-jo",		"*jfewOpen",	XrmoptionNoArg,		(caddr_t) "on"},
{"+jo",		"*jfewOpen",	XrmoptionNoArg,		(caddr_t) "off"},
{"-jm",		"*jfewMode",	XrmoptionSepArg,	(caddr_t) NULL},
#endif	/* XJP */
#endif	KANJI
...
#ifdef	XJP
{ "-/+jo",         	   "turn on/off connect front-end" },
{ "-jm jfewmode",          "convert window mode(inline|under|default)" },
#endif	/* XJP */
#endif	KANJI
...
#ifdef	KANJI	/* sano */
	if (strcmp(xterm_name, "-") == 0) xterm_name = "xjpterm";
#else	KANJI
...
#ifdef	SX9100
	if(term->misc.log_on) log_on = TRUE;
	if(term->misc.login_shell) login_shell = TRUE;
#endif	/* SX9100 */
#ifdef	XJP
	jp_att.fonts = (XJp_fontInfo *)malloc(sizeof(XJp_fontInfo) * 3);
	if (term->misc.j_m) {
		if (strcmp(term->misc.j_m, "inline") == 0) {
			screen->kmode = XJP_XY;
		} else if (strcmp(term->misc.j_m, "under") == 0) {
			screen->kmode = XJP_UNDER;
		} else if (strcmp(term->misc.j_m, "default") == 0) {
			screen->kmode = XJP_ROOT;
		}
	} else {
		screen->kmode = XJP_ROOT;
	}
 ...
 */
	...
	mode = 1;
	if (ioctl(pty, PIOCPKT, &mode) < 0) {
		SysError(ERROR_PIOCPKT);
	}
#endif	/* */
#endif	/* SX9100 */
...
open_wnmo()
{
	...
	/* screen->kcw_height = FontHeight(screen) + FontHeight(screen)/4; */
	if (screen->kmode == XJP_UNDER) {
	    /*screen->kcw_height = FontHeight(screen) * 2;  henkan w height */
	    jp_att.bp = FontHeight(screen)/7; /* henkan w border */
	    screen->kcw_full_height	/* total height when jinput */
		= term->core.height + screen->kcw_height + jp_att.bp ;
	    screen->kcw_posy = term->core.height;
	    jp_att.height = screen->kcw_height;
	} else if (screen->kmode == XJP_XY) {
	/* screen->kcw_height = FontHeight(screen) * 2; */
	    jp_att.bp = 0;
	    screen->kcw_posy =  0;
	    screen->kcw_full_height = term->core.height + screen->kcw_height;
	    jp_att.height = FontHeight(screen);
	}
	jp_att.p_width = term->core.width;
	jp_att.p_height = screen->kcw_full_height;
	jp_att.x = screen->kcw_posx;
	jp_att.y = screen->kcw_posy;
	jp_att.width = screen->kcw_width;
	jp_att.fg = screen->foreground;
	jp_att.bg = term->core.background_pixel;
	jp_att.num_fonts= 3;
	jp_att.fonts->fontname = term->misc.f_n;
	jp_att.fonts->id = XJP_romanX0201;
	(jp_att.fonts+1)->fontname = term->misc.f_rn;
	(jp_att.fonts+1)->id = XJP_kanaX0201;
	(jp_att.fonts+2)->fontname = term->misc.f_kn;
	(jp_att.fonts+2)->id = XJP_kanjiX0208;
/*
	set_jinput();
*/
}
...
set_jinput()
{
	...
	w = term->core.parent->core.window;
	if (screen->kmode != XJP_ROOT) {
		kcw_on();
	}
	if (!screen->kinit) {
		if(((ptr = getenv("WNNUSER")) == NULL || *ptr == NULL) &&
		((ptr = getenv("LOGNAME")) == NULL || *ptr == NULL)) {
			wnnuser[0] = NULL;
		} else {
			strcpy(wnnuser, ptr);
		}
		if (screen->kmode != XJP_ROOT) {
			mask = XJP_PWIDTH|XJP_PHEIGHT|XJP_X|XJP_Y|
			       XJP_WIDTH|XJP_HEIGHT|XJP_FG|XJP_BG|
			       XJP_BP|XJP_FONTS;
		} else {
			mask = 0;
		}
		if((screen->jpreq_id = XJp_open(term->screen.display, w,
					mask, &jp_att, screen->kmode,
					wnnuser))  == -1) {
			screen->kinit = 0;
			unset_jinput();
			return(-1);
	...
	if (XJp_begin(screen->jpreq_id, term->core.window) < 0) {
		screen->kinit = 0;
		unset_jinput();
		return(-1);
	}
	if (XJp_begin(screen->jpreq_id, term->core.parent->core.window) < 0) {
		screen->kinit = 0;
		unset_jinput();
		return(-1);
	}
	screen->kconvert = 1;
	if (screen->kmode == 0) {
		if (XJP_cursormove(screen, XJP_PWIDTH|XJP_PHEIGHT|XJP_WIDTH|
					XJP_HEIGHT|XJP_X|XJP_Y|XJP_FG|XJP_BG|
					XJP_BP|XJP_STATUS, 1) < 0) {
			screen->kinit = 0;
			unset_jinput();
			return(-1);
		}
	} else if (screen->kmode == 1) {
		jp_att.status_x = FontWidth(screen);
		jp_att.status_y = term->core.height + FontHeight(screen)/2;
		if (XJP_cursormove(screen, XJP_PWIDTH|XJP_PHEIGHT|XJP_WIDTH|
	...
	if (XJp_visible(screen->jpreq_id, term->core.window) < 0) {
		screen->kinit = 0;
		unset_jinput();
		return(-1);
	}
...
unset_jinput()
{
	...
	if (screen->kconvert) {
		if (XJp_end(screen->jpreq_id, term->core.window) < 0) {
			screen->kinit = 0;
		}else if (XJp_end(screen->jpreq_id, term->core.parent->core.window) < 0) {
			screen->kinit = 0;
		}else if (XJp_invisible(screen->jpreq_id) < 0) {
			screen->kinit = 0;
		}
		screen->kconvert = 0;
	}
...
kcw_on()
{
	...
	if (!kcw_on_flg) {
		(void) XResizeWindow(term->screen.display,
			term->core.parent->core.window,
			screen->kcw_width, screen->kcw_full_height);
		XGetNormalHints(screen->display, XtWindow(XtParent(term)),
				&sizehints);
		scrollbar_width = (term->misc.scrollbar ?
				  screen->scrollWidget->core.width : 0);
		sizehints.min_height = 2 * screen->border + screen->kcw_height
				     + jp_att.bp;
		sizehints.min_width = 2 * screen->border + scrollbar_width;
		sizehints.height = screen->kcw_full_height;
		sizehints.width = screen->kcw_width;
		sizehints.flags = PMinSize | PResizeInc;
		XSetNormalHints(screen->display, XtWindow(XtParent(term)),
				&sizehints);
		XFlush(term->screen.display);
		kcw_on_flg = 1;
	}
}
...
kcw_off()
{
	...
	if (kcw_on_flg) {
		...
		sizehints.min_width = 2 * screen->border + scrollbar_width;
		sizehints.height = term->core.height;
		sizehints.width = term->core.width;
		sizehints.flags = PMinSize | PResizeInc;
		XSetNormalHints(screen->display, XtWindow(XtParent(term)),
				&sizehints);
		XFlush(term->screen.display);
...
rearrange_ioctl()
{
	...
	(void)ioctl(screen->respond, JTERMGET, (char *)&jbuf1);
	if (screen->kinit && screen->kconvert) {
	...
	(void)ioctl(screen->respond, JTERMSET, (char *)&jbuf1);
}
...
arrange_ioctl()
{
	...
	(void)ioctl(screen->respond, JTERMGET, &jbuf1);
	(void)ioctl(screen->respond, TCGETA, &buf);
...
#endif	/* RAW_MODE_OFF */
	if ((jbuf1.j_flg & KANJIINPUT) != 0) {		/* kanjiinput turn on */
		if(!arrange_flag) {
			open_wnmo();
			set_jinput();
		}
	} else {					/* kanjiinput turn off */
		if(arrange_flag)
			unset_jinput();
	}
}
...
/* jtermio mode change JIS/UJIS/SJIS */
set_jterm()
{
	register TScreen	*screen = &term->screen;
	struct jtermio	jbuf1;
...
#ifndef	SYSV
	(void)ioctl(screen->respond, TIOCLGET, (char *)&lmode);
#endif	/* !SYSV */
...
#ifndef	SYSV
		ioctl(screen->respond, TIOCLSET, (char *)&lmode);
#endif	/* !SYSV */
	(void)ioctl(screen->respond, JTERMSET, (char *)&jbuf1);
}
#endif /* SX9100 */
...
#ifdef	SX9100
#define	PTY_BAD	1
#define	PTY_FATALERROR	2
#define	PTY_GOOD	3
#define	PTY_NEW	4
#define	PTY_NOMORE	5
#define	UTMP_ADDED	6
#define	UTMP_TTYSLOT	7
#else
...
#if defined(SX9100) && defined(SYSV)
	    TermName = resource.term_name;
#else	/* SX9100 and SYSV*/
...
#undef TMODE
		...
		signal(SIGTERM, SIG_DFL);
#ifdef EWS
		ioctl(0, TCJGETA, &jtmio);
		jtmio.c_oflag &= ~OJAPAN;
		ioctl(0, TCJSETA, &jtmio);
#endif
...
#ifdef	XJP
	if (screen->kinit)
		XJp_close(screen->jpreq_id);
#endif	/* XJP */
#ifdef	SX9100
	kill_wnn_if_exit(screen->respond);
#endif	/* SX9100 */
...
/*
	(void)ioctl(pty, JTERMGET ,&jbuf1);
	if ((jbuf1.j_flg & WNN_EXIST ) != 0){
		ioctl(pty, PIOCGET, &arg);
		(void)kill(arg.af0, SIGTERM);
	}
*/
#endif	/* SX9100 */
#endif	/* SYSV */
