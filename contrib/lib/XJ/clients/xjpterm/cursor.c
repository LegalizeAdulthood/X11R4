 ...
 *
 *	Original Source: ./client/kterm/cursor.c
 *
...
XJP_cursormove(screen, mask, true)
register TScreen	*screen;
unsigned int	mask;
int	true;
{
	...
	int	offset = 0;
	if (screen->kconvert) {
	    if (screen->kmode == XJP_XY) {
		x = screen->cursor_col;
		y = 2 * (screen->cursor_row);
		flag = screen->buf[y + 1][x];
		if (screen->buf[y][x] == 0) {
			offset = 0; 
		} else if (flag & KCS) {
				if (flag & KCS1)
		...
		if (mask) {
			if (X != save_x || Y != save_y || true) {
				jp_att.x = save_x = X;
				jp_att.y = save_y = Y;
				XJp_change(screen->jpreq_id, mask | XJP_X | XJP_Y, &jp_att);
			} else {
				XJp_change(screen->jpreq_id, mask, &jp_att);
			}
		} else {
			if ( X != save_x || Y != save_y || true) {
				/*
				XJp_move(screen->jpreq_id,
					 (save_x = X), (save_y = Y));
				*/
				save_flag = 1;
				save_X = save_x = X;
	    ...
	    } else if (screen->kmode == XJP_UNDER && mask) {
		XJp_change(screen->jpreq_id, mask, &jp_att);
	    }
	}
}
...
{
	if (save_flag) {
		XJp_move(screen->jpreq_id, save_X, save_Y);
		save_flag = 0;
	}
}
#endif	/* XJP */
