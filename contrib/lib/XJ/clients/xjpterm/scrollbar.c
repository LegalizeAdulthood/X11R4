 ...
 *
 *	Original Source: ./client/kterm/scrollbar.c
 *
...
#else	STATUSLINE
#ifdef	XJP
	if (screen->kinit && screen->kmode != XJP_ROOT) {
		newAttributes.bit_gravity = NorthEastGravity;
	} else {
...
#ifdef	XJP
	if (screen->kinit && screen->kmode != XJP_ROOT) {
	geomreqresult = XtMakeResizeRequest ((Widget)xw, reqWidth,
					     reqHeight + screen->kcw_height,
					     &repWidth, &repHeight);
	} else {
...
#ifdef	XJP
	if (screen->kinit && screen->kmode != XJP_ROOT) {
		screen->kcw_width = screen->fullVwin.width + screen->scrollbar + border;
		jp_att.p_width = screen->kcw_width;
		jp_att.width = screen->kcw_width;
		XJP_cursormove(screen, XJP_PWIDTH | XJP_WIDTH, 0);
	}
#endif	/* XJP */
}
...
#ifdef	XJP
	if (screen->kinit && screen->kmode != XJP_ROOT) {
		screen->kcw_width = screen->fullVwin.width + border;
		jp_att.p_width = screen->kcw_width;
		jp_att.width = screen->kcw_width;
		XJP_cursormove(screen, XJP_PWIDTH | XJP_WIDTH, 0);
	}
