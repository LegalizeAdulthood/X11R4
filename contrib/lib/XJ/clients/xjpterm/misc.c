 ...
 *
 *	Original Source: ./client/kterm/misc.c
 *
...
extern int	arrange_flag;
#define	IsVTWin(w)	(w == term->core.parent->core.window || w == term->core.window)
#endif	/* XJP */
...
xevents()
{
	...
	do {
		...
		XGetNormalHints(screen->display, XtWindow(XtParent(term)),
				&sizehints);
		if((IsVTWin(event.xany.window) &&(screen->kinit 
		   || (event.type == ConfigureNotify && term->misc.jfew_open)))
		   ||(event.xany.window == screen->jpreq_id))
	{
	static int henkan;
	static int leave;
	int ii, n;
	...
	switch (event.type) {
		...
		case ClientMessage:
			if (term->flags & EUC_KANJI)
					...
					pty_code = XJP_JIS8;
			if ((size = XJp_string(&event, data, 512, pty_code)) < 0) {
				/*
				msg = (char *)XJp_error(NULL);
				if (msg)
					fprintf(stderr, "%s\n", msg);
				*/
				screen->kinit = screen->kconvert = 0;
				kcw_off();
#ifdef	PKT_MODE
				rearrange_ioctl();
#endif	/* PKT_MODE */
				...
				all_size += size;
				/*
				if (event.xclient.data.b[3] == 0) {
				*/
					data = data_buf;
					while(all_size > 0){
						if((n = write(screen->respond, data , all_size)) >= 0){
							all_size -= n;
							data += n;
						}
					}
		...
		case ConfigureNotify:
			if (screen->kmode == 0 || screen->kmode == 1) {
			    if (term->misc.jfew_open) {
				term->misc.jfew_open = 0;
				/*
				XtDispatchEvent(&event);
				open_wnmo();
				*/
				event.xconfigure.height 
					-= (screen->kcw_height + jp_att.bp);
				if (event.xconfigure.height != term->core.height
				|| event.xconfigure.width != term->core.width) {
					XtDispatchEvent(&event);
					open_wnmo();
				} else {
					XtDispatchEvent(&event);
		...
		case MapNotify:
		/*
			if (event.xany.window == term->core.window && term->misc.jfew_open) {
				term->misc.jfew_open = 0;
				open_wnmo();
			} else {
				XtDispatchEvent(&event);
...
#ifdef	SX9100
unsigned char save_log[512];
SaveLog()
{
	bcopy(bptr, save_log, bcnt);
	save_log[bcnt] = NULL;
}
#endif	/* SX9100 */
...
#ifdef	SX9100
		write(screen->logfd, save_log, i);
	screen->logstart = screen->TekEmu ? Tbuffer : &buffer[1];
#else	/* SX9100 */
