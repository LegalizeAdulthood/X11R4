 ...
 *
 *	Original Source: ./client/kterm/button.c
 *
		...
		case SELECTWORD :
			if (startSCol > (LastTextCol(startSRow) + 1)) {
				...
				ptr = screen->buf[2*(startSRow+screen->topline)]
				 + startSCol;
#if defined(SX9100) || defined(EWS) || defined(FACOM) || defined(UX386) || defined(SUN)
				flags = screen->buf[2*(startSRow+screen->topline) + 1]
				 + startSCol;
				if (*ptr & 0x80) {
					do {
						--startSCol;
			...
			if (endSCol > (LastTextCol(endSRow) + 1)) {
				...
				ptr = screen->buf[2*(endSRow+screen->topline)]
				 + endSCol;
#if defined(SX9100) || defined(EWS) || defined(FACOM) || defined(UX386) || defined(SUN)
				flags = screen->buf[2*(endSRow+screen->topline) + 1]
				 + endSCol;
				if (*ptr & 0x80) {
					do {
						++endSCol;
...
/* Guaranteed (frow, fcol) <= (trow, tcol) */
{
	...
	if (to <= screen->startHCoord || from > screen->endHCoord
	    || screen->endHCoord == screen->startHCoord) {
#else
	if (to <= screen->startHCoord || from > screen->endHCoord) {
#endif
