 ...
 *
 *
 * Original Source: ./clients/uwm/GetButton.c
 *
 * Changes
...
#define STRLEN 50
...
#ifdef XJP
    if(button_event->type == DestroyNotify) {
        if (((XDestroyWindowEvent *)button_event)->window == jpconvid) {
	    XJp_reset(button_event);
	    jpconvid = XJPID_INIT;
	    Warning("abnormal terminated XJP Frontend");
	}
    }
#endif
	...
	switch (button_event->type) {
	  ...
	  default: 
#ifdef XJP
	    printf("xjpuwm internal error: unexpected event on Root Window\n");
#else
...
#ifdef EUC
	int desired_width = XJeTextWidth (IFontInfoEuc, ICONSTR, 
			    strlen (ICONSTR)) + (HIconPad << 1);
	int desired_height = (IFontInfoEuc->ascent + IFontInfoEuc->descent + 
			      (VIconPad << 1));
#else
	...
	XClearWindow(dpy, icon);
        if (icon_str_len != 0) {
#ifdef EUC
            XJeDrawImageString(dpy, icon, IconGC, IFontInfoEuc, 
		     HIconPad, VIconPad+IFontInfoEuc->ascent, 
...
#ifdef XJP
    if (button_event->type == ClientMessage) {
	nbytes = XJp_string(button_event, kbd_str, STRLEN, XJP_EUC);
    } else {
    ...
    for (i = 0; i < nbytes; i++) {
        ...
        if (key_char == '\177' || key_char == '\b') {
            ...
            if (icon_str_len > 0) {
#ifdef EUC
		icon_str_len -= _checkLastCharByte(&icon_str[icon_str_len]);
		if (icon_str_len < 0)
		    icon_str_len = 0;
#else
		icon_str_len--;
		...
		icon_str[0] = '\0';
		 ...
		 !(key_char & 0x80)) {		/* It's not complete */
#else
        else if (key_char < IFontInfo->min_char_or_byte2 ||
                 key_char > IFontInfo->max_char_or_byte2) {
#endif
     ...
     */
#ifdef EUC
    icon_h = IFontInfoEuc->ascent + IFontInfoEuc->descent + (VIconPad << 1);
    icon_w = XJeTextWidth(IFontInfoEuc, ICONSTR, strlen(ICONSTR)) + 
	     (HIconPad << 1);
#else
