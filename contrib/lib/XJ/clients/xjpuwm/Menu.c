 ...
 *
 *
 * Original Source: ./clients/uwm/Menu.c
 *
 * Changes
...
#define DisplayLine(w, pane, width, height, str, fg, bg, inv) \
	     ...
	     if (inv) \
	         XFillRectangle(dpy, w, MenuInvGC, 0, pane, width, height); \
                 ...
                 XJeDrawString(dpy, w, MenuGC, MFontInfoEuc, HMenuPad, pane + VMenuPad + MFontInfoEuc->ascent, str, strlen(str)); \
         } else { \
...
MenuInfo *menu;
{
     ...
     */
#ifdef EUC
    maxwidth = XJeTextWidth(MFontInfoEuc, menu->name, strlen(menu->name));
#else
    ...
    for(ml = menu->line; ml; ml = ml->next) {
        if ((len = strlen(ml->name)) == 0)
            break;
#ifdef EUC
        width = XJeTextWidth(MFontInfoEuc, ml->name, strlen(ml->name));
#else
     ...
     */
#ifdef EUC
    menu->iheight = MFontInfoEuc->ascent + MFontInfoEuc->descent + 
		    (VMenuPad << 1);
#else
