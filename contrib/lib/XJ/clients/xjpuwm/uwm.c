 ...
 *
 *
 * Original Source: ./clients/uwm/uwm.c
 *
 * Changes
...
char **environ;
{
...
#ifdef EUC
    XJeFontStruct *XJeLoadQueryFont();
    XJeFontNameStruct fname_st;
    register i;
#endif
...
#ifdef XJP
        perror("xjpuwm: child cannot disinherit TCP fd");
#else
...
#ifdef EUC
    /*
     * Read in and parse $HOME/.xjpuwmrc, if it exists.
     */
    sprintf(rc_file, "%s/.xjpuwmrc", getenv("HOME"));
    if ((yyin = fopen(rc_file, "r")) != NULL) {
        Lineno = 1;
        yyparse();
        fclose(yyin);
        if (Startup_File_Error)
            Error("Bad .xjpuwmrc file...aborting");
    }
#endif
...
#ifdef EUC
    for (i = 0; i < 4; i++) {
	fname_st.fontname_std[i] = IFontNameStd[i];
	fname_st.fontname_ext[i] = IFontNameExt[i];
    }
    IFontInfoEuc = XJeLoadQueryFont(dpy, &fname_st);

    for (i = 0; i < 4; i++) {
	fname_st.fontname_std[i] = PFontNameStd[i];
	fname_st.fontname_ext[i] = PFontNameExt[i];
    }
    PFontInfoEuc = XJeLoadQueryFont(dpy, &fname_st);

    for (i = 0; i < 4; i++) {
	fname_st.fontname_std[i] = MFontNameStd[i];
	fname_st.fontname_ext[i] = MFontNameExt[i];
    }
    MFontInfoEuc = XJeLoadQueryFont(dpy, &fname_st);

    if (!(IFontInfoEuc->font_std[0]) && !(IFontInfoEuc->font_ext[0])) {
#ifdef XJP
        fprintf(stderr, "xjpuwm: Unable to open icon font '%s', using server default.\n",
                IFontNameStd[0]);
#else
        fprintf(stderr, "uwm: Unable to open icon font '%s', using server default.\n",
                IFontNameStd[0]);
#endif
	IFontInfoEuc->font_std[0] = XQueryFont(dpy, DefaultGC(dpy, scr)->gid);
	fallbackIFont = True;
    }
    IFontInfo = IFontInfoEuc->font_std[0] ? 
		    IFontInfoEuc->font_std[0] : IFontInfoEuc->font_ext[0];
    if (!(PFontInfoEuc->font_std[0]) && !(PFontInfoEuc->font_ext[0])) {
#ifdef XJP
        fprintf(stderr, "xjpuwm: Unable to open resize font '%s', using server default.\n",
                PFontNameStd[0]);
#else
...
#endif
	if (fallbackIFont)
	    PFontInfoEuc->font_std[0] = IFontInfo;
	else
	    PFontInfoEuc->font_std[0] = 
		XQueryFont(dpy, DefaultGC(dpy, scr)->gid);
	fallbackPFont = True;
    }
    PFontInfo = PFontInfoEuc->font_std[0] ? 
		    PFontInfoEuc->font_std[0] : PFontInfoEuc->font_ext[0];
    if (!(MFontInfoEuc->font_std[0]) && !(MFontInfoEuc->font_ext[0])) {
#ifdef XJP
        fprintf(stderr, "xjpuwm: Unable to open menu font '%s', using server default.\n",
                MFontNameStd[0]);
#else
	...
	if (fallbackIFont || fallbackPFont)
	    MFontInfoEuc->font_std[0] = fallbackPFont ? PFontInfo : IFontInfo;
	else
	    MFontInfoEuc->font_std[0] = 
		XQueryFont(dpy, DefaultGC(dpy, scr)->gid);
	fallbackMFont = True;
    }
    MFontInfo = MFontInfoEuc->font_std[0] ? 
		    MFontInfoEuc->font_std[0] : MFontInfoEuc->font_ext[0];
#else
...
InitBindings()
{
    ...
    if (!tempfile) {
#ifdef XJP
	perror("xjpuwm: cannot allocate name for temp file");
#else
...
#ifdef XJP
        perror("xjpuwm: cannot create temp file");
#else
...
#ifdef XJP
        perror("xjpuwm: cannot open temp file");
#else
...
#ifdef XJP
                fprintf(stderr,
                        "xjpuwm: non-existent menu reference: \"%s\"\n",
                        bptr->menuname);
#else
                fprintf(stderr,
...
{
#ifdef EUC
    strcpy(IFontNameStd[0], DEF_FONT);
    strcpy(PFontNameStd[0], DEF_FONT);
    strcpy(MFontNameStd[0], DEF_FONT);
#else
