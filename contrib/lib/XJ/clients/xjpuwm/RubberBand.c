 ...
 *
 *
 * Original Source: ./clients/uwm/RubberBand.c
 *
 * Changes
...
AskUser(dpy, scr, window, x, y, width, height, hints)
	...
	/* XFontStruct *pfont; */
#ifdef EUC
#define pfont	PFontInfoEuc		/* text font for pop-up */
#else
...
#define invertGC DrawGC
	...
	nz += 11;
#ifdef EUC
	popw = XJeTextWidth (pfont, text, nz) + 2 * IBW;
#else
...
#ifdef EUC
		XJeDrawImageString(dpy, pop, popGC, pfont, IBW, 
		        IBW+pfont->ascent, text, nz);
#else
