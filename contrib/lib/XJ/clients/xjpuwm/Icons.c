 ...
 *		(0472) 77-8670
 *
 * Original Source: ./clients/uwm/Icons.c
 *
 * Changes
...
#ifdef XJP
	  if(jpconvid != XJPID_INIT)
	    XJp_end(jpconvid, ptr->icon);
#endif
...
#ifdef EUC
          *icon_h = IFontInfoEuc->ascent + IFontInfoEuc->descent;
          if (name) {
	    *icon_w = XJeTextWidth(IFontInfoEuc, name, strlen(name));
#else
...
#ifdef XJP
   mask = (KeyPressMask|ExposureMask|StructureNotifyMask|SubstructureRedirectMask);
#else
...
#ifdef XJP
   icon = AddIcon(window, icon, TRUE, mask, iconValues.background_pixmap);

   if (mask & KeyPressMask) {			/* add mask condition */
	if (jpconvid == XJPID_INIT) {
	    XJp_attributes jp_att;

	    if (IFontNameStd[1]) {
/*
		XJp_fontInfo    fonts[3];

		jp_att.num_fonts= 3;
		jp_att.fonts = fonts;
		jp_att.fonts[0].id = XJP_romanX0201;
		jp_att.fonts[0].fontname = IFontNameStd[0];
		jp_att.fonts[1].id = XJP_kanjiX0208;
		jp_att.fonts[1].fontname = IFontNameStd[1];
		jp_att.fonts[2].id = XJP_kanaX0201;
		jp_att.fonts[2].fontname = IFontNameStd[2];
*/
		if ((jpconvid = XJp_open(dpy, Pop, 0, &jp_att, XJP_ROOT, ""))
		    == XJPLIB_ERROR)
			jpconvid = XJPID_INIT;
	    }
	}
	...
	if (jpconvid != XJPID_INIT) {
	    XJp_begin(jpconvid, icon);
	    XJp_visible(jpconvid);
	}
   }
   return(icon);
#else
