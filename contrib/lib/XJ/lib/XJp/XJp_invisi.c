...
/* $Header: XJp_invisible.c,v 2.100 89/06/01 08:42:10 rws Exp $ */
...
XJp_id request_id;
{
...
#ifdef DEBUG
	fprintf(stderr,"Call XJp_invisible!!\n");
#endif
        if ((cnt = W_id_check(request_id)) == MAXWINDOW) {
#ifdef DEBUG
		fprintf(stderr,"XJp_invisible(): not a WINDOW\n");
#endif
		return (-1);
	}
	Message.xclient.type = ClientMessage;
	Message.xclient.data.l[0] = XJP_INVISIBLE;
	...
	XFlush (_KeepDisp);
#ifdef DEBUG
	fprintf(stderr,"XJp_invisible():: OK !!\n");
#endif
	return(0);
}
