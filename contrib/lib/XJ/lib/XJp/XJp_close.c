...
/* $Header: XJp_close.c,v 2.100 89/06/01 08:42:10 rws Exp $ */
...
#ifdef DEBUG
	fprintf(stderr,"Call XJp_close!!\n");
#endif
        if ((cnt = W_id_check(request_id)) == MAXWINDOW) {
#ifdef DEBUG
		fprintf(stderr,"ID not WINDOW\n");
#endif
		return (-1);
	}
	Message.xclient.type = ClientMessage;
	Message.xclient.window = _KeepW[cnt][WID];
	Message.xclient.data.l[0] = XJP_CLOSE;
