...
int first;
{
 ...
 if((fp= open_uumrc(first))==NULL){	return -1; }
 dn = 0;
...
int first;
{
 ...
 char *n, buf[1024];
	...
	if (*(n = user_uumrc[i].user_name) != NULL && lookUsername(n)) {
	    strcpy(buf,user_uumrc[i].uumrc_name);
	    if(fp = fopen(buf, "r")){
		if (first) strcpy(dic_info->uumrc_name, buf);
		return fp;
	    }
	}
    }
    if (*(n = user_uumrc[0].uumrc_name) != NULL) {
	strcpy(buf,n);
	if(fp = fopen(buf, "r")){
	    if (first) strcpy(dic_info->uumrc_name, buf);
	    return fp;
	...
	if (first) strcpy(dic_info->uumrc_name, RCFILE);
	return fp;
    }

 fprintf(stdout , "デフォルトのjfewrcがありません。");
 fflush(stdout);
 return(0);
}
...
char *s;
{
	return(*(c_c->user_name) == NULL ? 0 : strmatch(s, c_c->user_name));
}
