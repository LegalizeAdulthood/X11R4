...
push_cursor()
{
    if(cursor_stack_top >= STACKLENGTH ){
        printf("Error Cursor Stack");
    }
    savedupscreen[cursor_stack_top].colum = cursor_colum;
    savedupscreen[cursor_stack_top].reverse = cursor_reverse;
    savedupscreen[cursor_stack_top].underline = cursor_underline;
    savedupscreen[cursor_stack_top].invisible = cursor_invisible;
    cursor_stack_top++;
    reset_cursor_status();
}
...
pop_cursor()
{
    ...
    cursor_stack_top--;
    cursor_normal_raw();
    cursor_colum = savedupscreen[cursor_stack_top].colum;
    cursor_reverse = savedupscreen[cursor_stack_top].reverse;
    cursor_underline = savedupscreen[cursor_stack_top].underline;
    cursor_invisible = savedupscreen[cursor_stack_top].invisible;
    set_cursor_status();
}
#endif	!X11
