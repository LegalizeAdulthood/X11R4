...
#else	WNMO
/* henkan_off は jhlp.c do_main でやてるから、多分要らない。
  if(henkan_off_flag){
...
isconect_jserver()
{
    ...
    if (!jl_isconnect(bun_data_)) {
	print_msg_getc(" jserverとの接続が切れています。");
	return (0);
    }
    return(1);
}
...
henkan_off()
{
	...
	if (TTY->dispmode == XJP_ROOT) {
	    display_title();	/* かな漢字変換ウィンドウ表示 */
	} else {
	    display_henkan_off_mode();
	}
	invisual_note();
	invisual_sheet();	/* 非表示 KURI 63/1/29 */
#else	X11
	if (cur_x->dispmode == 1) {
	    invisual_window1();
	}
...
#ifndef	X11
    if (TTY->dispmode == XJP_ROOT) {
	display_title_off();	 /* かな漢字変換ウィンドウ非表示 */
    } else {
	disp_mode();
    }
#else	X11
