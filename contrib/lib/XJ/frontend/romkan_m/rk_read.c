 ...
 * SOFTWARE. 
 *
 *	Original Source: ./lib/Wnn/romkan/rk_read.c
 *
...
int	*errptr;
{
	...
	*errptr = 0;
	...
	if(fixednamep(name)){
		...
		for(src = cur_rk->pathmeiorg; *src != NULL; src++){
		...
		if(cur_rk->flags & RK_VERBOS){ /* Taio-hyo not found */
			char	*p, *q;
...
char	*modf;		/* モード定義表の名又はそのパス名が入っている */
{
	...
	datptr = data;
	henptr = hensudefhyo;
	*(cur_rk->pathmeiptr = cur_rk->pathmeiorg = pathmeibgn) = NULL;
	*(cur_rk->pathmeimem = pathmeimem_) = '\0';
	*(cur_rk->hyomeiptr = cur_rk->hyomeiorg = hyomeibgn) = NULL;
	*(cur_rk->hyomeimem = hyomeimem_) = '\0';
	*(cur_rk->modmeiptr = cur_rk->modmeibgn) = NULL;
	*(cur_rk->modmeimem = cur_rk->modmeimem_) = '\0';
	*(cur_rk->dspnamptr = cur_rk->dspnambgn) = NULL;
	*(cur_rk->dspcod = cur_rk->dspcod_) = '\0';
	*(cur_rk->naibu = cur_rk->naibu_) = '\0';
	...
	cur_rk->curdir = cur_rk->nulstr;
	readmode(cur_rk->curfnm = modfnm);

	for(i = 0; cur_rk->hyomeiorg[i] != NULL; i++){
		int	err;

		for(j = 0; j < FILNST; j++) cur_rk->nestfile[j] = NULL;
		cur_rk->nestfile[FILNST - 1] = ENDOF_NestFileArea;
		cur_rk->base = cur_rk->nestfile + 1;

		*(cur_rk->base) = trytoopen(cur_rk->curfnm = cur_rk->hyomeiorg[i], &(cur_rk->curdir), &err);
		if(err){
			switch(err){
				case 1:
				case 3: ERHOPN(1);
...
int	n;
{
	...
	cur_rk->hcurread = (char *)linbuf; /* エラー処理用 */
	cur_rk->ltrbufbgn = ltrbuf;
	...
	*(hen_iki = hen_iki_) = EOLTTR;
	...
	cur_rk->hyo_n[n] . hensudef = henptr;
	cur_rk->hyo_n[n] . data = datptr;

	hyosw = cur_rk->hyoshu[n];
	while(readln(linbuf)){
...
letter	get1ltr()
{
	return(get_ltr(cur_rk->lptr));
}

letter	unget1ltr(l)
letter	l;
{
	return(unget_ltr(l, cur_rk->lptr));
}

int_get1ltr() {return((int)(get1ltr()));}
int_unget1ltr(c) letter c; {return((int)(unget1ltr((letter) c)));}
...
int	flg;
{
	return((int)getfrom_dblq(&(cur_rk->lptr), sptr, flg));
}
		...
		case 1: /* fn No.1 include */
			{
				...
				*(s = fnmtmparea) = '\0';
				...
				if(*(++cur_rk->base) == ENDOF_NestFileArea){
					cur_rk->base--;
					ERRLIN(23);
				}
				*(cur_rk->base) = trytoopen(fnmtmparea, &dirnamptr,&err);
				if(err){
					switch(err){
						case 1:
						case 3:
...
register letter **socp, *dest;
{
	...
	*dest++ = ' ';
	...
	while(eofreach = blankpass(socp, 0), **socp != ')'){
		if(eofreach){
			if(! readln((uns_chr *)(cur_rk->hcurread))) ERRLIN(20);
			ustrtoltr((uns_chr *)(cur_rk->hcurread),
					(*socp= cur_rk->ltrbufbgn), 1);
...
letter	*lp;
{
	...
	for(n = 0; cur_rk->modmeibgn[n] != NULL; n++)
		if(!ltrstrcmp(lp, cur_rk->modmeibgn[n])) return(n);
...
unsigned int	n;
{
 ...
 /* 15 */	"User doesn't exist",
	...
	fprintf(stderr, "\r\nMode-hyo %s%s ---\r\n%s\r\n",
		cur_rk->curdir, cur_rk->curfnm, cur_rk->mcurread);
	fprintf(stderr, "%d: %s.\r\n", n, ermes[n]);
...
unsigned int	n;
{
 ...
 /* 25 */	"Can't get home directory",
	...
	fprintf(stderr, "\r\nTaio-hyo %s%s ---\r\n%s\r\n",
		cur_rk->curdir, cur_rk->curfnm, cur_rk->hcurread);
	fprintf(stderr, "%d: %s.\r\n", n, ermes[n]);

	while(NULL != *(cur_rk->base)) fclose(*(cur_rk->base--));
