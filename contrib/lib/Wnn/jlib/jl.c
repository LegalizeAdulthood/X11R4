...
static int dumbhinsi;		/* ADD KURI */
static wchar *mae_fzk;		/* ADD KURI */
static int syuutanv;
static int syuutanv1;		/* ADD KURI */
...
static
WNN_JSERVER_ID *
find_same_server(server_n)
register char *server_n;
{
    register int k;

    for(k = 0 ; k < MAXENVS; k++){
	if(strcmp(envs[k].server_n, server_n) == 0){
	    return(envs[k].js);
	}
    }
    return(NULL);
...
#ifdef nodef
static
delete_server_from_id(js)	/* ADD KURI */
WNN_JSERVER_ID *js;
{
    int k;
    for(k = 0 ; k < MAXENVS; k++){
...
static
add_new_env(js, env, env_n, server_n)
register WNN_JSERVER_ID *js;
register struct wnn_env *env;
char *env_n, *server_n;
{
    ...
    for(k = 0 ; k < MAXENVS; k++){
	if(envs[k].ref_cnt == 0){
	    strcpy(envs[k].server_n, server_n);
...
struct wnn_env *
jl_connect(env_n, server_n, wnnrc_n, error_handler, message_handler, timeout)
register char *env_n, *server_n, *wnnrc_n;
...
int timeout;
{
    ...
    wnn_errorno = 0;
    if((js = find_same_server(server_n)) == NULL){
	if((js = js_open(server_n, timeout)) == NULL){
	    return(NULL);
	}
/*	js_hinsi_list(js); */
    }
	...
	return (NULL);
    if((env = find_same_env(js, env_n)) == NULL){ /* Incr ref_cnt */
	/**********************/
...
#define ALLOC_SET(pter, type, size, size_var) \
    ...
    if(!ALLOC_SET(buf->bun, WNN_BUN *, DEFAULT_BUN_LEN, buf->msize_bun) ||
       ...
       !ALLOC_SET(buf->zenkouho_dai, int, DEFAULT_ZENKOUHO_LEN, buf->msize_zenkouho) ||
   ...
   /* To See jserver aliase in /etc/hosts
      when server_n is not specified. H.T. */
    if(!server_n || !*server_n){
	struct hostent *hent;
	if(hent = gethostbyname("jserver")){
	    server_n = hent->h_name;
	}
    }
    env = jl_connect(env_n, server_n, wnnrc_n, error_handler, message_handler, timeout);
    buf->env = env;
    return(buf);
}
...
register struct wnn_buf *buf;
{
    ...
    for(c = buf->heap; *c; c =  *(char **)c) free(c);
    free((char *)buf);
}
...
int use_maep;
{
	...
	syuutanv = buf->bun[bun_no2]->kangovect;	/* Chnge KURI */
	syuutanv1 = WNN_VECT_KANREN; /* ADD KURI */
    }else{
	syuutanv = WNN_VECT_KANREN;
	syuutanv1 = WNN_VECT_NO; /* ADD KURI */
	if(bun_no2 < buf->bun_suu){ /* ADD KURI */
	    buf->bun[bun_no2]->dai_top = 1;
	}
    }
    if((dcnt = js_kanren(buf->env, yomi, dumbhinsi, mae_fzk,
			 syuutanv, syuutanv1, WNN_VECT_BUNSETSU, &rb)) < 0)
	if_dead_disconnect(buf->env, -1);

    dp = (struct wnn_dai_bunsetsu *)rb.buf;

    free_bun(buf, bun_no, bun_no2);

    if(use_maep & WNN_USE_ATO && bun_no2 < buf->bun_suu){	/* ADD KURI */
...
int use_maep, ich_shop;
{
    ...
    if(use_maep & WNN_USE_ATO && bun_no2 < buf->bun_suu){
	syuutanv = buf->bun[bun_no2]->kangovect;	/* Chnge KURI */
	syuutanv1 = WNN_VECT_KANTAN; /* ADD KURI */
    }else{
...
{
    int cnt;
    wchar yomi[LENGTHBUNSETSU], yomi1[LENGTHBUNSETSU];
    struct wnn_sho_bunsetsu *sp;
    register int k;
	...
	syuutanv = buf->bun[bun_no+1]->kangovect;	/* Chnge KURI */
	syuutanv1 = WNN_VECT_KANZEN; /* ADD KURI */
	buf->zenkouho_endvect = syuutanv;	/* ADD KURI */
    }else{
	syuutanv = WNN_VECT_KANZEN;
	syuutanv1 = WNN_VECT_NO; /* ADD KURI */
	if(bun_no + 1 < buf->bun_suu){ /* ADD KURI */
	    buf->bun[bun_no + 1]->dai_top = 1;
	}
	buf->zenkouho_endvect = -1;	/* ADD KURI */
    }
...
int bun_no, bun_no2, use_maep, uniq_level;
{
    ...
    wnn_errorno = 0;
    if(bun_no2 > (tmp = dai_end(buf, bun_no))) bun_no2 = tmp;
	...
	make_space_for(buf, ZENKOUHO, buf->zenkouho_suu, buf->zenkouho_suu, bun_no2 - bun_no); /* ADD KURI */
	...
	k = get_c_jikouho_dai(dp, cnt, buf->bun, bun_no); /* KURI */
	if(k >= 0){
...
int bun_no, bun_no2;
{
	...
	/* BUG FIX とりあえず */
		...
		if(js_hindo_set(buf->env, wb->dic_no, wb->entry,
		    ...
		    if (wnn_errorno == WNN_JSERVER_DEAD) {
			jl_disconnect(buf->env);return(-1);
		    }
		}
#ifdef JL_DEBUG
    display_bun(wb);
...
int  (*error_handler)(), (*message_handler)();
{
	...
	if((int)error_handler == WNN_NO_CREATE || 
	   (rw == WNN_DIC_RDONLY)){
	    sprintf(tmp, "辞書ファイル \"%s\" が無いよ。",dic_name);
	...
	if(file_exist(env, hindo_name) == -1) {
	    ...
	    if((int)error_handler == WNN_NO_CREATE ||
	       (hrw == WNN_DIC_RDONLY)){
		sprintf(tmp, "頻度ファイル \"%s\" が無いよ。",dic_name);
		message_out(message_handler, tmp);
		wnn_errorno = WNN_NO_EXIST;
		return(-1);
	    }
	...
	if(wnn_errorno == WNN_JSERVER_DEAD){
	    ...
	    sprintf(tmp, "辞書と頻度 \"%s\" の整合性が無いよ。作り直す?(Y/N)",
		    hindo_name);
	    if(!((int)error_handler == WNN_CREATE ||
		 call_error_handler(error_handler, tmp))){
		return(-1);
...
static 
create_pwd_file(pwd_file, error_handler, message_handler)
char *pwd_file;
int  (*error_handler)(), (*message_handler)();
{
    FILE *fp;
    ...
    sprintf(gomi ,"password_file \"%s\" が無いよ。作る?(Y/N)", pwd_file);
	...
	wnn_errorno = WNN_CANT_OPEN_PASSWD_FILE;
	message_out(message_handler, wnn_perror());
	return(-1);
    }
#ifdef SYSVR2
    srand(time(0)+getuid());
...
int zenp, daip;
{
    ...
    len = Strlen(sb->fuzoku);
    wb->yomilen = Strlen(sb->yomi) + len;
    wb->kanjilen = Strlen(sb->kanji) + len;
/*
...
{
    register int k;
    wchar area[LENGTHKANJI];
    wchar area1[LENGTHKANJI];
    register WNN_BUN *b;
...
WNN_BUN *dest;
{
	...
	if(sp->entry == dest->entry && sp->dic_no == dest->dic_no
	   && sp->kangovect == dest->kangovect){
	    if(Strncmp(area, sp->kanji, len = Strlen(sp->kanji)) == 0 &&
	       Strcmp(area + len, sp->fuzoku) == 0){
		return(k);
	    }
	}
    }
...
int bun_no;
{
	...
	for(l = 0 ; l < dp->sbncnt; l++, sp++){
	    /* sp = dp->sbn; KURI */
	    /* KURI
	    if(sp->entry != (dest + l)->entry ||
	       sp->kangovect != (dest + l)->kangovect ||
	       sp->dic_no != (dest + l)->dic_no){
		break;
	    }
	    wnn_area(dest + l, area, WNN_KANJI);
	    KURI */
...
/*		if(sp->kangovect != b->kangovect) break; */
...
int level;
{
    ...
    wnn_errorno = 0;
    if(level > MAXINCLUDE){
	message_out(message_handler, "include のレベルが多過ぎます。");
	return(-1);
    }
    if((fp = fopen(wnnrc_n, "r")) == NULL){
	message_out(message_handler, "file \"%s\"が open できません", wnnrc_n);
	return(-1);
    }
...
register char	*s;
{
	...
	struct	passwd	*u;
        extern char *getenv();
	extern struct	passwd	*getpwnam();	/* ADD KURI */
#ifdef	SYSVR2
	extern char * strchr();
#endif
...
int  (*error_handler)(), (*message_handler)();
{
    ...
    if(d == HINDO){
	if(create_pwd_file(pwd_hindo, error_handler, message_handler) == -1)return(-1);
	if(get_pwd(pwd_hindo, hpwd) == -1)return(-1);
	if(n[0] == C_LOCAL){
		...
		message_out(message_handler,"頻度ファイル”%s”を作りました。",n);
	...
	if(create_pwd_file(pwd_hindo, error_handler, message_handler) == -1)return(-1);
	if(get_pwd(pwd_hindo, hpwd) == -1)return(-1);
	if(create_pwd_file(pwd_dic, error_handler, message_handler) == -1)return(-1);
	if(get_pwd(pwd_dic, pwd) == -1)return(-1);
	if(n[0] == C_LOCAL){
	    if(js_dic_file_create_client(env, n + 1, WNN_REV_DICT, NULL, pwd, hpwd) == -1){
		message_out(message_handler, wnn_perror());
		if_dead_disconnect(env, -1);
	    }else{
		message_out(message_handler, "辞書ファイル”%s”を作りました。",n);
		chown(n + 1, getuid(), -1); /* H.T. */
		return(0);
	    }
	}else{
	    if(js_dic_file_create(env, n, WNN_REV_DICT, NULL, pwd, hpwd)== -1){
		message_out(message_handler, wnn_perror());
		if_dead_disconnect(env, -1);
	    }else{
		message_out(message_handler, "辞書ファイル”%s”を作りました。",n);
...
int  (*error_handler)(), (*message_handler)();
{
    ...
    if((int)error_handler != WNN_CREATE){
	sprintf(gomi ,"directry \"%s\" が無いよ。作る?(Y/N)", dirname);
	if(call_error_handler(error_handler,gomi) == 0){
	    wnn_errorno = WNN_MKDIR_FAIL;
	    return(-1);
	}
...
register int bun_no, bun_no2;
{
    ...
    wnn_errorno = 0;
    if(bun_no2 >= buf->bun_suu || bun_no2 < 0) /* ADD KURI */
	bun_no2 = buf->bun_suu;
