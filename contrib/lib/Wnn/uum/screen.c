...
disp_mode()
{
  char	*p;
/*
  int previous_disp_mode_length;
  int old_vst;
*/
/*
  previous_disp_mode_length = disp_mode_length;
  old_vst = c_b->vst;
*/
  strcpy(rk_modes, (NULL == (p = romkan_dispmode()) ? "[   ]" : p));
/*
  disp_mode_length = strlen(rk_modes);
*/
  push_cursor();
  throw_col(0);
  printf("%s",rk_modes);
/*
  if (disp_mode_length != previous_disp_mode_length){
    c_b->vlen = maxlength * conv_lines - c_b->start_col - 3 - disp_mode_length ;
    c_b->duplicate = max((c_b->vlen + 3) >> 2 , 2);
    if (set_vst() < 0) c_b->vst = old_vst;
    t_print_l();
  }
*/
  pop_cursor();
