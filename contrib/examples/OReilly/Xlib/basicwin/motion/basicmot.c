	...
	while (1)  {
		...
		case MotionNotify:
			printf("got a motion event\n");
                        ...
                        XDrawPoint(display, wint, gc, report.xmotion.x, report.xmotion.y);
                        if (count <= BUF_SIZE)
                                count++;
                        else {
		...
		case Expose:
			...
			if (report.xexpose.window == wint) {
          			while (XCheckTypedWindowEvent(display, wint, Expose, &report));
                        	for (i=0 ; i < (buffer_filled ? BUF_SIZE : count) ; i++)
                             		XDrawPoint(display, wint, gc, xpositions[i], ypositions[i]);
			}
			else {
