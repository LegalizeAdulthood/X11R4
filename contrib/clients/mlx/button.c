...
char	c;
{
/*	fprintf(stderr,"in HcharClass ....\n"); */
	if (c & 0x80 )	return(charClass['a']);
	else		return(charClass[c]);
}
#endif	/* HANZI */
...
#ifdef	HANZI
	/* all orient languages should be exchange normal-reversefont */
		for (i=0;i<ML_MAX;i++) {
			tempgc = L_FONT[i].normalGC;
			L_FONT[i].normalGC = L_FONT[i].reverseGC;
			L_FONT[i].reverseGC = tempgc;
			tempgc = L_FONT[i].norboldGC;
			L_FONT[i].norboldGC = L_FONT[i].revboldGC;
			L_FONT[i].revboldGC = tempgc;
		}
#endif	/* HANZI */
...
#ifdef	HANZI
	if((line = malloc((unsigned) j + (j/2) + 1)) == (char *)NULL)
	/* j/2 is used to store languge-flag which restored from attr buff */
#else
...
#ifdef	HANZI
	MLcode(line);	/* Convert into MLcode from GCcode */
	XStoreBytes(screen->display,line, lp - line); /*3-byte stream stored */
#else
...
#ifdef	HANZI
	register char *att= screen->buf[2 * (row + screen->topline)+1];
	/* get attribut buffer. SaveText is a opposition of WriteText. */
#endif	/* HANZI */
...
register Menu **menu;
{
	...
	if (*menu == NULL) {
#ifdef	HANZI
		if ((*menu = NewMenu("hterm X11R2", term->misc.re_verse)) == NULL)
#else
