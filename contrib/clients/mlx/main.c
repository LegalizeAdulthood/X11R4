...
static struct  sgttyb d_sg = {
        0, 0, 0177, CKILL, ECHO|XTABS|CRMOD
};
...
#ifdef	HANZI
		/* If LPASS8 is set, csh logoff when command is HANZI-string.
		   But TTY is set pass8, HTERM still cannot get 8-bit string,
		   and applications can do. */
		lmode	|= LPASS8; 
		if (ioctl (screen->respond, TIOCLSET, (char *)&lmode) == -1)
...
#endif	/* TIOCLSET */
#else	/* !SYSV */
		sg.sg_flags &= ~(ALLDELAY | XTABS | CBREAK  | RAW |ANYP);
