...
/*
 * $Header: charproc.c,v 1.23 88/06/15 19:27:38 rws Exp $
 */
...
VTparse()
{
	...
	{
		c = doinput();
		if(c&0x80) 		/*  is Language_Flag ? */
			goto NoASCII;
		else switch(parsestate[c]) {
#else
		switch(parsestate[c = doinput()]) {
		 ...
		 case CASE_VMOT:
			...
			if(screen->display->qlen > 0 ||
			 (ioctl(screen->display->fd, FIONREAD, 
					(char *) &arg), arg) > 0)
				xevents();
			break;
...
NoASCII:
		{ 
			...
			l_flag = *bptr;
			cp = bptr;
			while(top>0 && (c= *cp)&0x80 && c==l_flag){
				cp +=3;
				top -=3;
				bcnt -=3;
/* Each time only one language is processed */
			}
			if(bptr < cp)
				dotext(screen, term->flags,'H',bptr,cp);
			bptr = cp;
			continue;
		}
#endif
		...
		if((select_mask & pty_mask) && (eventMode == NORMAL)) {
			...
			if((bcnt = read(screen->respond, bptr = buffer,
			 BUF_SIZE-2)) < 0) {
				...
				if (isflag(buffer[bcnt-2])) byte=1;
				if (isflag(buffer[bcnt-1])) byte=2;
				if(byte>0) {
					byte = read(screen->respond,
							&buffer[bcnt],byte);
					if(byte>0) 
						bcnt += byte;
				} 
				*(bptr+bcnt) = '\0';
				GCcode(bptr);	/* Convert to GCcode */
#else
				/* strip parity bit */
				for(i = bcnt, cp = bptr ; i > 0 ; i--,cp++)
					*cp &= CHAR;
#endif	/* HANZI */
 ...
 * by charset.  The string start from buf end at ptr, and only one language.
 * Worry about end of line conditions (wraparound if selected).
 * That charset is set 'H' indicates  any one of Orient languages.
 */
...
#ifdef	HANZI
	case 'H':	/* No ASCII languages 			*/
		han_flg = buf[0];
		for (s=tmp=buf; tmp < ptr;){
			tmp++;
			*s++ = *tmp++;
			*s++ = *tmp++;
		}
...
#ifdef	HANZI
		WriteText(screen, ptr, n, (flags&0xff)|han_flg);
#else
...

{
   ...
   if(screen->cur_row - screen->topline <= screen->max_row) {
	...
	if(screen->cur_row == screen->cursor_row && screen->cur_col <=
	 screen->cursor_max && screen->cursor_col <= screen->cur_col + len - 1)
	...
	if (fgs & HAN) {
		i = (fgs >> 8) & 0x1f;
		if (L_FONT[i].l_no == OFF) GCset(screen,i);
			/* first time use this language */
		if (fgs & BOLD)
			if (fgs & INVERSE)
				currentGC = L_FONT[i].revboldGC;
			else 	currentGC = L_FONT[i].norboldGC;
		else	
			if (fgs & INVERSE)
				currentGC = L_FONT[i].reverseGC;
			else 	currentGC = L_FONT[i].normalGC;
	} else
#endif	/* HANZI */
...
#ifdef	HANZI
	/* hanzi style is same as ASCII's bold, reverse and underline; */
	if (fgs & HAN) {
		DrawMLString(screen->display, TextWindow(screen), currentGC,
				cx, cy, (XChar2b *)str, len/2, 0);
		if((fgs & BOLD) && screen->enbolden) 
			if (currentGC || screen->reverseGC)
			      DrawMLString(screen->display, TextWindow(screen),
			      	currentGC,cx+1, cy, (XChar2b *)str, len/2, 1);
	}else {
...
XSetWindowAttributes *values;
{
...
#ifdef	HANZI
	FontsInit(screen);	/* Open Configuration File */

		/* set MAX width and height */
	j = screen->fullVwin.f_width;
	for (i=0;i<ML_MAX;i++)
		if( j< L_FONT[i].width) j=L_FONT[i].width;
	screen->fullVwin.f_width = j/2;
	j = screen->fullVwin.f_height;
	for (i=0;i<ML_MAX;i++)
		if( j< L_FONT[i].height) j=L_FONT[i].height;
	screen->fullVwin.f_height = j;
#endif	/* HANZI */
...
#ifdef	HANZI
	for (i=0; i<ML_MAX;i++) {
		if (L_FONT[i].l_no == OFF) continue;
		xgcv.font = L_FONT[i].l_fontid;
		L_FONT[i].normalGC = XtGetGC((Widget)term, mask, &xgcv);
		if (screen->enbolden) {	 /* there is no bold font */
			L_FONT[i].norboldGC =L_FONT[i].normalGC;
		} else {
			xgcv.font = L_FONT[i].l_bold_fontid;
		  L_FONT[i].norboldGC = XtGetGC((Widget)term, mask, &xgcv);
		}
	}
#endif	/* HANZI */
	...
	for (i=0;i<ML_MAX;i++) {
		if (L_FONT[i].l_no == OFF) continue;
		xgcv.font = L_FONT[i].l_fontid;
		L_FONT[i].reverseGC = XtGetGC((Widget)term, mask, &xgcv);

		if (screen->enbolden)  /* there is no bold font */
			xgcv.font = L_FONT[i].l_fontid;
		else 
			xgcv.font = L_FONT[i].l_bold_fontid;
		L_FONT[i].revboldGC = XtGetGC((Widget)term, mask, &xgcv);
	}
#endif	/* HANZI */
...
/*	free((char *)fInfo);	*/
	vp = &VTbox[1];
	(vp++)->x = FontWidthOld(screen) - 1;
	(vp++)->y = FontHeightOld(screen) - 1;
	(vp++)->x = -(FontWidthOld(screen) - 1);
	vp->y = -(FontHeightOld(screen) - 1);
	screen->box = VTbox;
...
#ifdef	HANZI
FontHeightOld(screen)
TScreen	*screen ;
{
	register	i;
	i = screen->fnt_norm->max_bounds.ascent 
		 +screen->fnt_norm->max_bounds.descent;
	return(i);
}
FontWidthOld(screen)
...
{
	register	i;
	i = screen->fnt_norm->min_bounds.width;
	return(i);
}
#endif	/* HANZI */
...
ShowCursor()
{
	...
	if (c == 0)
		...
		if (!(flags &0x80)){
			...
			h[0] =  screen->buf[y][x];
			h[1] = c;
			flags = screen->buf[y+1][x];	/* language flag */ 
			flags <<= 8;		
			flags |=screen->buf[y+1][x+1];	/* real attribut */
			back = 1;
		}else {
			h[0] = c;
			h[1] = screen->buf[y][x+1];
			flags <<= 8;			/* language flag */
			flags |= screen->buf[y+1][x+1]; /* real attribut */
			back = 0;
		}
/*	fprintf(stderr,"h0=%x,h1=%x,flags=%x HAN=%x\n",h[0],h[1],flags,HAN);	*/
	}
#endif	/* HANZI */
...
#ifdef	HANZI
		if (flags & HAN) {
			i = (screen->buf[y+1][x]) & 0x1f;
		  	if(flags & INVERSE) { /* is reverse video */
		    		if (screen->cursorGC) 
					currentGC = screen->cursorGC;
		    		else 
					if (flags & BOLD) 
						currentGC = L_FONT[i].norboldGC;
					else 
						currentGC = L_FONT[i].normalGC;
		  	} else { /* normal video */
...
#ifdef	HANZI
	if (flags&HAN){
		x = CursorX (screen, screen->cur_col-back);
	}else
#endif
...
#ifdef	HANZI
	back = CursorX (screen, screen->cur_col);
	if (flags &HAN ) {
	DrawMLString(screen->display, TextWindow(screen), currentGC,
		x, y, (XChar2b *)h, 1 ,0);
	...
	if(flags & UNDERLINE) 
		XDrawLine(screen->display, TextWindow(screen), currentGC,
			x, y+1, x + FontWidth(screen)*2, y+1);
	} else {		/* No hanzi */
#endif	/* HANZI */
	XDrawImageString(screen->display, TextWindow(screen), currentGC,
		x, y, &c, 1);
...
HideCursor()
{
	...
	if (flags&HAN)
		x = CursorX (screen, screen->cursor_col-back);
	else
#endif	/* HANZI */
	...
	if (flags & HAN ){
		DrawMLString(screen->display, TextWindow(screen), 
				currentGC, x, y, (XChar2b *)h, 1, 0);
		if((flags & BOLD) && screen->enbolden)
			DrawMLString(screen->display, TextWindow(screen), 
				currentGC, x + 1, y, (XChar2b *)h, 1, 1);
		if(flags & UNDERLINE) 
			XDrawLine(screen->display, TextWindow(screen), 
				currentGC, x,y+1, x+FontWidth(screen)*2, y+1);
	}else {
#endif	/* HANZI */
