...
register int length;		/* length of string */
{
	...
	while(length-- > 0)
#ifdef	HANZI
		/* This function only called by WriteText which has
		 * converted from 3-byte stream to code-attr stream.
		 * So, the goal of this function is store code-attr
		 * in screen buffer. Fellowing screen operations can 
		 * not consider 3-byte format.
		*/
		if (*str++ & 0x80) {
			*att++ = flags>>8;  /* language-flag */
			*att++ = flags;
			str++;
			length--;
		}else
...
int toprow, leftcol, nrows, ncols;
{
	...
	for (row = toprow; row <= maxrow; y += FontHeight(screen), row++) {
	   ...
	   while (col <= maxcol && (att[col] & ~BOLD) == 0 &&
		...
		col++;
#ifdef	HANZI
	/* adjustify left-col to HANZI-first-byte. */
	   if ((chars[col]&0x80) && (!(att[col]&0x80)) && col) col--;
#endif	/* HANZI */
	   ...
	   while (col <= maxcol && (att[maxcol] & ~BOLD) == 0 &&
		...
		maxcol--;
#ifdef	HANZI
	/* adjustify right-col to HANZI-first-byte. */
	   if ((chars[maxcol]&0x80) && (!(att[maxcol]&0x80) )) maxcol++;
#endif	/* HANZI */

	   if (col > maxcol) continue;
#ifdef	HANZI
	   if (chars[col] & 0x80) {	/* hanzi */
loopHAN:
		han_mode = 1;
	   	flags = att[col];
		current_l = flags&0x1f;
		flags <<= 8;
		flags |= att[col+1] & 0xff;
	   	if (flags & INVERSE)
	       		if (flags & BOLD) gc = L_FONT[current_l].revboldGC;
	       		else gc = L_FONT[current_l].reverseGC;
	   	else 
	       		if (flags & BOLD) gc = L_FONT[current_l].norboldGC;
	       		else gc = L_FONT[current_l].normalGC;

	   	x = CursorX(screen, col);
	   	lastind = col;

	   	for (; col <= maxcol; col++) {
			register int	col_flg;

	   		col_flg = att[col];
			col_flg <<= 8;
			col_flg |= att[col+1];
			if ((col_flg ==flags) && (chars[col] & 0x80)) {
				col ++;  /* attr unchanged & still hanzi */
				continue;
			}else {		  /* if (att[col] != flags) { 	  */
				n=(col-lastind)/2;
		   		DrawMLString(screen->display,TextWindow(screen),
				   gc, x, y, (XChar2b *) &chars[lastind], n,0);
		   		if((flags & BOLD) && screen->enbolden)
		 		DrawMLString(screen->display,TextWindow(screen),
			 	   gc, x+1, y,(XChar2b *) &chars[lastind], n,1);
		   		if(flags & UNDERLINE) 
				XDrawLine(screen->display, TextWindow(screen), 
			 	   gc, x, y+1, x+2*n*FontWidth(screen), y+1);

		   		x += 2 * n * FontWidth(screen);
			}
			if (!(chars[col] & 0x80)) 
				han_mode = 0;	/* ASCII */
			break;			/* break for */
		}	 /* end for */
		if (col > maxcol) goto endHrow;
		if (han_mode == 1 ) goto loopHAN;	/* An other Orient Language */
	     }		/* end if hanzi */
...
#ifdef	HANZI
	   if (col >maxcol ) goto  endArow;
	   if (han_mode) goto loopHAN;
	   else goto loopASC;
endArow:
#else
	   flags = att[col];
