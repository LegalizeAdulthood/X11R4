...
#ifdef	HANZI
	{
		...
		switch(Tparsestate[c = input()]) {
		 ...
		 case CASE_OSC:
			...
			break;
#ifdef HANZI
NoLatin: /*	 case CASE_NO_LATIN_LANGUAGE: */
			{	char	hanzi[3],l_flag;
				...
				DrawMLString(
					screen->display, TWindow(screen), 
			    		L_FONT[l_flag].normalGC,
(int)(screen->cur_X                     * TekScale(screen))+screen->border,
(int)((TEKHEIGHT+TEKTOPPAD-screen->cur_Y)*TekScale(screen))+screen->border,
			    		(XChar2b *)hanzi, 1,0
				);
			THanCursorForward(L_FONT[l_flag].width *6 );
			break;
			}
		}
#endif
	...
	if(Tbcnt-- <= 0) {
		...
		for( ; ; ) {
			if(Tselect_mask & pty_mask) {
				...
				if (isflag(Tbuffer[bcnt-2])) byte=1;
				if (isflag(Tbuffer[bcnt-1])) byte=2;
				if(byte>0) {
				byte=read(screen->respond,&Tbuffer[bcnt],byte);
					if(byte>0) 
						Tbcnt += byte;
				} 
				Tbuffer[Tbcnt] = 0;
...
#ifdef	HANZI
THanCursorForward(width) /* forward a hanzi but down a ascii */
int	width;		 /* Forward width, Tek unit */
{
	register TScreen *screen = &term->screen;
	register struct Tek_Char *t;
	register int l;

	if( ( screen->cur_X += width ) > TEKWIDTH) {
...
#ifdef  HANZI
    XtAddEventHandler(new, PropertyChangeMask, FALSE,
		      MLCodeInput, (Opaque)NULL);
#endif
