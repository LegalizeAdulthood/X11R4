...
mcube(isoWin,d, xn, yn, zn, xi, xj, yi, yj, zi, zj, values, numvalues)
	...
	int	xi, xj, 	/* range of data to plot, for each dimension */
		yi, yj, 
		zi, zj;
	...
	int numvalues;	/* length of values */
{
	...
	for (i = 0; i < numvalues; i++)
		mcubeOneValue(isoWin,values[i], xi, xj, yi, yj, zi, zj);
}
	...
	int V[4];	/* vertices .. */
{
	...
	int numTriangles;
	InterpCoor p[3],q[3];	/* three points of interpolated triangle */
	int axis = isoWin->axesOrient.axis;
	int	edge;
