...
int maxsize;
{
    ...
    Dilist = (DFdi *) DFIgetspace((unsigned)(maxsize * sizeof(DFdi)));
    if (!Dilist) {
        DFerror = DFE_NOSPACE;
        return(-1);
    }
...
uint16 tag, ref;
{
    ...
    ret = DFputelement(dfile, tag, ref, (char*)Dilist,(int32)Ndi*sizeof(DFdi));
    DFIfreespace((char*)Dilist);
    Dilist = NULL;		/* flag value */
    Dinlist = Ndi = 0;
    return(ret);
 ...
 DFdi diarray[], *groupdi;
{
    ...
    ret = dlep->dd[cdd].length / sizeof(DFdi);
