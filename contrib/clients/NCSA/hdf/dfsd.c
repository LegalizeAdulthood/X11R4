			...
			DFIgetspace((unsigned) sdg->dimsizes[i] *
                    ...
                    if (userNT == fileNT) {     /* no conversion needed */
					  ...
					  fileNTsize);
                        ...
                        p = buf;
				/* convert */
                        for (j=0; j<sdg->dimsizes[i]; j++)
                            DFconvert(p+j*fileNTsize,
				      (char*) &sdg->dimscales[i][j],
				      DFNT_FLOAT, fileNT, userNT);
		...
		/* read coordsys */
            ...
            case DFTAG_SDM:	/* max/min */
                if (fileNT==userNT) {       /* no conversion */
                    ...
                    if (DFgetelement(dfile, elmt.tag, elmt.ref, buf)<0)
                        return(-1);
                    DFconvert(buf, (char*) &sdg->max, DFNT_FLOAT, fileNT,
			      userNT);
                    DFconvert(buf+fileNTsize, (char*) &sdg->min, DFNT_FLOAT,
			      fileNT, userNT);
                    buf = DFIfreespace(buf);
                }
                Ismaxmin = 1;
...
DFSsdg *sdg;
{
            ...
            if (localNT==outNT) {       /* no conversion needed */
                ...
                if (buf==NULL) {
                    ...
                    return(-1);
                        ...
                        /* convert */
                for (i=0; i<sdg->dimsizes[j]; i++)
                    DFconvert((char*) &sdg->dimscales[j][i], &buf[i*outNTsize],
                                            DFNT_FLOAT, localNT, outNT);
                        /* write it all out */
    ...
    if (!Ref.maxmin) {
        if (localNT == outNT) {     /* no conversion */
            if (DFputelement(dfile, DFTAG_SDM, ref, (char*) &sdg->max,
			     (int32) (2 * sizeof(float32)))<0) return(-1);
            Ref.maxmin = ref;
        }
        else {
	    /* allocate buffer */
            buf = DFIgetspace((unsigned) 8); /* max/min is 8 bytes */
            if (buf==NULL) return(-1);

	    /* convert */
            DFconvert((char*) &sdg->max, buf, DFNT_FLOAT, localNT, outNT);
            DFconvert((char*) &sdg->min, buf+outNTsize, DFNT_FLOAT, localNT,
		      outNT);
	    /* write */
...
DF *dfile;
{
    ...
    /* find next sdg */
    if (DFIfind(dfile, DFTAG_SDG, DFREF_WILDCARD, !Readsdg.data.ref,
...
int isfortran;
{
    ...
    ret = DFSDIgetslice(filename, winst, Readsdg.dimsizes, data, maxsizes,
                                                                isfortran);
...
#ifdef DF_CAPFNAMES
int DSISDAS(label, unit, format, coordsys, isfortran, llabel, lunit,
                                                            lformat, lcoordsys)
#else /*DF_CAPFNAMES*/
int dsisdas_(label, unit, format, coordsys, isfortran, llabel, lunit,
                                                            lformat, lcoordsys)
#endif
...
#ifdef DF_CAPFNAMES
int DSISDIS(dim, label, unit, format, isfortran, llabel, lunit, lformat)
#else /*DF_CAPFNAMES*/
int dsisdis_(dim, label, unit, format, isfortran, llabel, lunit, lformat)
#endif
...
int isfortran;
{
    int rank, conv, ret, leastsig, done;
    /* leastsig - fastest varying subscript in the file */
    int32 i, j, readsize, *offset;
    /* readsize - how many bytes to read on each read */
    int32 *posn,		/* elements being read currently */
          fileoffset,
          copystart, copysize;  /* start position, len to read for least sig */
    float32 *datap;
    char *buf=NULL, *inp;
    DF *dfile;
    ...
    for (i=0; i<rank; i++) {
        if ((windims[i]<=0) || (winst[i]<=0) ||	/* negative dims */
    ...
    userNT = (DF_MT>>8) & 0x0f;	/* get third nibble from right */
    conv = (fileNT != userNT);	/* is conversion necessary */

    leastsig = FileTranspose ? 0 : rank-1; /* which is least sig dim */
    readsize = Readsdg.dimsizes[leastsig]*fileNTsize; /* # bytes to read */
    copysize = windims[leastsig]; /* # floats to copy at a time */
    copystart = (winst[leastsig]-1)*fileNTsize; /* byte of buf to start copy */

    /* position at start of image */
    if (DFaccess(dfile, Readsdg.data.tag, Readsdg.data.ref, "r")<0)
        return(DFIerr(dfile));

    /* compute starting position in file, seek to it */
    fileoffset = 0;
    for (i=rank-1; i>0; i--)    /* cum offset from most sig to next to least */
        if (FileTranspose)
            fileoffset = (fileoffset+winst[i]-1) * Readsdg.dimsizes[i-1];
        else
            fileoffset = (fileoffset+winst[rank-1-i]-1) *
		Readsdg.dimsizes[rank-i];
    fileoffset *= fileNTsize;   /* convert to bytes */

    if (fileoffset) {           /* fileseek if necessary */
        ret = DFseek(dfile, fileoffset);
        if (ret<0)
            return(DFIerr(dfile));
    }

    /* array posn is used to keep track current position in data */
    posn = (int32 *) DFIgetspace((unsigned) rank * sizeof(int32));
    if (posn==NULL)
        return(DFIerr(dfile));

    for (i=0; i<rank; i++)      /* compute initial position in data */
        posn[i] = 0;		/* convert 1-based winst to 0-based */

    buf = DFIgetspace((unsigned) readsize); /* allocate read buffer */
    if (buf==NULL) {
        DFIfreespace((char*)posn);
	DFerror = DFE_NOSPACE;
        return(DFIerr(dfile));
    }

    /* array offset is used for accessing the next elt */
    offset = (int32 *) DFIgetspace((unsigned) rank * sizeof(int32));
    if (offset==NULL) {
	DFIfreespace((char*) posn);
	DFIfreespace(buf);
	DFerror = DFE_NOSPACE;
    ...
    do {			/* loop till dest array filled */
	ret = DFread(dfile, buf, readsize); /* read 1 row/col */
        if (ret<0) {
	...
	for (j=0;j<copysize && !done;j++) {
	    if (conv)
		DFconvert(inp,(char*) datap, DFNT_FLOAT, fileNT, userNT);
	    else
		*datap = *((float *) inp);
		...
		i = 0;
		do {
		    if (posn[i] < windims[i]-1) {
			datap += offset[i];
			posn[i]++;
			break;
		    } else {
			datap -=offset[i]*posn[i];
			posn[i]=0;
		    }
		    i++;
		} while (i<rank);
...
int isfortran;
{
    int rank, conv, done, ret, leastsig;
    /* leastsig - fastest varying subscript in the array */
    int32 i, j, rowsize, writesize, *posn, *offset;
    /* windims - array containing dimensions of slice */
    /* writesize - how many bytes to write on each write */
    /* posn - elements being written currently */
    float32 *datap;
    char *buf=NULL, *outp;
	...
	for (i++;i<rank-1;i++)
	    if (Sddims[i] >= dims[i]) {
		Sddims[i] -= dims[i]; /* promote the unit */
		Sddims[i+1] += 1;
	    }
    } else {
	/* find the first significant dimension */
    ...
    userNT = (DF_MT>>8) & 0x0f;	 /* get third nibble from right */
    conv = (outNT != userNT);	 /* is conversion necessary */

    leastsig = Fortorder ? 0 : rank-1;  /* which is least sig dim */
    rowsize = windims[leastsig];
    writesize = rowsize*outNTsize;
    
    buf = DFIgetspace((unsigned)writesize); /* allocate buffer of 1 line */
    if (buf==NULL) {
	DFerror = DFE_NOSPACE;
	return(-1);
    }
    ...
    do {
	outp = buf;		 /* start of one-line buffer */
	
	for (j=0;j<rowsize && !done;j++) {
	    if (conv)
		DFconvert((char*) datap, outp, DFNT_FLOAT, userNT, outNT);
	    else
		*((float32 *) outp) = *datap;
	    outp += outNTsize;
	...
	ret = DFwrite(Sdfile, buf, writesize);
    } while (!done && ret >= 0);
