...
static int Grreqil[2]= {-1, -1}; /* requested lut/image il */
...
int ncomps, il;
{
    return(DFGRIsetdims(xdim, ydim, ncomps, il, LUT));
}
...
int ncomps, il;
{
    return(DFGRIsetdims(xdim, ydim, ncomps, il, IMAGE));
}
...
int DFGRIsetdims(xdim, ydim, ncomps, il, type)
int32 xdim, ydim;
int ncomps, il;
int type;
{
...
int type, isfortran;
{
    ...
    if (Ref.dims[type]<0)	/* if dims not set, set dimensions */
        if (DFGRIsetdims(xdim, ydim, 1, 0, type)<0) return(-1);
