...
int ndds;
{
    ...
    if (!DFlist) {
        DFlist = (DF *) malloc(DF_MAXDFS * sizeof(DF));
    ...
    if (created || !(DFlist[i].file = DF_OPEN(name, DF_RDACCESS))) {
...
DF *dfile;
{
    ...
    dfile->list= (DFdle *) malloc(sizeof(DFdle)); /* includes one DD - unused */
