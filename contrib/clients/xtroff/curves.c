 ...
 *
 *	Troff_curves.c - Code to draw the fancy curves needed in the
 *		ditroff spec.  Code written at Berkeley and cleaned up
 *		just a bit at SPAR.
 *
...
#define FALSE	0
				/* imports from dver.c */
#define  hmot(n)	HorizontalMove(n)
#define  vmot(n)	VerticalMove(n)
#define  vgoto(n)	VerticalGoto(n)

extern int HorizontalPosition;
extern int VerticalPosition;
extern int DisplayOutput;
extern point();
extern int scale();
 ...
 *		from current (HorizontalPosition, VerticalPosition) to
 *		(HorizontalPosition + dh, VerticalPosition + dv).
 *
 * Side Efct:	Resulting position is at end of line (HorizontalPosition + dh,
 *							VerticalPosition + dv)
 *----------------------------------------------------------------------------*/
...
{
    if (DisplayOutput) {
	    HGtline (HorizontalPosition, VerticalPosition,
		     HorizontalPosition + dh, VerticalPosition + dv);
    }
...
/*----------------------------------------------------------------------------
 * Routine:	DrawCircle (diameter)
 *
 * Results:	Draws a circle with leftmost point at current
 *		(HorizontalPosition, VerticalPosition)
 *		with the given diameter d.  Does a thick line by calling
 *		RoundEnd many times with varying radii.
 *
 * Side Efct:	Resulting position is at (HorizontalPosition + diameter,
 *						VerticalPosition)
 *----------------------------------------------------------------------------*/
...
int d;
{
	...
	while (delta_rad <= limit) {
	    RoundEnd (Scale(HorizontalPosition)+radius,
		      Scale(VerticalPosition), delta_rad, FALSE);
	    delta_rad++;
	}
    }
...
/*----------------------------------------------------------------------------
 * Routine:	DrawEllipse(horizontal_diameter, vertical_diameter)
 *
 ...
 *
 * Side Efct:	Resulting position is at (HorizontalPosition + hd,
 *						VerticalPosition).
 *---------------------------------------------------------------------------*/
...
int vd;
{
    ...
    register int y;
    					...
    					/* bases == coordinates of center */
    basex = Scale(HorizontalPosition + (hd >> 1));	
    hmot(hd);				/* horizontal motion */
    					/*   (HorizontalPosition should */
					/*   NOT be used after this) */
    basey = Scale(VerticalPosition);	
 ...
 *
 * Results:	Draws an arc starting at current (HorizontalPosition,
 *		VerticalPosition).  Center is at (HorizontalPosition + cdh,
 *		VerticalPosition + cdv) and the terminating point is
...
register int pdv;
{
    ...
    if (DisplayOutput) {
	    HGArc(HorizontalPosition + cdh, VerticalPosition + cdv,
		  HorizontalPosition, VerticalPosition, (int) angle);
    }
    hmot(cdh + pdh);
    vmot(cdv + pdv);
}
...
/*----------------------------------------------------------------------------
 * Routine:	DrawSpline (character_buffer, type_flag)
 *
 * Results:	Given the starting position, the motion list in buf, and any
 *		extra characters from the file (terminated by a \n), drawwig
 *		sets up a point list to make a spline from.  If "pic" is set
 *		picurve is called to draw the curve in pic style; else it
 *		calls HGCurve for the gremlin-style curve.
 *
...
int pic;
{
    ...
    register int npts = 1;		/* point list index */
	...
	if (len - (ptr - buf) < 15 && *(buf + len - 1) != '\n') {
	    ...
	    (void) GetLine(cop-1, len-(cop-buf));
...
int npts;
{
	...
	/* skip first point and zero'th entry*/
	npts-=1; x+=2; y+=2;
	for ( i = 0; i < npts; i++ )
		HGtline(x[i], y[i], x[(i+1)%npts], y[(i+1)%npts]);
}
