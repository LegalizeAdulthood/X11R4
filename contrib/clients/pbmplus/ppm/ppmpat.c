...
#define PAT_TEST 9
	...
	{
	if ( strncmp(argv[argn],"-gingham2",max(strlen(argv[argn]),9)) == 0 ||
	     strncmp(argv[argn],"-g2",max(strlen(argv[argn]),3)) == 0 )
	    {
	    if ( pattern != PAT_NONE )
		pm_error( "only one base pattern may be specified", 0,0,0,0,0 );
	    pattern = PAT_GINGHAM2;
	    }
	else if ( strncmp(argv[argn],"-gingham3",max(strlen(argv[argn]),9)) == 0 ||
	     strncmp(argv[argn],"-g3",max(strlen(argv[argn]),3)) == 0 )
	    {
	    if ( pattern != PAT_NONE )
		pm_error( "only one base pattern may be specified", 0,0,0,0,0 );
	    pattern = PAT_GINGHAM3;
	    }
	else if ( strncmp(argv[argn],"-madras",max(strlen(argv[argn]),2)) == 0 )
	    {
	    if ( pattern != PAT_NONE )
		pm_error( "only one base pattern may be specified", 0,0,0,0,0 );
	    pattern = PAT_MADRAS;
	    }
	else if ( strncmp(argv[argn],"-tartan",max(strlen(argv[argn]),2)) == 0 )
	    {
	    if ( pattern != PAT_NONE )
		pm_error( "only one base pattern may be specified", 0,0,0,0,0 );
	    pattern = PAT_TARTAN;
	    }
	else if ( strncmp(argv[argn],"-poles",max(strlen(argv[argn]),2)) == 0 )
	    {
	    if ( pattern != PAT_NONE )
		pm_error( "only one base pattern may be specified", 0,0,0,0,0 );
	    pattern = PAT_POLES;
	    }
	else if ( strncmp(argv[argn],"-squig",max(strlen(argv[argn]),2)) == 0 )
	    {
	    if ( pattern != PAT_NONE )
		pm_error( "only one base pattern may be specified", 0,0,0,0,0 );
	    pattern = PAT_SQUIG;
	    }
	else if ( strncmp(argv[argn],"-camo",max(strlen(argv[argn]),2)) == 0 )
	    {
	    if ( pattern != PAT_NONE )
		pm_error( "only one base pattern may be specified", 0,0,0,0,0 );
	    pattern = PAT_CAMO;
	    }
	else if (strncmp(argv[argn],"-anticamo",max(strlen(argv[argn]),2)) == 0)
	    {
	    if ( pattern != PAT_NONE )
		pm_error( "only one base pattern may be specified", 0,0,0,0,0 );
	    pattern = PAT_ANTICAMO;
	    }
	else if ( strncmp(argv[argn],"-test",max(strlen(argv[argn]),3)) == 0 )
	    {
	    if ( pattern != PAT_NONE )
		pm_error( "only one base pattern may be specified", 0,0,0,0,0 );
	    pattern = PAT_TEST;
    ...
    ppm_writeppm( stdout, pixels, cols, rows, PPM_MAXMAXVAL );
...
pixval maxval;
	...
	cols3, rows, PPMD_NULLDRAWPROC, &fore1color );
