#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)win_treeop.c% 20.40 89/11/21";
#endif
#endif
...
#ifdef PRE_R4_ICCCM
#define XWithdrawWindow	xv_withdraw_window
#endif
 ...
 * of the window device name. In SunView 2, a link is an XID for a window,
    ...
    int             linkname;
{
	...
	fprintf(stderr, "win_getlink: XQueryTree failed!\n");
	goto deallocate;
    }
    switch (linkname) {
      case WL_ENCLOSING:
	...
	if (nchildren) {
	    ...
	    if (*c != window_xid) {
		fprintf(stderr, "win_getlink(sibling): window not in tree\n");
	...
	break;
      default:
	fprintf(stderr, "win_getlink: unknown linkname: %d\n", linkname);
	break;
    }
    ...
    XID             number;
{
	...
	if (number == 0) {
	    xv_error(window,
		     ...
		     XV_ERROR_STRING,
		     "Call to win_setlink() with ZERO link value would orphan a child. Ignored.",
	...
	break;
      default:
	fprintf(stderr, "win_setlink: unknown linkname: %d\n", linkname);
	return;
    }
    ...
    Xv_object       window;
{
    ...
    XWithdrawWindow(xv_display(info), xv_xid(info), DefaultScreen(xv_display(info)));
...
win_name_for_qualified_xid(name, display, xid)
    ...
    XID             xid;
{
    return sprintf(name, "%s:%d", XDisplayString(display), xid);
}
...
win_nametonumber(name)
    char           *name;
{
    char           *number_part, *rindex();

    number_part = rindex(name, ':');
    if (number_part == 0)
        return (-1);
    ...
    XID             xid;
{
    ...
    /* CHECK: Should be complain about what kind of error it is */
    if (XFindContext(display, xid, CONTEXT, &result)) {
	result = 0;
    }
    return (result);
}
