#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)fs.c 20.31 89/11/21";
#endif
#endif
    ...
    int            *offset_ptr;
{
    ...
    if (xv_get(fullscreen->input_window,
	       (Attr_attribute)WIN_IS_IN_FULLSCREEN_MODE)) {
	xv_error(XV_NULL, XV_ERROR_NON_RECOVERABLE, XV_ERROR_INTERNAL,
		 XV_ERROR_STRING, "Already in fullscreen mode!",
		 XV_ERROR_PKG, ATTR_PKG_FULLSCREEN, NULL);
	return (XV_ERROR);
    } else {
    ...
    *offset_ptr = xv_set_embedding_data((Xv_opaque)myfullscreenstruct,
    ...
    server_set_fullscreen(xv_get(xv_default_screen,
				 (Attr_attribute)SCREEN_SERVER),
			  TRUE);
    return (XV_OK);
}
    ...
    int	code;
{
    switch (code) {
      case (AlreadyGrabbed):
        return ((char *)
		"Already in fullscreen by another client. Fullscreen Failed.");
        
      case (GrabInvalidTime):
        return ((char *)"Invalid time. Fullscreen failed.");
        
      case (GrabNotViewable):
        return ((char *)"Fullscreen window or confine-to window not mapped. Fullscreen failed.");
        
      case (GrabFrozen):
        return ((char *)"Already frozen by another client. Fullscreen Failed.");
