#ifndef	lint
#ifdef sccs
static char     sccsid[] = "@(#)ndis_d_pri.c 20.10 89/08/17 Copyr 1985 Sun Micro";
#endif
#endif
    ...
    Notify_error_func func;
{
	...
	if (bits_ptr->fds_bits[i])	/* For each fd_mask set in     */
	    /* bits_ptr, mask off all but  */
	    /* one byte and see if anything */
	    /* is set...                   */
	    for (byte = 0xff, byteNum = 0; byte != 0L;
		 byte <<= NBBY, byteNum++)
		if (bits_ptr->fds_bits[i] & byte)
		    /* If a byte is set, find out */
		    /* which bit is set...        */
		    for (fd = byteNum * NBBY;
			 fd < byteNum * NBBY + NBBY; fd++)
