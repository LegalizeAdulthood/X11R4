#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)svrim_ops.c 20.27 89/11/20";
#endif
#endif
    ...
    unsigned long   op;
{
	...
	if (src_type == SERVER_IMAGE_PR) {
	    ...
	    xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		     XV_ERROR_STRING,
		     "server_image_rop(): src is not a server image",
	...
	break;
      case SERVER_IMAGE_PR:{
		...
		gc = xv_find_proper_gc(display, dest, info, PW_ROP);
	...
	xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		 XV_ERROR_STRING,
	"server_image_rop(): dest is not a memory pixrect or a server_image",
		 XV_ERROR_PKG, ATTR_PKG_SERVER_IMAGE,
		 0);
	return (PIX_ERR);
    }
    ...
    unsigned long   op;
{
	...
	xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		 XV_ERROR_STRING,
		 "server_image_stencil(): stencil is not a memory pr or a server image",
		 XV_ERROR_PKG, ATTR_PKG_SERVER_IMAGE,
		 0);
	return (PIX_ERR);
    }
	...
	if (src_type == SERVER_IMAGE_PR) {
		...
		xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
			 XV_ERROR_STRING,
			 "server_image_stencil(): Can't create mpr in server_image_stencil",
			 XV_ERROR_PKG, ATTR_PKG_SERVER_IMAGE,
			 0);
		return (PIX_ERR);
	    }
	    ...
	    xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		     XV_ERROR_STRING,
		  "server_image_stencil(): dest is mpr, src isn't image pr",
		     XV_ERROR_PKG, ATTR_PKG_SERVER_IMAGE,
		     0);
	    return (PIX_ERR);
	}
	break;
      case SERVER_IMAGE_PR:{
	    ...
	    XV_OBJECT_TO_STANDARD(dest, "server_image_stencil",
				  dest_standard);
	    stencil_gc = xv_find_proper_gc(display, dest, dest_info, PW_STENCIL);
	...
	xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		 XV_ERROR_STRING,
	       "server_image_stencil(): dest is not mpr or server_image_pr",
		 XV_ERROR_PKG, ATTR_PKG_SERVER_IMAGE,
		 0);
	return (PIX_ERR);
	break;
    ...
    unsigned long   op;
{
    ...
    switch (dest_type) {
      case MEMORY_PR:{
		...
		xv_replrop(temp, 0, 0, dw, dh, PIX_SRC, src, sx, sy);
      ...
      case SERVER_IMAGE_PR:{
	    ...
	    XV_OBJECT_TO_STANDARD(dest, "server_image_replrop",
				  dest_standard);
	    replrop_gc = xv_find_proper_gc(display, dest, info, PW_REPLROP);
    ...
    int             x, y, w, h;
{
    xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
	     XV_ERROR_STRING,
	     "server_image_region: Unsupported operation",
	     XV_ERROR_PKG, ATTR_PKG_SERVER_IMAGE,
	     0);
    return (NULL);
}
    ...
    unsigned char   red[], green[], blue[];
{
    xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
	     XV_ERROR_STRING,
	     "Server images do not have associated colormaps",
	     XV_ERROR_PKG, ATTR_PKG_SERVER_IMAGE,
	     0);
    return (PIX_ERR);
}
    ...
    char           *string;
{
    ...
    display = xv_display(info);
    xid = xv_xid(info);
    gc = xv_find_proper_gc(display, rpr.pr, info, PW_TEXT);
