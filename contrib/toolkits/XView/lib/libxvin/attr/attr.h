...
#define ATTR_PKG_LAST_VALUE		ATTR_PKG_FIRST + 12
...
#define ATTR_VALID_PKG_ID(attr)		\
	(((int)ATTR_PKG(attr)) > ((int) ATTR_PKG_ZERO_VALUE) && \
	 ((int)ATTR_PKG(attr)) <= ((int)ATTR_PKG_LAST_VALUE))
...
#define attr_cu_to_y(encoded_value, font, top_margin, row_gap) 	\
    attr_rc_unit_to_y(encoded_value, font->pf_defaultsize.y, top_margin, \
	row_gap)
#endif	lint
 ...
 *	  to support cut and paste between SunView1 and SunXView windows.
 *	  Nothing changes!
 */
typedef enum {
