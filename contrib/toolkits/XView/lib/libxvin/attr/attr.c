#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)attr.c 20.16 89/11/21";
#endif
#endif
...
static char    *attr_int_names[ATTR_PKG_LAST - ATTR_PKG_FIRST + 1] = {
    "Generic",
    "Cursor",
    "Drawable",
    "Font",
...
char           *
attr_pkg_name(pkg_nbr)
    register int    pkg_nbr;
{
    if (pkg_nbr >= (int) ATTR_PKG_FIRST && pkg_nbr <= (int) ATTR_PKG_LAST)
	return attr_int_names[pkg_nbr - (int) ATTR_PKG_FIRST];
    else
	return 0;
}
    ...
    register Attr_attribute attr;
{
    ...
    char           *name = attr_pkg_name((int) ATTR_PKG(attr));
	...
	(void) sprintf(s, "Attr pkg= %d, id= %d, cardinality= %d (0x%x)",
