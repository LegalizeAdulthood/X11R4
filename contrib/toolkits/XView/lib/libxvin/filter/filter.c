#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)filter.c 20.16 89/11/21";
#endif
#endif
    ...
    char           *filename;	/* use this to label errors */
{
	...
	if (rec == NULL) {
	    ...
	    (void) sprintf(dummy,
			   "while parsing filter file %s", filename);
	    xv_error(XV_NULL, XV_ERROR_RECOVERABLE, XV_ERROR_SYSTEM,
	...
	if (!strcmp(scratch, "\n")) {
	    ...
	    (void) sprintf(dummy,
			   "filter file %s: missing command-line", filename);
...
error:
		...
		break;
	{
	    ...
	    (void) sprintf(dummy,
			   "problem parsing filter file %s", filename);
	    xv_error(XV_NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		     XV_ERROR_STRING, dummy, NULL);
	}
errorNoSkip:
	{
	    ...
	    if (pos.lineno != -1)
		(void) sprintf(dummy,
			       "problem on line number %d", pos.lineno);
	    else
		(void) sprintf(dummy,
	...
	(void) sprintf(dummy, "while parsing filter file %s", filename);
	xv_error(XV_NULL, XV_ERROR_RECOVERABLE, XV_ERROR_SYSTEM,
		 XV_ERROR_STRING, dummy, NULL);
	return ((struct filter_rec **)NULL);
    }
