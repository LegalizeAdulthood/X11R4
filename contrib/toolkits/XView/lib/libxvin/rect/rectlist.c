#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)rectlist.c 20.9 89/08/18";
#endif
#endif
    ...
    register struct rectlist *rl;
{
	...
	xv_error(rl, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		 XV_ERROR_STRING, "Malformed rl in rl_free",
		 0);
    }
    *rl = rl_null;
}
    ...
    char           *tag;
{
    ...
    (void) fprintf(stderr, "%s: Bounding ", tag);
    ...
    register struct rect *r;
{
	...
	if (!rnTable || rnTableIndex == RNTABLEINC) {	/* table empty */
	    ...
	    rnTable = (struct rectnode *) calloc(
