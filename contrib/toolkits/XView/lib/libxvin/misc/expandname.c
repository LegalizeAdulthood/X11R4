#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)expandname.c 20.12 89/08/17";
#endif
#endif
...
char           *index();
char           *malloc();
struct namelist *xv_make_0list();
struct namelist *xv_make_1list();
    ...
    char            name[];
{
	...
	xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		 XV_ERROR_STRING, "\"Echo\" failed", 0);
	return (NONAMES);
    }
    if (length == 0) {
	return (xv_make_0list());
    }
    if (length == NCARGS) {
	char            dummy[128];

	(void) sprintf(dummy, "Buffer overflow (> %d)  expanding \"%s\"",
		       NCARGS, name);
	xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
...
xv_make_0list()
{
	...
	xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_SYSTEM,
		 XV_ERROR_STRING, "in xv_expand_name", 0);
    } else {
	result->count = 0;
