#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)sel_agent.c 1.47 89/11/27";
#endif
#endif
    ...
    Seln_agent_info *agent;
{
	...
	perror("Agent couldn't reply about a file");
	return SELN_FAILED;
    }
    if (context->context == 0) {
	if (lseek(fd, 0L, 0) == (long) -1) {
	    perror("Agent couldn't reset to start of file");
	    return SELN_FAILED;
	}
    }
    switch (attr) {
...
register Atom  *current_value;
{
        ...
        if ((*current_value = XInternAtom(dpy, prop_name, FALSE)) == None) 
                xv_error(XV_NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
			 XV_ERROR_STRING,
			"Couldn't get selection atom, selections may not work.",
			 XV_ERROR_PKG_NAME, "Selection",
			 0);
