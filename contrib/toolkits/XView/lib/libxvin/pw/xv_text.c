#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)xv_text.c 20.27 89/08/18";
#endif
#endif
    ...
    struct pixfont *pixfont;
{
    ...
    str[0] = c;
    str[1] = NULL;
    (void) xv_text(pw, xw, yw, op, pixfont, str);
}
    ...
    char           *str;
{
    ...
    d = xv_xid(info);
    gc = xv_find_proper_gc(display, window, info, PW_TEXT);
    ...
    int		    color_index;   /* -1 => use default foreground color */
{
    ...
    s[1] = NULL;
    gc = xv_find_proper_gc(display, window, info, PW_TEXT);
    gc1 = xv_find_proper_gc(display, window, info, PW_ROP_NULL_SRC);
    /*
