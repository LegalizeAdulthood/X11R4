#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ttyselect.c 20.24 89/09/12";
#endif
#endif
    ...
    int             start, end, row;
{
    ...
    if ((end + 1) < strlen(str)) {	/* This is a very dirty trick for
					 * speed */
	temp_char = str[end + 1];
	str[end + 1] = '\0';
    ...
    *te = *tb;
	...
	break;
      case SEL_WORD:{
		...
		chr = line[col];
		if (!isascii(chr) || (!isalnum(chr) && chr != '_'))
		    break;
	    }
    ...
    register Seln_function_buffer *buffer;
{
	...
	break;
      ...
      default:
	xv_error(ttysw, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		 XV_ERROR_STRING,
		 "ttysw didn't recognize function to perform on selection",
		 XV_ERROR_PKG_NAME, "Tty",
		 0);
	break;
    }
...
continue_reply:
	...
	if (context->context == (char *) NULL) {
		...
		xv_error(ttysel, XV_ERROR_RECOVERABLE, XV_ERROR_SYSTEM,
			 XV_ERROR_STRING, "failed for selection copy-out",
			 XV_ERROR_PKG_NAME, "Tty",
			 0);
		return SELN_FAILED;
	    }
    ...
    register Seln_request *buffer;
{
    ...
    ttysw = context->ttysw;
    response_ptr = (Seln_attribute *) buffer->data;
