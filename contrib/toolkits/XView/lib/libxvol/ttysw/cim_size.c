#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)cim_size.c 20.22 89/09/12";
#endif
#endif
...
static char    *lines_ptr, *mode_ptr;	/* BUG ALERT! Get rid of this global. */
static char   **temp_image, **temp_mode;	/* BUG ALERT! Get rid of this
						 * global. */
static char    *temp_lines_ptr, *temp_mode_ptr;	/* BUG ALERT! Get rid of this
						 * global. */

Pkg_private void ttysw_pos();
    ...
    Ttysw_view_handle ttysw_view;
{
	...
	if (sl > ttysw_right)
	    sl = ttysw_right;
	bcopy(image[oldrow], temp_image[row], sl);
	bcopy(screenmode[oldrow], temp_mode[row], sl);
