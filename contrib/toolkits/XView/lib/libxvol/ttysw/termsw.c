#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)termsw.c 1.37 89/09/12";
#endif
#endif
...
Pkg_private Es_handle ts_create();
    ...
    Textsw_attribute avlist[];
{
    ...
    if (tty_view_init(parent, termsw_view_public, (Tty_attribute *)avlist)
	== XV_ERROR) {
	goto Error_Return;
    }
    /*
    ...
    Textsw_attribute avlist[];
{
    ...
    /* Initialized to ttysw */
    if (tty_folio_init(parent, termsw_public, (Tty_attribute *)avlist)
	== XV_ERROR) {
	goto Error_Return;
    }
    ...
    (void) ioctl(fd, TIOCGETP, &termsw_folio->sgttyb);
    (void) ioctl(fd, TIOCGETC, &termsw_folio->tchars);
    (void) ioctl(fd, TIOCGLTC, &termsw_folio->ltchars);
    ...
    Attr_avlist     avlist;
{
	...
	switch ((int) attrs[0]) {
	  ...
	  case TERMSW_MODE:{
		...
		ttysw_setopt((caddr_t) ttysw, TTYOPT_TEXT,
    ...
    *               termsw_object = (Xv_termsw *) termsw_folio_public;
	...
	break;
      ...
      case DESTROY_CLEANUP:{
	    ...
	    while (temp_view) {
		next = temp_view->next;
		xv_destroy(TERMSW_VIEW_PUBLIC(temp_view), status);
		temp_view = next;
	    }
	    termsw_object->parent_data.private_data = termsw_object->private_tty;
	    result = xv_tty_pkg.destroy(termsw_folio_public, status);
