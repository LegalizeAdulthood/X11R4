#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_attr.c 20.60 89/11/02";
#endif
#endif
    ...
    Attr_avlist     attrs;
{
    ...
    Textsw_attribute *view_attrs = (Textsw_attribute *) avarray;
    ...
    for (; *view_attrs; view_attrs = textsw_attr_next(view_attrs),
	...
	switch ((Textsw_attribute) (*attrs)) {
	  case XV_END_CREATE:
		...
		if (is_folio) {
			...
			view->scrollbar = xv_create(textsw_public, SCROLLBAR, 0);
	  ...
	  case TEXTSW_CLIENT_DATA:
	    textsw->client_data = (caddr_t) attrs[1];
	    break;
	  ...
	  case WIN_SET_FOCUS:{
		...
		*attrs = (Textsw_attribute) ATTR_NOP(*attrs);
		...
		FORALL_TEXT_VIEWS(textsw, view_next) {
		    Xv_Window       view_public = WINDOW_FROM_VIEW(view_next);
	  ...
	  case WIN_CMS_CHANGE:
		...
		FORALL_TEXT_VIEWS(textsw, view_next) {
		    ...
		    window_set_cms(view_public, cms, xv_bg(info), xv_fg(info));
		    /* Scrollbar always inherits colors from the textsw */
		    window_set_cms(view_next->scrollbar, cms, xv_bg(info), 
				   xv_fg(info));
		}
	    } else {
    ...
    register Textsw_attribute attribute;
{
	...
	return ((Xv_opaque)
		defaults_get_character("keyboard.deleteChar",
				       "Keyboard.DeleteChar", DEL));	/* ??? Keymapping
									 * strategy? */
      case TEXTSW_EDIT_BACK_WORD:
	return ((Xv_opaque)
		defaults_get_character("keyboard.deleteWord",
				       "Keyboard.DeleteWord", CTRL(W)));	/* ??? Keymapping
										 * strategy? */
      case TEXTSW_EDIT_BACK_LINE:
	return ((Xv_opaque)
		defaults_get_character("keyboard.deleteLine",
				       "Keyboard.DeleteLine", CTRL(U)));	/* ??? Keymapping
										 * strategy? */
      case WIN_FONT:{
	    PIXFONT        *font;

	    /* Text.d may have "" rather than NULL, so check for this case.  */
	    def_str = defaults_get_string("font.name", "Font.Name", (char *) 0);
	    font = (def_str && (strlen(def_str) > 0)) ? xv_pf_open(def_str) : 0;
	    return ((Xv_opaque) font);
	}
      case TEXTSW_HISTORY_LIMIT:
	return ((Xv_opaque)
	...
	if (def_str && (strlen(def_str) > 0)) {
	    ...
	    return (Xv_opaque) TEXTSW_IF_AUTO_SCROLL;
      case TEXTSW_LINE_BREAK_ACTION:
	...
	if (def_str && (strlen(def_str) > 0)) {
	    ...
	    return (Xv_opaque) TEXTSW_WRAP_AT_WORD;
      case TEXTSW_LOWER_CONTEXT:
	return ((Xv_opaque)
    ...
    va_list         args;
{
	...
	return ((Xv_opaque)
      ...
      case TEXTSW_CLIENT_DATA:
	return ((Xv_opaque) folio->client_data);
      case TEXTSW_COALESCE_WITH:
    ...
    va_list         args;
{
    ...
    /*
     * Special case XV_IS_SUBTYPE_OF since this is used in an xv_get() call
     * in  textsw_view_abs_to_rep()
     */
    if (attribute == (Textsw_attribute) XV_IS_SUBTYPE_OF) {
	*status = XV_ERROR;
	return ((Xv_opaque) 0);
    }
    folio = FOLIO_ABS_TO_REP(abstract);
