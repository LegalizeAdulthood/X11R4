#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)txt_selsvc.c 20.35 89/11/02";
#endif
#endif
...
#define	SAME_HOLDER(folio, holder)				\
	seln_holder_same_client(holder, FOLIO_REP_TO_ABS(folio))
    ...
    int             retry_okay;
{
	...
	fprintf(stderr, "textsw selection_client is null\n");
	return (FALSE);
    }
#ifdef WAS_X11
...
#endif
	    fprintf(stderr, "seln_client returned null");
	    abort();
	}
    }
    return (TRUE);
...
{
    		int             	result = 0;
    register	Attr_avlist		attr;
    		int             	got_contents_ascii = FALSE, to_read;
    register    Textsw_selection_handle	selection = context->selection;

    if (context->continued_attr != SELN_REQ_END_REQUEST) {
	return (TFS_ERROR);
    }
    for (attr = (Attr_avlist) reply->data; *attr;
	 attr = attr_next(attr)) {
	switch ((Seln_attribute) (*attr)) {
	  ...
	  case SELN_REQ_UNKNOWN:
	    ...
	    break;
	  default:
	    if ((attr == (Attr_avlist) reply->data) ||
