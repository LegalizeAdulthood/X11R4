#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ev_display.c 20.30 89/11/02";
#endif
#endif
    ...
    int             on;
{
	...
	FORALLVIEWS(views, view) {
		...
		/*
		 * the following test is true, and the render skipped, if
		 * this is the
		 */
		/*
		 * first render of the caret after the screen was cleared by
		 * the ttysw
		 */
		if (!termsw_caret_invalid())
		    ev_put_caret(view->pw, which,
				 private->caret_pr_pos.x,
				 private->caret_pr_pos.y);

		/*
		 * printf("%3d: remove at %3d,%3d\n", ++rcount,
		 * private->caret_pr_pos.x, private->caret_pr_pos.y );
		 */
