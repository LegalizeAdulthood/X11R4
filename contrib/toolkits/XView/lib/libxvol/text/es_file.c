#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)es_file.c 20.15 89/11/02";
#endif
#endif
 ...
 * write(2), we force WRITE_BUF_LEN > BUFSIZE, forcing all full-buffer
...
es_file_move_write_buf(private, include, also_include, include_offset)
    ...
    char          **include_offset;
/*
 * Caller ensures: include <= also_include < include+WRITE_BUF_LEN <
 ...
 */
{
	 ...
	 * also_include < buf->start+WRITE_BUF_LEN 2) Slide buffer around to
