#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ps_impl.c 20.21 89/10/26";
#endif
#endif
    ...
    Es_handle       original, scratch;
{
	...
	xv_error(scratch, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
		 XV_ERROR_STRING, "ps_create(): cannot reset scratch stream",
...
AllocFailed:
    xv_error(NULL, XV_ERROR_RECOVERABLE, XV_ERROR_INTERNAL,
	     XV_ERROR_STRING, "ps_create(): alloc failure",
	     XV_ERROR_PKG_NAME, "Textsw",
	     0);
    return (NULL);
}
	...
	if (read_count < to_read) {
		...
		if (*resultp == 0 &&
		    ...
		    *resultp = read_count;
			...
			fprintf(console_fd, "Text has been lost in a cmdtool transcript because the maximum edit log size has been exceeded.\n");
			fflush(console_fd);
    ...
    Attr_avlist	    attrs;
{
    ...
    caddr_t         notify_data = (caddr_t) NULL;
    Es_index        undo_mark;
    Es_index        first;
    Es_handle       to_recycle;
	...
	*status = ES_INVALID_TYPE;
	...
	switch ((Es_attribute) * attrs) {
	  case ES_CLIENT_DATA:
	    private->client_data = (caddr_t) attrs[1];
	    break;
