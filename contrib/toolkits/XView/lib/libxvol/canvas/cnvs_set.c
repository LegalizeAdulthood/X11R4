#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)cnvs_set.c 20.30 89/11/21";
#endif
#endif
    ...
    Attr_avlist     avlist;
{
	...
	switch (attr) {
	  ...
	  case CANVAS_MARGIN:
	    /*
	     * margin = (int) avlist[1]; if (margin < 0) ok = FALSE; else if
	  ...
	  case WIN_CMS_CHANGE:
		 ...
		 OPENWIN_EACH_VIEW(canvas_public, view_public)
		     ...
		     window_set_cms(view_public, cms, bg, fg);
		     window_set_cms(view->paint_window, cms, bg, fg);
		     if (sb = (Scrollbar) xv_get(canvas_public, 
					         OPENWIN_HORIZONTAL_SCROLLBAR, 
					         view_public)) {
			 window_set_cms(sb, cms, bg, fg);
		     }
