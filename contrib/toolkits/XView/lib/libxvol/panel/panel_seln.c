#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)panel_seln.c 20.18 89/10/24";
#endif
#endif
    ...
    register Panel_info *panel;
{
    ...
    panel->seln_client = (Xv_opaque) seln_create(panel_seln_function,
						 panel_seln_request,
						 (char *) panel);
    ...
    register Seln_function_buffer *buffer;
{
	...
	panel_seln_acquire(panel, (Panel_info *) 0,
			   SELN_SHELF, TRUE);
#else
#ifdef notdef
	...
	break;
      ...
      case SELN_DELETE:{
		...
		*(selection_string + selection_length) = save_char;
		...
		panel->shelf = panel_strsave("");
	    panel_seln_acquire(panel, (Panel_item *) 0,
			       SELN_SHELF, TRUE);
...
#endif	/* ecd_input */
		    panel_handle_event(ITEM_PUBLIC(panel->caret), &event);
		}
		free(char_buf);
	    }
	    break;
    ...
    int             max_length;
{
...
#ifdef ecd_panel
	{
		...
		panel_get_text_selection(selection->ip,
		     &selection_string, &selection_length, selection->rank);
	}
#else
