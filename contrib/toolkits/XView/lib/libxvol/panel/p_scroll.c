#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_scroll.c 20.16 89/10/24 Copyr 1984 Sun Micro";
#endif
#endif
    ...
    long unsigned  *vs;
{
	...
	if (align_to_max)
	    ...
	    normalize_top(panel, scrolling_up, &offset);
    } else {
	if (align_to_max)
	    normalize_right(panel, pw, scrolling_up, &offset);
	else
	    normalize_left(panel, scrolling_up, &offset);
    }
    *vs = offset;
    ...
    int            *offset;
{
    ...
    int             target = *offset + viewable_height(panel, pw);
    int             intersects = FALSE;
    ...
    top = bottom + panel->v_margin - viewable_height(panel, pw);
    if (top <= panel->v_margin)
	top = 0;
    ...
    int            *offset;
{
    ...
    int             target = *offset + viewable_width(panel, pw);
    int             intersects;
    ...
    left = right + panel->h_margin - viewable_width(panel, pw);
    if (left <= panel->h_margin)
	left = 0;
