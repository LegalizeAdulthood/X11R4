#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_view.c 20.16 89/08/18";
#endif
#endif
...
panel_view_init(parent, view_public, avlist)
    ...
    Panel_attribute avlist[];
{
    Xv_Window       pw = (Xv_Window) xv_get(view_public, CANVAS_VIEW_PAINT_WINDOW);
    Xv_Screen       screen = (Xv_Screen) xv_get(pw, XV_SCREEN);

    /* register the new paint window */
    if (pw != NULL) {
	(void) xv_set(pw,
		      WIN_RETAINED,
