#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)p_image.c 20.20 89/10/24";
#endif
#endif
    ...
    Pixfont        *font;
{
    ...
    if (!font)
	font = (Pixfont *) xv_get(PANEL_PUBLIC(panel), WIN_FONT);
    ...
    where.pos.y = panel_fonthome(font, string);

    xv_text(where.pr, where.pos.x, where.pos.y, PIX_SRC, font, string);
