#ifndef lint
#ifdef sccs
static char     sccsid[] = "@(#)ow_view.c 1.35 89/10/17";
#endif
#endif
...
static          openwin_regiister_initial_sb();
    ...
    Xv_openwin_info *owin;
{
    ...
    /* set that sb's being removed so femove code in layout */
    /* won't get called */
    ...
    int             view_start;
{
	...
	if (direction == OPENWIN_SPLIT_HORIZONTAL) {
	    ...
	    xv_set(sb, SCROLLBAR_VIEW_START, view_start / (int) xv_get(sb, SCROLLBAR_PIXELS_PER_UNIT), 0);
	}
    }
    ...
    Openwin_view_info *to_view;
{
    ...
    view_length = (direction == SCROLLBAR_VERTICAL) ? (int) xv_get(to_view->view, WIN_HEIGHT) : (int) xv_get(to_view->view, WIN_WIDTH);
    ...
    Openwin_view_info **new_view;
{
    ...
    *new_view = NULL;
    ...
    openwin_view_rect_from_avail_rect(owin, (twin != NULL) ? twin : view, r);
...
{
    openwin_remove_scrollbars(view);
    xv_destroy_status(view->view, xv_get_destruction_type());
    ...
    Openwin_view_info *view;
{
    ...
    if (vsb != NULL) {
	xv_destroy_status(vsb, xv_get_destruction_type());
    }
    if (hsb != NULL) {
	xv_destroy_status(hsb, xv_get_destruction_type());
    }
}
    ...
    Rect           *r;
{
	...
	xborders = status_is(owin, show_borders) ? TRUE : FALSE;
    }
	...
	view->view = (Xv_Window) xv_create(OPENWIN_PUBLIC(owin),
					   owin->view_class,
