 ...
 *
 * Description:
 *    This program demonstrates the combined use of the XView color model/API
 *    and Xlib graphics calls. The program uses XView to create and manage 
 *    its colormap segment while doing its actual drawing using Xlib.
 *    The program draws the X logo in red, green and blue.
 */
#include <X11/Xlib.h>
...
int	pixel_table[NUM_COLORS];

/*
 *		main()
 *    Create a frame and a canvas.
 *    Allocate read-only colors (called a static colormap segment in XView 
 *    parlance) and associate colors with the canvas.
 *    The indices into an XView colormap segment always range from 0 to 
 *    size-1, where size is the number of colors allocated in the colormap 
 *    segment. These logical index values translate into actual indices into 
 *    the colormap map as allocated by the X server. The WIN_X_COLOR_INDICES 
 *    attribute returns the actual colormap indices. The indices are returned 
    ...
    Xv_xrectlist *xrects;
{
	...
	XFillRectangle(display, xid, gc, 64, 64, xlogo64_width, xlogo64_height);

	pixel_value = (long)pixel_table[GREEN];
	XSetForeground(display, gc, pixel_value);
	XFillRectangle(display, xid, gc, 192, 64, xlogo64_width, xlogo64_height);

	pixel_value = (long)pixel_table[BLUE];
	XSetForeground(display, gc, pixel_value);
	XFillRectangle(display, xid, gc, 320, 64, xlogo64_width, xlogo64_height);
}
