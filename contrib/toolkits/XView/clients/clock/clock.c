...
#define		MIN_ANALOG_WIDTH	192
#define		MIN_ANALOG_HEIGHT	192
#define		MIN_DIG_WIDTH		192
#define		MIN_DIG_HEIGHT		64
...
typedef struct {
	...
	} Clock_Display_Info,  *Clock_Display;
	...
	Notify_arg arg; Notify_event_type type;
{
	...
	if (event_action(event) == ACTION_CLOSE) {
		...
		xv_set (c->options->frame, XV_SHOW, FALSE, 0);
	}
...
icon_timer_expired (me, which)
{
	...
	if (date_on (c->options) && tm->tm_hour==0 && tm->tm_sec==0) 
		 paint_date (c);	
	return (NOTIFY_DONE);
}
...
/*		FRAME_PROPS_RESET_PROC, clock_reset,
		FRAME_PROPS_APPLY_PROC, clock_apply,
*/
		0);
...
/*		XV_KEY_DATA, key, c,		*/
	...
	o-> defaults_button =
		...
		0);
	o-> faceStr =
		(char *) xv_create (o-> panel, PANEL_MESSAGE,
		PANEL_LABEL_BOLD, TRUE,
...
init_display (c)
	Clock c;
{
	Clock_Display d	= (Clock_Display)c->display;
	d->secondhand.lastSecX	= -1;
	...
	int  argc; char **argv;
{
	...
	clck-> display	= (Clock_Display) malloc (sizeof (Clock_Display_Info));
