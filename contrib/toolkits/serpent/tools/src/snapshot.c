static char rcsid [] =
        "$Header: /u/04c/mnt/integ/tools/src/RCS/snapshot.c,v 1.5 89/09/05 13:47:08 little Exp $";

/*
$Log:	snapshot.c,v $
 * Revision 1.5  89/09/05  13:47:08  little
...
enum orientation orientation;
{
	...
	if (obuf==0) {
	    fprintf(stderr,"snap: cannot allocate %d bytes\n",owidth*oheight);
	    exit(1);
	}
...
int dscale;
{
  ...
  if ((image->bits_per_pixel >= 8) && (scale > 1))
	scale = scale >> 1;
  if (image->bits_per_pixel > 1)
	image = transform(image);
  else if (reverse) bit1_reverse(image);
...
unsigned int n;
{
  ...
  XQueryTree(dpy, win, &root, &parent, &children, &num);
  if (n > (num-1))
	return win;
  else {
	win = children[n-1];
	if (debug > 1) {
		printf(stderr, "window selected = %x", win);
	}
	XFree(children);
	return win;
  }
...
char *argv[];
{
  ...
  if (argc > 1)
	...
	if (argv[i][0] == '-') {
		...
		case 'c':
			...
			break;
		case 'd':
			where = (char *)malloc(strlen(argv[i+1] + 1));
			strcpy(where, argv[i+1]);
			++i;
			break;
		case 'h':
			fprintf(stderr, "Usage: snap -s <size> -a -r -h -w -d <display_name> -c <debug level>\n");
			fprintf(stderr, "	a: autoresize == -s 0\n");
			fprintf(stderr, "	s: size of magnification\n");
			fprintf(stderr, "	r: root window snapshot\n");
			fprintf(stderr, "	h: help- this menu\n");
			fprintf(stderr, "	w: reverse background\n");
			exit(0);
			break;
		case 'w':
			reverse = 1;
  ...
  dpy = XOpenDisplay(where);
  if (dpy == NULL) {
	fprintf("Cannot open display\n");
	exit(1);
  }
