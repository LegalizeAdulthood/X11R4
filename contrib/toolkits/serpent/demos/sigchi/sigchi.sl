...
vc: title_slide_vc
  ...
  objects:
    ...
    major_heading(j1, slide_form, "Serpent, A Media Independent User", 200)
    major_heading(j2, slide_form, "Interface Management System", 260)
...
vc: slide8_vc
  ...
  objects:
    ...
    subbullet(j6, "- media independent application", 590)  
...
vc: slide11_vc
  ...
  objects:
    ...
    title(j1, "Constraint Mechanism")
    bullet(j2, "* Uses one way constraints", 250)
    bullet(j3, "* Implemented using `eager evaluation' technique", 320)
    bullet(j4, "  at run-time", 370)
    bullet(j5, "* Automatically used to maintain data items in", 440)
    bullet(j6, "  appropriate states", 490)
    subbullet(j7, "- object attributes and variables are automatically", 560)
    subbullet(j8, "  re-evaluated based on constraints", 600)
    subbullet(j9, "- calculations within methods are not re-evaluated", 650)
...
vc: slide12_vc
  ...
  objects:
    ...
    title(j1, "View Controllers - 1")
    bullet(j2, "* Slang productions are called `view controllers'", 250)
    bullet(j3, "* Map application objects into technology objects", 320)
    bullet(j4, "* Collects objects that are created or destroyed", 390)
    bullet(j5, "  together", 440)
...
vc: slide13_vc
  ...
  objects:
    ...
    title(j1, "View Controllers - 2")
    bullet(j2, "* Are templates which, when instantiated, make", 250)
    bullet(j3, "  collection of objects visible", 290)
    bullet(j4, "* Have `creation conditions' to control instantiation", 360)
    bullet(j5, "* Bound to particular instance of application data", 430)
    bullet(j6, "  which causes instantiation", 480)
    bullet(j7, "* Can be nested", 550)
...
vc: slide15_vc
  ...
  creation condition: (slide = 15)

  objects:

    title(j1, "Communication Between Layers")
    bullet(j2, "* Same for both applications and I/O technologies", 250)
    bullet(j3, "* Data format description for application or", 320)
    bullet(j4, "  technology generated from special Serpent interface", 370)
    bullet(j5, "  description language", 420)
    bullet(j6, "* Communications bundled into transactions", 490)
...
vc: slide16_vc
  ...
  creation condition: (slide = 16)

  objects:

    title(j1, "Communication Mechanism")
    bullet(j2, "* In terms of records and components of records", 250)
    bullet(j3, "* From perspective of application", 320)
    subbullet(j4, "- C Structures", 390)
    subbullet(j5, "- Ada Packages", 440)
    bullet(j6, "* From perspective of technology objects", 510)
    subbullet(j7, "- attributes", 580)
    subbullet(j8, "- methods", 630)
