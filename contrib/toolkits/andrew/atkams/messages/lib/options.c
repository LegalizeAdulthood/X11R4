...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atkams/messages/lib/RCS/options.c,v 2.23 89/11/02 14:11:54 cfe Exp $ */
/* $ACIS: $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atkams/messages/lib/RCS/options.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atkams/messages/lib/RCS/options.c,v 2.23 89/11/02 14:11:54 cfe Exp $ ";
#endif /* lint */
...
int IntegerDefaults[] = {8, 12, 8, 50, 12, 1};
...
static struct OptionChoice Options[] = {
	...
	{"File into... menu size", OT_INTEGER, OT_INTEGER_MAXCLASSMENU, 1, "This option tells Messages how big you will allow your 'File into...' menu to grow; by default, it is limited to 8 items.  This is primarily of interest to people with MANY mail folders."},
...
int which, hisrock;
{
	...
	if (parm == OT_INTEGER_MAXCLASSMENU) ams_ResetClassList();
...
*/
    ...
    et = environment_InsertStyle(captext->rootEnvironment, cappos, CapStyle, 1);
    cappos += len;
    text_AlwaysInsertCharacters(captext, cappos, "\n", 1);
    environment_SetLength(et, len);
    ++cappos;
    text_AlwaysInsertCharacters(t, tpos, Intro1, len);
    et = environment_InsertStyle(t->rootEnvironment, tpos, MajorHeadingStyle, 1);
    tpos += len;
	...
	tpos += len;
	text_AlwaysInsertCharacters(t, tpos, "\n\377\n\n", 4);
	environment_SetLength(et, len + 3);
	v = value_New();
	if (Options[i].OptType == OT_EXPLEVEL) {
	    ...
	    text_AlwaysAddView(t, tpos + 1, "onoffV", v);
	} else if (Options[i].OptType == OT_INTEGER) {
	    text_AlwaysAddView(t, tpos + 1, "sliderV", v);
	    ...
	    text_AlwaysAddView(t, tpos + 1, "buttonV", v);
	}
	tpos += 4;
