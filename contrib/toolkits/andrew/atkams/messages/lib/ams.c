...
struct classheader *c;
{
    static struct ams *myams = NULL;
    if (!myams) {
	message_DisplayString(NULL, 10, "Loading message server libraries...");
	im_ForceUpdate();
#ifdef SNAP_ENV
	if (IWantSnap) {
	    myams = (struct ams *) amss_New();
	} else {
	    myams = (struct ams *) amsn_New();
	}
#else /* SNAP_ENV */
	if (IWantSnap) {
	    ams_ReportError(ams_GetAMS(), "No separate message server available: SNAP_ENV was not set at system build time.", ERR_WARNING, FALSE, 0);
	}
	myams = (struct ams *) amsn_New();
#endif /* SNAP_ENV */
	if (!myams || ams_CUI_Initialize(myams, NULL, MyRock)) {
	    ams_ReportError(myams, "Error initializing; program terminated.", ERR_FATAL, FALSE, 0);
	    exit(-1); /* not reached */
	}
    }
    return(myams);
...
long ams__CUI_GetFileFromVice(self, tmpfile, vfile) 
struct ams *self;
char *tmpfile, *vfile;
{
...
UpdateServerState() {
    ...
    im_ForceUpdate();
    if (ams_MS_FastUpdateState(ams_GetAMS())) {
...
static struct event *MyInteractionEvent = NULL;
...
boolean DoQuit, MustQuit, MayPurge, OfferQuit;
{
	...
	if (!MustQuit) DisplayAMS_ERRNO("Cannot update state");
...
long dummy, size;
{
    ...
    fclose(fp);
    ams_WaitCursor(FALSE);
    if (ct == 0) return(message_Invalid);
