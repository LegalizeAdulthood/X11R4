...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atkams/messages/lib/RCS/sendmsg.c,v 2.31 89/11/03 17:31:16 cfe Exp $ */
/* $ACIS: $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atkams/messages/lib/RCS/sendmsg.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atkams/messages/lib/RCS/sendmsg.c,v 2.31 89/11/03 17:31:16 cfe Exp $ ";
#endif /* lint */
...
HandleButton(sendmessage, whichbut, butt, action)
...
enum view_MouseAction action;
{
	...
	if (amsutil_GetOptBit(whichbut)) {
	    ...
	    butter_SetText(butt, No);
	} else {
	    amsutil_SetOptBit(whichbut, 1);
	    butter_SetText(butt, Yes);
	}
    }
    return(0);
}
...
struct sendmessage *sendmessage;
{
    struct lpair *headeroptlp, *butterset, *butterset2, *butterset3, *butterset4;
    ...
    sendmessage->bblind = butter_New();
    sendmessage->bclear = butter_New();
    sendmessage->bsign = butter_New();
    sendmessage->bhide = butter_New();
    sendmessage->breset = butter_New();
    sendmessage->bbv = butterview_New();
    sendmessage->bcv = butterview_New();
    sendmessage->bsv = butterview_New();
    sendmessage->bhv = butterview_New();
    sendmessage->brv = butterview_New();
    butterview_SetDataObject(sendmessage->bbv, sendmessage->bblind);
    butterview_SetDataObject(sendmessage->bcv, sendmessage->bclear);
    butterview_SetDataObject(sendmessage->bsv, sendmessage->bsign);
    butterview_SetDataObject(sendmessage->bhv, sendmessage->bhide);
    butterview_SetDataObject(sendmessage->brv, sendmessage->breset);
    butter_SetText(sendmessage->bblind, amsutil_GetOptBit(EXP_KEEPBLIND) ? "Will Keep Copy" : "Won't Keep Copy");
    butter_SetText(sendmessage->bclear, amsutil_GetOptBit(EXP_CLEARAFTER) ?  "Will Clear" : "Won't Clear");
    butter_SetText(sendmessage->bsign, amsutil_GetOptBit(EXP_SIGNMAIL) ?  "Will Sign" : "Won't Sign");
    butter_SetText(sendmessage->bhide, amsutil_GetOptBit(EXP_HIDEAFTER) ? "Will Hide" : "Won't Hide");
    butter_SetText(sendmessage->breset, "Reset");
    butter_SetHitFunction(sendmessage->bblind, HandleButton);
    butter_SetHitFunction(sendmessage->bclear, HandleButton);
    butter_SetHitFunction(sendmessage->bsign, HandleButton);
    butter_SetHitFunction(sendmessage->bhide, HandleButton);
    butter_SetHitFunction(sendmessage->breset, HandleButton);
    butter_SetRocks(sendmessage->bblind, sendmessage, EXP_KEEPBLIND);
    butter_SetRocks(sendmessage->bclear, sendmessage, EXP_CLEARAFTER);
    butter_SetRocks(sendmessage->bsign, sendmessage, EXP_SIGNMAIL);
    butter_SetRocks(sendmessage->bhide, sendmessage, EXP_HIDEAFTER);
    butter_SetRocks(sendmessage->breset, sendmessage, 0);

    butterset = lpair_New();
    butterset2 = lpair_New();
    butterset3 = lpair_New();
    butterset4 = lpair_New();
    headeroptlp = lpair_New();
    lpair_VSplit(butterset2, sendmessage->bbv, sendmessage->bcv, 50, 1);
    lpair_VSplit(butterset3, sendmessage->bhv, sendmessage->bsv, 50, 1);
    lpair_VSplit(butterset4, butterset2, butterset3, 50, 1);
    lpair_VSplit(butterset, butterset4, sendmessage->brv, 20, 1);
    lpair_HFixed(headeroptlp, sendmessage->HeadScroll, butterset, 115, 1);
...
{   /* This is a bit bogus, because there isn't really a sendmessage dataobject/view split */
    ...
    butter_Destroy(self->bclear);
    butter_Destroy(self->bsign);
    butter_Destroy(self->bhide);
    butter_Destroy(self->bblind);
    butter_Destroy(self->breset);
    butterview_Destroy(self->bcv);
    butterview_Destroy(self->bsv);
    butterview_Destroy(self->bhv);
    butterview_Destroy(self->bbv);
    butterview_Destroy(self->brv);
...
{
	int code, total, external, ans, StreamVersion = 12, TrustDelivery = 0, nkids, format, strip, trust;
	...
	if (amsutil_GetOptBit(EXP_SIGNMAIL) && !sendmessage->HasSigned) {
	    ...
	    ams_TildeResolve(ams_GetAMS(), "~/.signature", fname);
	    fp = fopen(fname, "r");
	    if (!fp) {
	...
	if (Submit(sendmessage, Unformat, StreamVersion, TrustDelivery) == 0) {
...
void BS_SendmessageCompound(sm, cmds)
struct sendmessage *sm;
char *cmds;
{
...
void BS_SendmessageFoldersCompound(sm, cmds)
struct sendmessage *sm;
char *cmds;
{
    if (sm->folders) {
...
void BS_SendmessageMessagesCompound(sm, cmds)
struct sendmessage *sm;
char *cmds;
{
    if (sm->folders) {
...
void SBS_TextviewCompound(tv, cmds)
struct textview *tv;
char *cmds;
{
...
void SBS_DoBodiesCommand(sm, cmds)
struct sendmessage *sm;
char *cmds;
{
    SBS_TextviewCompound(sm->BodyTextview, cmds);
}

void SBS_DoHeadersCommand(sm, cmds)
struct sendmessage *sm;
char *cmds;
{
    SBS_TextviewCompound(sm->HeadTextview, cmds);
}
