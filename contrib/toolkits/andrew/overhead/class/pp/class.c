...
\* ********************************************************************** */
/* $Header: class.c,v 1.2 89/12/07 10:35:03 xguest Exp $ */
 ...
 **/
static void errorhandler(sig, code, scp)
int sig;
int code;
...
#define getFuncName(thismp) (sprintf(scratchbuf,"%s__%s",FinalClassName,thismp->name),scratchbuf)
    ...
    outstr3("  (void) class_EnterInfo(NULL, %s_classheader.name, %s__GetClassInfo, NULL, %s_classheader.namekey); } \n \n", FinalClassName, FinalClassName, FinalClassName);
    ...
    outstr1("    \"%s\",\n", FinalClassName);
    outstr1("    \"%s\",\n", BaseFileName);
    outstr0("    (struct basicobject_methods *) &class_RoutineStruct,\n");
    outstr0("};\n\n");
 ...
 */
	...
	/* don't use an outstr function as this has an integer */
        ...
        for (mp = methodlist->next; mp != NULL; mp = mp->next) {
            if (mp->type==ptype_method){
                ...
                if(mp->macrodef==NULL){
		    /* don't use an outstr function as this has	an integer */
		    fprintf(importfile, "    ((* ((%s (*)())((self)->header.%s_methods->routines[%d]))) (self%s))\n", mp->methodtype, FinalClassName, rtnNum, mp->methodargs);
		    fprintf(exportfile, "    ((* ((%s (*)())((self)->header.%s_methods->routines[%d]))) (self%s))\n", mp->methodtype, FinalClassName, rtnNum, mp->methodargs);
                    rtnNum++;
                }else
                    outstr1("%s\n",mp->macrodef);
            }
	...
	outstr2("    ((* ((struct %s * (*)())((self)->header.%s_methods->info->procs->routines[0])))", FinalClassName, FinalClassName);
        outstr2(" (&%s_classheader, (self)->header.%s_methods->info->versionnumber))\n", FinalClassName, FinalClassName);

        outstr1("#define %s_Destroy(self) \\\n", FinalClassName);
	outstr2("    ((* ((void (*)()) ((self)->header.%s_methods->info->procs->routines[1]))) (&%s_classheader, self))\n", FinalClassName, FinalClassName);

        outstr0("\n");

        outstr1("#define %s_New() \\\n", FinalClassName);
	(void) fprintf(importfile, "    (*((struct %s * (*)()) (%s_classheader.classprocedures->routines[0])))(&%s_classheader,%s_VERSION)\n", FinalClassName, FinalClassName, FinalClassName, FinalClassName);
	...
	(void) fprintf(importfile, "    (*((void (*)()) (%s_classheader.classprocedures->routines[2])))(&%s_classheader,self, %s_VERSION)\n", FinalClassName, FinalClassName, FinalClassName);
	...
	(void) fprintf(importfile, "    (*((void (*)()) (%s_classheader.classprocedures->routines[3])))(&%s_classheader,self)\n", FinalClassName, FinalClassName);
	...
	for (mp = methodlist->next; mp != NULL; mp = mp->next)  {
		...
		if (mp->argcount != 0)
		    ...
		    (void) fprintf(importfile, "    (*((%s (*)()) (%s_classheader.classprocedures->routines[%d])))(&%s_classheader)\n", mp->methodtype, FinalClassName, rtnNum, FinalClassName);
		if (mp->argcount != 0)
 ...
 */
    ...
    if(classDefinition){
        ...
        for (mp = methodlist->next; mp != NULL; mp = mp->next)  {
		    ...
		    (void) fprintf(exportfile,"	   %s_%s((struct %s *)self%s)\n",FinalParentName,mp->name,FinalParentName,mp->methodargs);
                    ...
                    (void) fprintf(exportfile, "    ((* ((%s (*)()) (%s_supermethods->routines[%d]))) (self%s))\n", mp->methodtype, FinalClassName, rtnNum, mp->methodargs);
                    rtnNum++;
                }
            }
        }
    ...
    (void) fprintf(exportfile, "    struct classinfo *info;\n");
    (void) fprintf(exportfile, "    long (*routines[%d])();\n", (classDefinition) ? (procedureCount + 4) : procedureCount);
    (void) fprintf(exportfile, "};\n\n");
    
    if (classDefinition)  {
        for (mp = methodlist->next; mp != NULL; mp = mp->next)  {
            if (mp->type==ptype_method && mp->defined && mp->macrodef==NULL)  {
		(void) fprintf(exportfile, "%s %s();\n", mp->methodtype, getFuncName(mp));
            }
        }
	...
	(void) fprintf(exportfile, "    (long (*)()) %s__Finalize", FinalClassName);
    }
    ...
    if(classDefinition){
	    ...
	    (void) fprintf(exportfile, "    if(!(*((boolean (*)()) (%s_classheader.classprocedures->routines[2])))(&%s_classheader,self, %s_VERSION))\n", FinalParentName, FinalParentName, FinalParentName);
		...
		(void) fprintf(exportfile, "\t(*((void (*)()) (%s_classheader.classprocedures->routines[3])))(&%s_classheader,self);\n", FinalParentName, FinalParentName);
	    }
	    (void) fprintf(exportfile, "\treturn FALSE;\n    }\n");
	}
	(void) fprintf(exportfile,"    return TRUE;\n}\n\n");
        ...
        (void) fprintf(exportfile, "\t(void) fprintf(stderr, \"Incompatible version of %s requested from %%s!\\n\", classID->name);\n", FinalClassName);
...
/*    (void) fprintf(exportfile, "\treturn NULL;\n");  %%%%% think about this... */
	...
	(void) fprintf(exportfile, "struct classinfo *%s__GetClassInfo(classID, versionnumber)\n", BaseFileName);
	(void) fprintf(exportfile, "struct classheader *classID;\n");
	(void) fprintf(exportfile, "unsigned long versionnumber;\n");
	(void) fprintf(exportfile, "{\n");
	(void) fprintf(exportfile, "    return %s__GetClassInfo(classID, versionnumber);\n", FinalClassName);
	(void) fprintf(exportfile, "}\n");
    } else {
...
int FoundError;		/* used to keep track of multiple errors */
	...
	if (token != class_RightParen)  {
		...
		if (token == class_Comma || (token == class_RightParen && parenLevel==0))  {
		    ...
		    strcpy(LastArg, str);
		    LastTokenType = token;
		} else if(token==class_RightParen)
		    parenLevel--;
		else if(token==class_LeftParen)
		    parenLevel++;
