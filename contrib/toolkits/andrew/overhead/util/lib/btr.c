...
static char rcsid[] = "$Header: /afs/.andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/overhead/util/lib/RCS/btr.c,v 2.10 89/09/01 15:39:49 cfe Exp $";
...
(UppBd..maxIx]. */
    LowBd = 0;
    UppBd = bF->IndexNum - 1;
    KeyLen = strlen(Key) + 1;	/* read match for trailing NUL, too */
    while (LowBd <= UppBd) {
	Mid = (LowBd + UppBd) / 2;
	errno = 0;
	...
	errno = 0;
	Res = fread(&DataFrag[0], 1, DFSiz, bF->File);
	...
	while (1) {
	    ...
	    Res = strncmp(KeyP, DataFragPtr, ThisGo);
	    if (Res != 0) {
		...
		*FlagsPtr = Flags;
	    ...
	    errno = 0;
	    Res = fread(DataFragPtr, 1, DFSiz, bF->File);
	    if (Res <= 0 || ferror(bF->File))
