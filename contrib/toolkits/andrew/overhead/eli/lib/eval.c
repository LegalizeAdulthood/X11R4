...
/*
 * $Header: /afs/.andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/overhead/eli/lib/RCS/eval.c,v 2.7 89/06/17 15:39:44 bobg Exp $ 
 *
 * $Source: /afs/.andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/overhead/eli/lib/RCS/eval.c,v $ 
 */
        ...
        case e_data_symbol:
            ...
            switch (eliFn_GetType(fnval)) {
                ...
                case e_fn_compiled:
                    ...
                    (*(eliFn_GetCompiled(fnval))) (st, tmpcons, resbuf);
                    if (wasTrace && st->tracep)
                        --st->indentTrace;
                    if (EliErr_ErrP(st))
                        return;	/* Dumb.  What *else* is it going to do? */
                    break;
                case e_fn_none:
                    EliError(st, ELI_ERR_FN_UNDEF, lcar, "INTERNAL [eliEvalList (functionless symbol at list car)]", 0);
                    return;
            }
            break;
        case e_data_list:
        ...
        case e_data_fn:	/* This whole section needs to be
            ...
            switch (eliFn_GetType(fnval)) {
                ...
                case e_fn_compiled:
                    ...
                    (*(eliFn_GetCompiled(fnval))) (st, anotherCons, resbuf);
                    if (EliErr_ErrP(st))
                        return;	/* Dumb.  What *else* is it going to do? */
                    break;
                case e_fn_none:
                    EliError(st, ELI_ERR_FN_UNDEF, lcar, "INTERNAL [eliEvalList (empty functionnode at list car)]", 0);
                    return;
            }
            break;
        case e_data_none:
...
EliSexp_t      *resbuf;
{
    int             numvars;	/* Holds number of items pushed onto the
				 * stack */
    EliCons_t      *varlist,	/* The varlist is the 2nd element in the
				 * lambda list, to wit: (lambda (a b c) (foo
				 * you)) (car (cdr '(lambda (a b c) (foo
				 * you)))) returns this list [(a b c)]. */
                   *llistcdr,	/* Will contain the cdr of the lambda list */
    ...
    varlist = EliSexp_GetCons(tmp);
    if (!(evaledArgs = EliEvalListToList(st, arglist)))
	return;
    numvars = eliBind(st, EliEvalStack(st), varlist, evaledArgs);
    if (EliErr_ErrP(st)) {
...
EliSexp_t      *resbuf;
{
    int             numvars;	/* Holds number of items pushed onto the
				 * stack */
    EliCons_t      *varlist,	/* The varlist is the 2nd element in the
				 * lambdaq list, to wit: (lambdaq (a b c)
				 * (foo you)) (car (cdr '(lambdaq (a b c)
				 * (foo you)))) returns this list [(a b c)]. */
                   *llistcdr,	/* Will contain the cdr of the lambdaq list */
                   *llistcdrcdr;/* Contains the cdr of llistcdr */
    EliSexp_t      *tmp;
    ...
    tmp = EliCons_GetCar(llistcdr);
    varlist = EliSexp_GetCons(tmp);
    numvars = eliBind(st, EliEvalStack(st), varlist, arglist);
    if (EliErr_ErrP(st)) {
	eliEvalStk_PopN(st, EliEvalStack(st), numvars);
	return;
    }
