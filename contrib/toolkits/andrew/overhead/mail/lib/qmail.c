...
char *returnpath, *auth, *forstr, *dirname, *pfx, *Cell;
{
	...
	while ((dp = readdir(dirp)) != NIL) {
	    if (dp->d_namlen >= pfxlen && strncmp(pfx, dp->d_name, pfxlen) == 0
...

static int create(name, f, filekind)
char *name, *filekind;
FILE **f;
{
    register int fd;

    fd = open(name, O_CREAT|O_WRONLY, 0400);
    if (fd < 0)
	if (vdown(errno)) {
	    sprintf(Qmail_ErrMsg,
