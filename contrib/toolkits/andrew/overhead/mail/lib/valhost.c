...
#ifndef res_SEARCH
    /* Version 4.8 of the Bind distribution includes a res_search function (in its res_query.c module).  Earlier versions do not, so the companion file rsearch.c defines the cptres_search function to work in the same way that gethostbyname() does, for more general domain queries, in versions of Bind that support domain abbreviations at all.
	Bind 4.8 also changed /usr/include/netdb.h by defining the NO_DATA value (for h_errno) as a new name for the old NO_ADDRESS value.
	    This code wants to call a res_search() function, of either origin.  We test here, at compile time, whether the Bind4.8 netdb.h file has been installed, and thus whether to call our old cptres_search function or the Bind4.8 res_search function.
	      We thus hope that if your /usr/include/netdb.h file defines NO_DATA, then you've installed Bind4.8 and have the res_search function available, but that if your /usr/include/netdb.h file does not define NO_DATA, you will need the cptres_search function included in the rsearch.c file.  If this does not represent the pairing found on your installation, all you need do is define res_SEARCH to be the correct function name on your installation. */
#ifdef NO_DATA
#define res_SEARCH res_search
#define res_NEWSEARCHSTYLE 1
#else /* NO_DATA */
#define res_SEARCH cptres_search
#define res_NEWSEARCHSTYLE 0
#endif /* NO_DATA */
#endif /* res_SEARCH */
    hp = (HEADER *) &answer;
    ...
    if ((n <= 0 && hp->rcode == 14) || h_errno == 0 || h_errno == TRY_AGAIN) return TryAgain;
