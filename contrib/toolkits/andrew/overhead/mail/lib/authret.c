...
static char rcsid[] = "$Header: authret.c,v 1.1 89/12/05 21:15:24 xguest Exp $";
...

static int GetAMSDel(ca)
struct CellAuth *ca;
{/* returns CheckAMSDelivery(ca->CellName), but sometimes faster. */
	if (ca->UsesAMSDelivery != 0) return ca->UsesAMSDelivery;
	return CheckAMSDelivery(ca->CellName);
}

...
main () {
	char *Whatever;
	int RC, Ix;
	char *FNs[] = {"/afs/andrew.cmu.edu/usr13/cfe", "/tmp", "/afs/.cs.cmu.edu", "/afs/not/here"};
