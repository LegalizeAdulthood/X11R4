...
/* What we need to do here is to find out what CheckAMSNameSep(domain) returns.  If 0 or -1, or if there's a second part or a +foo+ form, use uid+ spelling.  If result is >0, read the N wp field and spell that way. */
    if (laType == latype_LocalName || (laType == latype_LocalID && *laSecond == '\0')) {
	OutMatch = CheckAMSNameSep(PrevailingDomain);
	if (OutMatch > 0) {
	...
	(void) sprintf(NewName, "%s+%s", CanonID, laSecond);
    } else {
	(void) sprintf(NewName, "%s+", CanonID);
    }
    if (Addr->LocalPart != NULL) free(Addr->LocalPart);
    Addr->LocalPart = NewName;
    if (KVal != NULL) free(KVal);
