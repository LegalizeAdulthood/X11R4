...
char *NNTPhost = "pt.cs.cmu.edu";	/* Host with NNTP service */
...
int AMS_ViceQueueLifetime = 8 * 24 * 60 * 60;	/* Default is eight days */
...
int AMS_ExtraViceQueueLifetime = 5 * 24 * 60 * 60;	/* Default is five days */
...
int AMS_ViceQueueLifetimeCap = 2 * (8 + 5) * 24 * 60 * 60;	/* Default is 26 days */
...
char *WelcomeMailFile = "%a/lib/Hello";

/* The following specifies the location of the ELI file loaded at startup. */

char *EliStartupFile = "%a/lib/eli/ms_startup.eli";
...
main() {
    ...
    CkAMSCellConfig(UserPrimaryCell);
    printf("In %s, LOCALSEARCHPATHTEMPLATE is %s.\n",
	UserPrimaryCell, LOCALSEARCHPATHTEMPLATE);
    printf("In %s, numAMS_ValidDomainRelays is %d:",
	UserPrimaryCell, numAMS_ValidDomainRelays);
