...
static char rcsid[] = "$Header: /afs/.andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/overhead/mail/lib/RCS/ckamsdel.c,v 2.16 89/05/22 16:17:30 cfe Exp $";
...
{/* Get, or allocate, a struct AMSConfig record corresponding to the given domain. */
	...
	if (acf != NULL) {
		...
		acf->AMSDel = acf->NameSep = acf->ATKFmtOK = acf->UUCPSupp = 0;
		acf->MBoxName = acf->PManName = acf->WPIAddr = NULL;
		acf->ValidMask = -1;
		acf->numdflt_MSElts = 0;
		acf->dflt_MSElts = NULL;
		*pAlloc = 1;
	    }
	}
    }
...
{/* Load values from the AMS-Server file for the given domain.  If the given domain is running AMS delivery, it can set default values if the AMS-Server file doesn't have a given field. */
    ...
    free(SrvFile);
    if (srvHead == NULL) return -1;	/* Errno will have something for the caller. */
    if (acf->AMSDel > 0) {
	accum = vld_WPValid;
    } else {
    ...
    if (vp != NULL) {
	acf->MBoxName = malloc(1+strlen(vp));
	if (acf->MBoxName != NULL) strcpy(acf->MBoxName, vp);
    } else if (acf->AMSDel > 0) {
	acf->MBoxName = "Mailbox";  /* Again, AMS delivery means we know. */
    }
    vp = GetConfig(srvHead, "PostmasterName", 1);
    if (vp != NULL) {
	acf->PManName = malloc(1+strlen(vp));
	if (acf->PManName != NULL) strcpy(acf->PManName, vp);
    } else if (acf->AMSDel > 0) {
	acf->PManName = "postman";  /* Again, AMS delivery means we know. */
    }
    vp = GetConfig(srvHead, "WPIUpdateReqAddr", 1);
    if (vp != NULL) {
	acf->WPIAddr = malloc(1+strlen(vp));
	if (acf->WPIAddr != NULL) strcpy(acf->WPIAddr, vp);
    }
