...
\* ********************************************************************** */
/* $Header: /afs/.andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/overhead/cmenu/RCS/cmactiv.c,v 2.4 89/02/10 23:08:51 ghoti Exp $ */
/* $Source: /afs/.andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/overhead/cmenu/RCS/cmactiv.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/.andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/overhead/cmenu/RCS/cmactiv.c,v 2.4 89/02/10 23:08:51 ghoti Exp $";
#endif /* lint */
    ...
    long background;
{
    ...
    XExposeEvent *exposeEvent = (XExposeEvent *) &event; /* Actually used for getting fields of event. */
...
/* Prevent server from covering area with background when menus go down. */
    ...
    if (state.drawingState.doSaveUnder) {
        do {
            XIfEvent(display, &event, DiscardableEvents, (char *) &state);
        } while (exposeEvent->type != Expose || exposeEvent->window != state.parentWindow || exposeEvent->count != 0);
