...
static bool_t emulate_forward = false; /* If true, act like the forward program */
static bool_t force_warn = false;	/* If true, don't exit, but continue */
static bool_t changes_made = false; /* if true, some fields were modified */
static bool_t req_auth = false;	/* if true, some authorization needed */
static bool_t quiet = false; /* if true, suppress messages */
...
{				/* parse the argument list */
  if (arg[0] == '-' && arg[2] == '\0') {
    if (emulate_forward) {
      ...
      case 'n': return(just_validate_address);
...
{
  char *arg;
  bool_t command_line_fields_used = false;
  ...
  int i;
  /* forward emulation */
  bool_t just_print = false;
  bool_t just_validate = false;
  char *old_addr = NULL;
  char *address = "";
  ...
  for(i=1;i<argc;i++){
	...
	break;
      ...
      if (!emulate_forward && !adminp && strcmp(username,self)) {
  ...
  if (!emulate_forward && !command_line_fields_used) {
    if (!entry) {
	if (self == username) {	/* defaulting to self */
	  if ((self = username = WPI_Self())==NULL) {	/* in new domain */
	    error("could not find you in other cell",12);
	...
	entry = WPI_Lookup(username, false);
    }

    if (just_print) {
      puts(WPI_Value("Fwd", entry));
      exit(0);
    }
    ...
    if (old_addr && !strcmp(old_addr, WPI_Value("Fwd", entry))) {
