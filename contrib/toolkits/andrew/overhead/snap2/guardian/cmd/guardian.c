...
static char GuardianVersion[] = "$Revision: 2.83 $ $Date: 89/11/07 10:55:52 $";
...
extern int strcmp(), errno, sys_nerr;
...
USERINFO *u;
{
    ...
    /* Initialize RPC if necessary */
    rc = InitializeRPC();
    if (rc != 0) return
	  set_gerror(GASP_RPC_INIT,"vice_auth_rpc_init");

    debug(128,("Attempting Vice authentication: "));
    errno=0;
    rc = U_Authenticate(u->name,u->password, &toks.ctoken, &toks.stoken);
    debug(128, ("%d\n", rc));

    /* New (R) authentication */
    if (rc != 0)
...
{
    DEALOG((
	     "death,%d,%d,%d,%d,%d,%d,%s,%ld,%ld,%ld,%ld,%ld,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
	     st->pid,
	     st->s_termsig,
	     st->s_coredump,
	     st->s_retcode,
...
int type;
{
    ...
    *scn_out=0;
    ...
    if (pw == NIL) return set_gerror(GASP_CANT_FIND_USER,"user_not_in_passwd");
