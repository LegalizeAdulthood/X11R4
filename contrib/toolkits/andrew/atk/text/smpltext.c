...
\* ********************************************************************** */
/* $Header: smpltext.c,v 1.1 89/12/05 15:13:33 xguest Exp $ */
/* $ACIS:smpltext.c 1.6$ */
/* $Source: /xsrc/contrib/toolkits/andrew/atk/text/RCS/smpltext.c,v $ */

#ifndef lint
static char *rcsid = "$Header: smpltext.c,v 1.1 89/12/05 15:13:33 xguest Exp $";
#endif /* lint */
...
long neededGap;
{
    ...
    t = self->string + self->length + self->gapSize - 1;
    s = self->string + totalSize - 1;
...
long pos;
{
    ...
    if (pos > self->lowSize) {
        ...
        s = self->string + self->lowSize + self->gapSize - 1;
        t = self->string + self->lowSize - 1;
        self->lowSize -= amount;
        while (amount-- > 0)
            *s-- = *t--;
    }
    ...
    long len;
{
	...
	for (s = &(srctext->string[srcpos]); i > 0; s++, i--)  {
	    *t++ = *s;
	}
	srcpos = srctext->lowSize;
    }
    if (remlen > 0)  {
	for (s = &(srctext->string[srcpos + srctext->gapSize]); remlen > 0; remlen--, s++)  {
	    *t++ = *s;
	}
    }
    self->lowSize += len;
