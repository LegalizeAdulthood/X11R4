...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/basics/wm/RCS/wgraphic.c,v 1.6 89/08/29 15:58:51 wjh Exp $ */
/* $ACIS:graphic.c 1.6$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/basics/wm/RCS/wgraphic.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/basics/wm/RCS/wgraphic.c,v 1.6 89/08/29 15:58:51 wjh Exp $";
#endif /* lint */
...
#if 0
#define wmgraphic_GetLogicalWidth(self) \
    (((struct graphic *)self)->localBounds.width)
#define wmgraphic_GetLogicalHeight(self) \
    (((struct graphic *)self)->localBounds.height)
#define wmgraphic_GetLogicalLeft(self) \
    (((struct graphic *)self)->localBounds.left)
#define wmgraphic_GetLogicalTop(self) \
    (((struct graphic *)self)->localBounds.top)

#define wmgraphic_GetVisualWidth(self) \
    (((struct graphic *)self)->visualBounds.width)
#define wmgraphic_GetVisualHeight(self) \
    (((struct graphic *)self)->visualBounds.height)
#define wmgraphic_GetVisualLeft(self) \
    (((struct graphic *)self)->visualBounds.left)
#define wmgraphic_GetVisualTop(self) \
    (((struct graphic *)self)->visualBounds.top)

#define wmgraphic_GetEnclosedWidth(self) \
    (((struct graphic *)self)->localBounds.width)
#define wmgraphic_GetEnclosedHeight(self) \
    (((struct graphic *)self)->localBounds.height)
#define wmgraphic_GetEnclosedLeft(self) \
    (point_X(&(((struct graphic *)self)->enclosedOrigin)))
#define wmgraphic_GetEnclosedTop(self) \
    (point_Y(&(((struct graphic *)self)->enclosedOrigin)))
 ...
 */
    ...
    wm_SetFunction(wm_map[(self->header.graphic.transferMode>>2)&0x3]);
...
void wmgraphic__InsertGraphicSize(self, EnclosingGraphic, xOriginInParent,
...
long xOriginInParent, yOriginInParent, width, height; {
    ...
    rectangle_SetRectSize(&r,xOriginInParent, yOriginInParent,
	    width,height);
    wmgraphic_InsertGraphic(self,EnclosingGraphic,&r);
}
...
struct rectangle * EnclosedRectangle;
    ...
    rectangle_SetRectSize(&self->header.graphic.localBounds,point_X(&self->header.graphic.savedOrigin),
			point_Y(&self->header.graphic.savedOrigin),
    ...
    self->header.graphic.visualBounds = *EnclosedRectangle;
    rectangle_IntersectRect(&self->header.graphic.visualBounds,&self->header.graphic.visualBounds,
			    &EnclosingGraphic->header.graphic.visualBounds);
    /* Now reorient the rect to local (savedOrigin based) coordinates. */
    rectangle_OffsetRect(&self->header.graphic.visualBounds,
			 -rectangle_Left(EnclosedRectangle),
			 -rectangle_Top(EnclosedRectangle));
    rectangle_OffsetRect(&self->header.graphic.visualBounds,point_X(&self->header.graphic.savedOrigin),
		    point_Y(&self->header.graphic.savedOrigin));

    /* Install the origin relative to the enclosing grphic (just
    copy over the rectangle that it specified! */
    point_SetPt(&self->header.graphic.enclosedOrigin,
			 rectangle_Left(EnclosedRectangle),
			 rectangle_Top(EnclosedRectangle));
    ...
    /* Since the clippingRect is relative to the visual rect, we
       ...
       insertion operation */
    if (self->header.graphic.clippingRect) free(self->header.graphic.clippingRect);
    self->header.graphic.clippingRect = NULL;
...
struct graphic * wmgraphic__ColorPattern(self, red, blue, green)
struct wmgraphic * self;
long red, blue, green; {
    if (red==0 && blue == 0 && green == 0) return wmgraphic_BlackPattern(self);
    return wmgraphic_WhitePattern(self);
}

void wmgraphic__SetForegroundColor(self, colorName, red, blue, green)
struct wmgraphic * self;
char *colorName;
long red, blue, green; {

    super_SetForegroundColor(self, colorName, red, blue, green);
    ValidateState(self);
    ...
    if (colorName != NULL) {
        ...
        if (red == 0 && blue == 0 && green == 0)
            self->forePixel = 1; /* black */
        else self->forePixel = 0; /* white */
    }

    wmgraphic_LocalSetTransferFunction(self);
}

void wmgraphic__SetBackgroundColor(self, colorName, red, blue, green)
struct wmgraphic * self;
char *colorName;
long red, blue, green; {

    super_SetBackgroundColor(self, colorName, red, blue, green);
    ValidateState(self);
    ...
    if (colorName != NULL) {
        ...
        if (red == 0 && blue == 0 && green == 0)	
            self->backPixel = 1; /* black */
        else self->backPixel = 0; /* white */
    }
    wmgraphic_LocalSetTransferFunction(self);
}
