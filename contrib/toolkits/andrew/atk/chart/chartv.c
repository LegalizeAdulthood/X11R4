...
#ifndef lint
static char *rcsidchartv = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/chart/RCS/chartv.c,v 1.18 89/09/07 18:30:38 tom Exp $";
#endif
...
void				    Save_Command(), Add_Command(),
				    Delete_Command(), ReChart_Command(),
...
/*===
  ...
  { "chartv-Palette",  "", 0,	"Hide Palette~70", 0, menu_palette_exposed,
    Palette_Command,	"Palette-toggle" },
===*/
  { "chartv-DEBUG",  "",   0,	"DEBUG~80",	0, menu_default,
    DEBUG_Command,	"Debug-toggle" },
  { "chartv-Quit",  "",    0,	"Quit~99",	0, menu_default,
    Quit_Command,	"Quit" },
/*===MUST BE DYNAMIC===*/
  { "chartv-ReChart", "",   0,	"ReChart~10,Histogram~1",
    (long)"Histogram",	menu_default,	ReChart_Command,"ReChart To Histogram" },
  { "chartv-ReChart", "",   0,	"ReChart~10,Pie~2",
    (long)"Pie",	menu_default, ReChart_Command,	"ReChart To Pie" },
  { "chartv-ReChart", "",   0,	"ReChart~10,Dot~3",
    (long)"Dot",	menu_default, ReChart_Command,	"ReChart To Dot" },
  { "chartv-ReChart", "",   0,	"ReChart~10,Line~4",
    (long)"Line",	menu_default, ReChart_Command,	"ReChart To Line" },
/*===
...
chartv__ReceiveInputFocus( self )
  ...
  if ( Menu )
    {
    menulist_SetMask( Menu, menu_default | menu_palette_hidden |
...
chartv__SetDebug( self, state )
  ...
  IN(chartv_SetDebug);
  debug = state;
  if ( Chart )		chart_SetDebug( Chart, debug );
  if ( ChartViewer )	chartobj_SetDebug( ChartViewer, debug );
  OUT(chartv_SetDebug);
  }

void 
...
Initialize( self )
  ...
  if ( moniker )
	...
	class_NewObject( chart_ModuleName( Chart, moniker ) ) )
      { DEBUG(Created);
      Initialized = true;
      chartobj_SetDebug( ChartViewer, debug );
      chartobj_SetDataObject( ChartViewer, Chart );
...

void
ReChart_Command( self, moniker )
  register struct chartv     *self;
  register char		     *moniker;
  {
  IN(ReChart_Command);
  DEBUGst(moniker,moniker);
  ReChart( self, moniker );
  OUT(ReChart_Command);
  }
