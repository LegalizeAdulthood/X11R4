...
#ifndef lint
static char *rcsidchartobj = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/chart/RCS/chartobj.c,v 1.11 89/09/07 16:50:35 tom Exp $";
#endif
...
Set_Shadows( self )
  ...
  if ( excess = (ChartWidth - (width * count)) / 2 )
    excess = count / excess;
  DEBUGdt(width,width);
  if ( chart_ItemValueGreatest( Chart ) % ScaleTick )
  ...
  if ( chart_ItemValueLeast( Chart) < 0 )
    ...
    if ( chart_ItemValueLeast( Chart ) % ScaleTick )
  ...
  for ( i = 0; i < count  &&  shadow; i++ )
	...
	PixelsPerUnit)) + high_adjust - low_adjust) > ChartBottom )
      y = ChartBottom;
    ShadowY(shadow) = y;
...
Draw_Left_Scale( self )
  ...
  register float		  y, y_increment, half_y_increment;
  char				  value_string[25];
...
Prepare_Vertical_Scale( self )
  register struct chartobj	 *self;
  {
  register long			  tick = 10;

  IN(Prepare_Vertical_Scale);
  DEBUGdt(Value Span,chart_ItemValueSpan( Chart ));
  while ( (chart_ItemValueSpan( Chart ) / tick) > tick )
    {tick *= 10;  DEBUGdt(Tick_1,tick);}
  if ( (chart_ItemValueSpan( Chart ) - tick) <= tick )
    tick /= 10;
  DEBUGdt(Tick_2,tick);
  while ( (PixelsPerUnit * tick) < 20 )
    {tick *= 10;  DEBUGdt(Tick_3,tick);}
  ScaleTick = tick;
