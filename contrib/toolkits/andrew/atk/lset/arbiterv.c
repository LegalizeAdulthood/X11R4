...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/lset/RCS/arbiterv.c,v 2.16 89/09/08 16:38:10 ghoti Exp $ */
/* $ACIS: $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/lset/RCS/arbiterv.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/lset/RCS/arbiterv.c,v 2.16 89/09/08 16:38:10 ghoti Exp $ ";
#endif /* lint */
...
struct arbiterview *firstlink , *lastlink;
extern char *rindex();
...
struct text *EditText;
{
    ...
    if(strcmp(class_GetTypeName(truechild),"frame") == 0 &&
...
printf("in arb for %s\n",ObjectName);
	...
	if((st = rindex(buf,arbiterview_SEPCHAR)) != NULL){
	    *st = '\0';
	    ObjectName = ++st;
...
printf("Self = %ld\n",self);
	...
	for(v = (self->celviewlist), i = 0; i < self->celcount; v++,i++){	
	    cl = Cel((*v));
	    st = cel_GetRefName(cl);
	    if(*st == *ObjectName && (strcmp(st,ObjectName) == 0)){
#ifdef DEBUG
