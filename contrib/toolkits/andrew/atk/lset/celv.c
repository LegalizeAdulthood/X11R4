...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/lset/RCS/celv.c,v 2.22 89/11/14 14:21:20 tpn Exp $ */
/* $ACIS: $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/lset/RCS/celv.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/lset/RCS/celv.c,v 2.22 89/11/14 14:21:20 tpn Exp $ ";
#endif /* lint */
    ...
    struct celview *self;
{
	...
	celview_PostMenus(self,self->menus);
	self->HasFocus = TRUE;
    }
   celview_WantUpdate(self,self);
}
...
struct cel *ls;
{
	...
	if(ls->viewType != NULL && *ls->viewType != '\0' /* && ls->dataObject != NULL */ && (truechild = (struct view *) class_NewObject(ls->viewType)) )  {
    ...
    if((self->refatm != cel_GetRefAtom(ls)) && ls->refname && *ls->refname){
	...
	if(self->truechild){
	    if(ls->application == cel_APPLICATION || ls->application == cel_VALUE){
...
editresourceList( self, resources,askres )
struct celview * self;
struct resourceList * resources;
int askres;
{
    struct resourceList *rl;
    char buf[1024],iname[512],*cp,*rindex();
    int pf;
    for(rl = resources; rl->name != NULL; rl++){
...
boolean StringToResourceList(rl,str)
struct resourceList *rl;
char *str;
{
    char buf[512], *tmp;
	...
	case FLOAT:
	    ...
	    break;
	case STRING:
	    cp = malloc(strlen(buf ) + 1); /* CORE LEAK !!!!!!! */
	    strcpy(cp,buf);
	    rl->data = (long) cp;
	    break;
	default:
...
if(self->arb){
    editresourceList( self, resources,FALSE );
/*	Moved below 
...
/*
else  /no longer supported /
    editresourceList( self, resources,TRUE );
*/
