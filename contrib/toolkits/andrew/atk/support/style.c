...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/support/RCS/style.c,v 2.10 89/07/09 10:33:51 wjh Exp $ */
/* $ACIS:style.c 1.4$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/support/RCS/style.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/support/RCS/style.c,v 2.10 89/07/09 10:33:51 wjh Exp $";
#endif /* lint */
...
struct style *self, *dest;
{
    ...
    dest->NumTabChanges= self->NumTabChanges;
...
char * NewAttributeValue; {
    ...
    if(self->AdditionalAttributes)    namespace_SetValue(self->AdditionalAttributes,tmpAtom, tmpValue);
}
...
char * OldAttributeName;{
    ...
    tmpValue = (char *) namespace_GetValue(self->AdditionalAttributes, tmpAtom);
...
char * OldAttributeName; {
    ...
    tmpAtom = atom_Intern(OldAttributeName);
    return (char *) namespace_GetValue(self->AdditionalAttributes, tmpAtom);
}
...
FILE *fp;
{
	...
	fprintf(fp, AttrD, AttributeAlias[(int) style_SpacingAttr], SpacingValueAlias[(int) newjust], OptypeAlias[(int) unit], operand);

    style_GetNewAbove(self, &newspacing, &operand, &unit);
    if (newspacing != style_AboveSpacing || operand != 0 || unit != style_Points)
	fprintf(fp, AttrD, AttributeAlias[(int) style_AboveAttr], SpacingValueAlias[(int) newjust], OptypeAlias[(int) unit], operand);

    style_GetNewBelow(self, &newspacing, &operand, &unit);
    if (newspacing != style_BelowSpacing || operand != 0 || unit != style_Points)
	fprintf(fp, AttrD, AttributeAlias[(int) style_BelowAttr], SpacingValueAlias[(int) newjust], OptypeAlias[(int) unit], operand);
	...
	fprintf(fp, AttrD, AttributeAlias[(int) style_SpreadAttr], SpacingValueAlias[(int) newjust], OptypeAlias[(int) unit], operand);
...
FILE *fp;
{
	...
	case style_TabsAttr:
	    ...
	    style_AddTabChange(self, newtabalign, operand<<16, unit);
	    break;
