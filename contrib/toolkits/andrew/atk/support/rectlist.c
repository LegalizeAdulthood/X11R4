...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/support/RCS/rectlist.c,v 2.3 89/02/17 17:14:25 ghoti Exp $ */
/* $ACIS:rectlist.c 1.2$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/support/RCS/rectlist.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/support/RCS/rectlist.c,v 2.3 89/02/17 17:14:25 ghoti Exp $";
#endif /* lint */
...
struct classheader *classID;
int bottom, top, left, right, startscan; {
...
void rectlist__InvertRectangles(classID, gPtr)
struct classheader *classID;
    struct view *gPtr;
{
    register int i;
	...
	if (OldList[i].bottom != OldList[i].top && OldList[i].left != OldList[i].right)  {
	    ...
	    pat = view_BlackPattern(gPtr);
	    view_FillRect(gPtr, &invertRect, pat);
	}
    }    
    for (i = 0; i < EndNew; i++)  {
