...
\* ********************************************************************** */
/* $Header: complete.c,v 1.1 89/12/04 20:28:36 xguest Exp $ */
/* $ACIS:complete.c 1.2$ */
/* $Source: /xsrc/contrib/toolkits/andrew/atk/support/RCS/complete.c,v $ */

#ifndef lint
static char *rcsid = "$Header: complete.c,v 1.1 89/12/04 20:28:36 xguest Exp $";
#endif /* lint */
...
static enum message_CompletionCode completion__FileComplete(classID, pathname, directory, buffer, bufferSize)
    ...
    int bufferSize;
{
    return FileComplete(pathname, directory, buffer, bufferSize);
}
 ...
 */
    ...
    if ((code = message_AskForStringCompleted(view, 40, prompt,
         initialString, buffer, bufsiz, fileRock.keystate, FileComplete,
         FileHelp, directoryP, mustMatch ? message_MustMatch : 0)) != -1)
