...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/preview/RCS/preview.c,v 2.12 89/11/07 17:35:25 cfe Exp $ */
/* $ACIS:preview.c 1.2$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/preview/RCS/preview.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/preview/RCS/preview.c,v 2.12 89/11/07 17:35:25 cfe Exp $";
#endif /* lint */
...
/*    self->scroll = textview_NoScroll;
    ...
    tempProc = proctable_DefineProc("preview-Print", LastPage, &preview_classinfo, NULL, "Last Page");
    menulist_AddToML(self->menulist, "Previous Page~21", tempProc, NULL, 0);
 ...
 */
   ...
   if (n != 0 || FilePosition != 0)
   {
	...
	/* set up menu info for this page */
   	sprintf(self->menubuf, "Pages %d-%d~%d,Page %d~%d", (m == 0)? 1:m, m+9,(m / 10) +10, n,n - m);
	self->menupage = i;
   }
...
struct preview *self;
{
   ...
   while ((c = getc(filein)) != EOF)
	 ...
	 if (self->SizeChanged)
	    break;
	 if (filein->_cnt <= 0 && !self->DviFileComplete)
	    break;
      }
   self->lastc = lastc;
...
/* Put up a clock until the first page is ready to display */
        ...
        else cursor_SetStandard(self->cursor,Cursor_Arrow);
...
int page;
{
   ...
   if(page != -1){
	   ...
	   printf(stderr,"Can't execute %s",PrintCommand);
	   exit(1);
       }
       if((fi = fopen(self->DviFileName,"r")) == NULL){
	   printf(stderr,"Can't open %s",self->DviFileName);
	   exit(1);
       }
 ...
 */
...
#ifdef hp9000s300
  {
   ...
   execlp("/bin/csh", "csh", "-cf", PrintCommand, 0);
   exit(0);
