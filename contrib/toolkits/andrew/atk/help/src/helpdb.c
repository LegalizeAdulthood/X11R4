...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/help/src/RCS/helpdb.c,v 1.10 89/10/04 19:12:41 cfe Exp $ */
/* $ACIS$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/help/src/RCS/helpdb.c,v $ */

#if !defined(lint) && !defined(LOCORE) && defined(RCS_HDRS)
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/help/src/RCS/helpdb.c,v 1.10 89/10/04 19:12:41 cfe Exp $";
...
int strip;			/* whether to strip changes files */
{
	...
	if (alias[0] == '#') {
	    ...
	    message_DisplayString(c->view, 0, msg_new_win);
	    mysystem(&alias[1]);
...
register char *aname;
{
    ...
    */
    ...
    helpDir = environ_GetConfiguration(SETUP_HELPDIR);
    if (helpDir == NULL)
	helpDir = environ_AndrewDir(DEFAULT_HELPDIR);
    ...
    if ((dd = opendir(dname)) == (DIR *)NULL)
        return;
    while (((ent = readdir(dd)) != (struct direct *)NULL) && !found) {
...
struct helpFile *tmplist;
{
    ...
    *tfp++ = '/';	/* tfp points just after the last '/' */
	...
	if (!Match(aname, tde->d_name, 1)) continue;
	...
	tf->fileName =
	    (char *) malloc(strlen(dname) + 1 + tde->d_namlen + 1);
...
int strip;			/* whether to strip changes files */
{
    ...
    for(i=0;i<ts->count;i++) {
        ...
        if (!Match(aname, pathName, 0))
	    continue;
        t = (struct helpFile *) malloc (sizeof(struct helpFile));
        t->fileName = (char *) malloc(strlen(pathName) + 1);
        strcpy(t->fileName, pathName);
	...
	t = tmplist;
	while (t) {
	    if (!strncmp(t->extPtr,CHANGE_EXT,sizeof(CHANGE_EXT)) ||
