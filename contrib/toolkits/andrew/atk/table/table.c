...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/table/RCS/table.c,v 1.9 89/09/21 09:52:12 ajp Exp $ */
/* $ACIS:table.c 1.5$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/table/RCS/table.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/table/RCS/table.c,v 1.9 89/09/21 09:52:12 ajp Exp $";
#endif /* lint */
...
int level;
{
	...
	fprintf (f, "\\begindata{table,%ld}\n", table_UniqueID(T));
	chunk.LeftCol = -1;
	chunk.RightCol = table_NumberOfColumns(T) - 1;
	chunk.TopRow = -1;
	chunk.BotRow = table_NumberOfRows(T) - 1;
	WriteASCII (T, f, &chunk, level);
	fprintf (f, "\\enddata{table,%ld}\n", table_UniqueID(T));
	getDataObject(T).modified = FALSE;
    }
    return getDataObject(T).id;
}
...
char *buff;
{
    ReadCell (T, NULL, buff, buff, buff+1000, cell);
    T->cellChanged = ++(T->timeStamper);
    table_NotifyObservers(T, 0);
    table_SetModified(T);
}
...
struct cell *newcell, *oldcell;
{
	...
	newcell->lock = 0;
	newcell->precision = DefaultPrecision;
	newcell->lastcalc = ++(T->timeStamper);
	newcell->celltype = table_EmptyCell;
    }
}
...
struct cell *oldcell;
{
    ...
    oldcell->lock = 0;
    oldcell->precision = DefaultPrecision;
    oldcell->celltype = table_EmptyCell;
...
int     r, c, iftaped;
{
	...
	else if (cell->celltype == table_EmptyCell)
