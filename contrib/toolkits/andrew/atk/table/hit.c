...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/table/RCS/hit.c,v 1.1 89/02/24 09:55:04 ghoti Exp $ */
/* $ACIS:hit.c 1.3$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/table/RCS/hit.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/table/RCS/hit.c,v 1.1 89/02/24 09:55:04 ghoti Exp $";
#endif

/* hit.c - handle mouse hits for table */
#include <class.h>
...
long numberOfClicks;		/* how should i use this?? */
{
	...
	if ((child = FindSubview(V, hitcell))) {
	    if (debug)
...
Chunk chunk;
{
	...
	V->icx = x;
	V->icy = y;
	InvertRectangle (V, V->currentoffset, V->icy, V->icx - V->currentoffset, 2);
    }
}
...
Chunk chunk;
{
	...
	V->icy = y;
	spread_SetTransferMode (V, graphic_INVERT);
	InvertRectangle (V, V->icx, V->currentoffset, 2, V->icy - V->currentoffset);
    }
}
...
register Chunk chunk;
{
	...
	if (V->selectionvisible) {
	    ...
	    if (x0 != x2)
		InvertRectangle (V, x0, y0, x2 - x0, y1 - y0), x0 = x2;
	    if (x1 != x3)
		InvertRectangle (V, x1, y0, x3 - x1, y1 - y0), x1 = x3;
	    if (y0 != y2)
		InvertRectangle (V, x0, y0, x1 - x0, y2 - y0), y0 = y2;
	    if (y1 != y3)
		InvertRectangle (V, x0, y1, x1 - x0, y3 - y1), y1 = y3;
	}
	CopyChunk (&(V->selection), chunk);
	spread_WantUpdate (V, &getView(V));
    }
