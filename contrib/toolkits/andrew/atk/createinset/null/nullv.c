...
\* ********************************************************************** */
/* $Header: nullv.c,v 1.2 89/12/05 11:33:39 xguest Exp $ */
/* $ACIS:$ */
/* $Source: /xsrc/contrib/toolkits/andrew/atk/createinset/null/RCS/nullv.c,v $ */

#if !defined(lint) && !defined(LOCORE) && defined(RCS_HDRS)
	char *nullv_c_rcsid = "$Header: nullv.c,v 1.2 89/12/05 11:33:39 xguest Exp $";
	...
	long left, top, width, height;
{
	...
	for (d = null_GetFirstDot(dobj);  d != NULL;
		...
		/* display each dot as a 10x7 black blob  */
		struct rectangle r;
		nullview_SetTransferMode(self, graphic_COPY);
		rectangle_SetRectSize(&r, null_GetDotX(dobj, d),
			null_GetDotY(dobj, d), 10, 7);
		nullview_FillRect(self, &r, self->BlackPattern);
	}
}
	...
	long rock;
{
	...
	struct rectangle r;
	nullview_GetLogicalBounds(self, &r);
	nullview_FillRect(self, &r, self->WhitePattern);
	null_ClearDots(dobj);
