...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/console/lib/RCS/setup.c,v 2.45 89/11/22 13:19:54 cfe Exp $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/console/lib/RCS/setup.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/console/lib/RCS/setup.c,v 2.45 89/11/22 13:19:54 cfe Exp $";
#endif /* lint */
...
void Reaper()
{
    int pid;
    int *child;
    /* the following counter is added as an array bounds check --MKM */
    int i;
    int status;
...
struct FuncStruct FuncParse[] = {
	...
	"pagereplacable","$ replacable pages.",  PAGEREPLACABLE,
        ...
        "unauthenticated", "Your AFS connection has expired; use 'log' to reconnect.",  UNAUTHENTICATED,
...
#ifdef SIGURG
    i = sigblock((1 << (SIGCHLD - 1)) | (1 << (SIGTERM - 1)) | (1 << (SIGURG - 1)));
#else /* NOT SIGURG */
    i = sigblock((1 << (SIGCHLD - 1)) | (1 << (SIGTERM - 1)));
#endif /* SIGURG */
    j = 0;
...
#define MAXTRIES 5
	...
	if ((theTypeInPtr = FindConsoleInPaths(theTypeInPtr)) == NULL) {
	    pfd = NULL; /* obviously */
	}else {
	    if (theTypeInPtr != NULL) {
...
boolean IsStartup;
{
	...
	if (pfd == NULL) {
	    ...
	    theTypeInPtr = FindConsoleInPaths(theTypeInPtr);
	    if(theTypeInPtr != NULL) {
		strcpy(ConFile, theTypeInPtr);
		free(theTypeInPtr);
		theTypeInPtr = NULL;
...
#ifdef  hpux
    setpgrp(); /* breaks process group affiliation of controlling terminal */
#else /* hpux */
