...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/console/lib/RCS/mailmonf.c,v 2.16 89/10/20 17:10:32 cfe Exp $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/console/lib/RCS/mailmonf.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/console/lib/RCS/mailmonf.c,v 2.16 89/10/20 17:10:32 cfe Exp $";
#endif /* lint */
...
int *LastModTime, LastValue;
{
	...
	if ((!index(f, '/'))&&(!index(f + 1, '*'))){
		...
		*f++; /* f is now the prefix */
		...
		if (*(f - 1) == '/'){
		    ...
		    *f = ' ';
		    ReportInternalError(self, sprintf(ErrTxt, "console: '%s' is neither prefix nor suffix"));
		    return(-1);
		}
	    }
	}
	    ...
	    *f = ' ';
	    ReportInternalError(self, sprintf(ErrTxt, "console: '%s' is invalid InitString specification"));
	    return(-1);
	...
	ReportInternalError(self, sprintf(ErrTxt, "console: Cannot stat directory '%s': cannot check %s now.", NewName, LastModTime == &LastMailMod ? "your mail" : "its contents"));
	*LastModTime = 0;
	return(-1);
    }
    if ((dirstatbuf.st_mode & S_IFMT) != S_IFDIR) {
	ReportInternalError(self, sprintf(ErrTxt, "console: '%s' is not a directory!  %s", NewName, LastModTime == &LastMailMod ? "You probably can't receive mail!" : "Can't count any files in it!"));
	*LastModTime = 0;
	return(-1);
    }
	...
	ReportInternalError(self, sprintf(ErrTxt, "console: Cannot open directory '%s'; problem with AFS?", NewName));
	*LastModTime = 0;
	return(-1);
    }
    if(WildCard){
...
int requested;
{
    ...
    if (iserror) {
        value = -1;
        ReportInternalError(self, sprintf(ErrTxt, "console: Mail checking terminated (%s)", sys_errlist[errno]));
        ...
        if (Numbers[i].IsDisplaying) {
            ...
            if (Numbers[i].Value < 0) {
                if (!IsViceError(errno)) {
                    ReportInternalError(self, sprintf(ErrTxt, "console: Monitoring of %s terminated (%s)", Numbers[i].RawText, sys_errlist[errno]));
                    free(Numbers[i].RawText);
                    Numbers[i].RawText = Nullity;
                    Numbers[i].IsDisplaying = FALSE;
                } else {
                    ReportInternalError(self, sprintf(ErrTxt, "console: Temporary failure to check %s", Numbers[i].RawText));
                }
            }
...
struct consoleClass *self;
{
    ...
    if (stat(PrintDirName, &dirstatbuf) == -1) {
        if (IsViceError(errno)) {
	    ReportInternalError(self, sprintf(ErrTxt, "console:  Cannot stat directory %s: cannot check print queue now.", PrintDirName));
            PrintDirModTime = 0;
            return(-1);
	}
        return AbortPrintChecking(self, sprintf(ErrTxt, "console:  Printing monitor terminated (%s)", sys_errlist[errno]));
    }
    if ((dirstatbuf.st_mode & S_IFMT) != S_IFDIR) {
    ...
    if ((dp = opendir(PrintDirName)) == NULL) {
        return AbortPrintChecking(self, sprintf(ErrTxt, "console: Printing monitor terminated on opendir (%s)", sys_errlist[errno]));
    }
...
struct consoleClass *self;
{
    ...
    if (stat(OutgoingDir, &statbuf) == -1) {
        if (IsViceError(errno)) {
	    ReportInternalError(self, sprintf(ErrTxt, "console:  Cannot stat directory %s: cannot check outgoing mail now.", OutgoingDir));
            OutgoingModTime = 0;
            return;
        }
    ...
    if ((dp = opendir(OutgoingDir)) == NULL) {
        ReportInternalError(self, sprintf(ErrTxt, "console: Printing monitor terminated on opendir (%s)", sys_errlist[errno]));
        return;
    }
    ...
    while ((dirent = readdir(dp)) != NULL) {
        if (dirent->d_name[0] == 'Q') {
            ...
            if (stat(FullName, &statbuf) == -1) {
                if (errno != ENOENT){
                    ReportInternalError(self, sprintf(ErrTxt, "console: Cannot stat outgoing mail file %s (%s)", dirent->d_name, sys_errlist[errno]));
                }
                continue;
            }
	...
	if (ThisTime > LastTime || requested){
		...
		while(1){
			...
			if ((c > 14 && !strncmp(header, "ReSent-From:", 12))
			    ...
			    ReportInternalError(self, sprintf(buf, "mail>> %s", header));
			    fclose(fp);
			    return;
			}
		    }
