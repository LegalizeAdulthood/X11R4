...
******************************************************************* */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/console/lib/RCS/input.c,v 2.32 89/10/21 16:22:06 bobg Exp $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/console/lib/RCS/input.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/console/lib/RCS/input.c,v 2.32 89/10/21 16:22:06 bobg Exp $";
#endif /* lint */
    ...
    int rock;
{
    ...
    if (whichvar < 0 || whichvar > NUMINTERNALVARIABLES) {
        ReportInternalError(self, sprintf(ErrTxt, "console: Variable %d does not exist", whichvar));
    }
    else {
...
struct menulist *tempMenulist;
{
    ...
    sprintf(ErrTxt, "Display~2, Restart %s", conlib[0].confile);
...
struct menulist *tempMenulist;
{
	...
	while (((dp = readdir(dirp)) != NULL) && connum < MAXCONFILES) {
		...
		if(!strcmp(tail, TmpBuf)){
		    *tail = '\0';
		    conlib[connum].path = (char *) malloc(strlen(libpaths[0]) + len2 + 4);
...
char *cardTitle;
{
	...
	while (((dp = readdir(dirp)) != NULL) && connum < MAXCONFILES){
		...
		if (!strcmp(tail, TmpBuf)) {
		    *tail = '\0';
		    conlib[connum].path = (char *) malloc(strlen(conpath) + len2 + 4);
