...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/supportviews/RCS/lprrulv.c,v 2.7 89/04/04 19:37:44 wjh Exp $ */
/* $ACIS:lprrulv.c 1.5$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/supportviews/RCS/lprrulv.c,v $ */

#ifndef lint
	char *lprrulerview_rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/supportviews/RCS/lprrulv.c,v 2.7 89/04/04 19:37:44 wjh Exp $";
#endif /* lint */
...
struct TickTbl {
	short major, minor;	/* number of pixels for major and minor cycles */
...
BogusCallFromParent(self, where, msg)
	register struct lprrulerview *self;
	char *where, *msg;
{
...
RepaintIcon(self, icon, color)
	register struct lprrulerview  *self;
	register enum iconcode icon;
	short color;
{
	register struct icondata *i = &(self->iconloc[(short)icon]);
	...
	register struct lprrulerview *self;
{
	...
	register cnt;
	for (i = &(self->iconloc[(short)leftIcon]), cnt = 3; cnt--; i++) {
		register boolean wasBlack = i->isBlack;
		if (i->value <= lprrulerview_NoValue)
			i->isBlack = FALSE;
		else
			i->isBlack = (i->x < i->left || i->x > i->right);
		if (i->isBlack != wasBlack) {
			register enum iconcode icon = (enum iconcode)(i - self->iconloc);
...
{
	short cycmax = tbl->major / tbl->minor;	/* number of minor cycles in a major */
	...
	*/
	tickloc = left + (zeroloc + (tbl->minor << 12) - left) % tbl->minor;
	ordloc = left + (zeroloc + (tbl->major << 12) - left) % tbl->major;
	ordval = (ordloc - zeroloc) / tbl->major * tbl->one;
	cyclecnt = cycmax - (ordloc-tickloc)/tbl->minor;
	if (cyclecnt == cycmax) cyclecnt = 0;

	lprrulerview_SetFont(self, TextFont);
	for ( ; tickloc < right; tickloc += tbl->minor) {
	...
	register long  x, y, num_clicks;
{
	...
	if (self->MovingIcon == noIcon  &&  y > self->topline) {
		...
		if (action == view_LeftUp) {
			register struct icondata *i = &(self->iconloc[(short)self->MovingIcon]);
			...
			if (self->MovingIcon != noIcon) {
				...
				if (self->leftline <= x && x <= self->rightline)
					sprintf(buf, self->TickTbl->fmt, 
							(relx + 0.0) / self->TickTbl->major * self->TickTbl->one);
				else 
