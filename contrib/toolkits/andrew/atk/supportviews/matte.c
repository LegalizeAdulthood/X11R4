...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/supportviews/RCS/matte.c,v 2.8 89/04/10 18:01:52 ghoti Exp $ */
/* $ACIS:matte.c 1.8$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/supportviews/RCS/matte.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/supportviews/RCS/matte.c,v 2.8 89/04/10 18:01:52 ghoti Exp $";
#endif /* lint */
...
struct view *parent;
{
    ...
    if(vr->viewType == NULL || !class_IsTypeByName(vr->viewType,"view")){
	if(!matte_PromptForViewName(self)) {
	    matte_Destroy(self);
	    return(NULL);
	}
    }
    if((self->child = (struct view *)class_NewObject(vr->viewType)) == NULL){
	matte_Destroy(self);
	return(NULL);
    }
    if(vr->dataObject)
...
struct menulist *menulist;
{
    ...
    view_PostMenus(self->header.view.parent, self->menus);
#else /* USEMENUS */
...
static struct bind_Description matteBindings[]={
    {"Allow-resizing",NULL,0,"matte,Toggle Resize",0,0,matte__SetResizing,"Allow view to resize" },
    {"Allow-drawing",NULL,0,"matte,Toggle Draw",0,0,matte__SetDrawing,"Draw the view's border" },
    NULL
};
#endif /* USEMENUS */
