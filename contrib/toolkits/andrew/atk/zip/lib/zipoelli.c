...
static char	rcsid[] = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/zip/lib/RCS/zipoelli.c,v 2.8 89/09/08 17:41:49 ghoti Exp $";
...
Draw( self, figure, pane, action )
  ...
  register unsigned char		  shade, width;
  ...
  if (( width = zip_Contextual_Figure_Line_Width( Data, figure )) > 0 ||       View->mouse_action == view_LeftMovement )
      {
    if ( zipview_GetLineWidth( View ) != width )
      zipview_SetLineWidth( View, width );
    zipview_DrawOvalSize( View, window_x_point - minor_radius, window_y_point - major_radius,
			  minor_radius << 1, major_radius << 1 );
