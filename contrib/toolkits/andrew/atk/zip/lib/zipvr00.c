...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/zip/lib/RCS/zipvr00.c,v 2.14 89/10/16 11:10:48 cfe Exp $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/zip/lib/RCS/zipvr00.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/zip/lib/RCS/zipvr00.c,v 2.14 89/10/16 11:10:48 cfe Exp $";
#endif /* lint */
...
zipprint__Draw_Multi_Line( self, npoints, x_origin, y_origin, points )
  ...
  if ( PostScriptLanguage )
    ...
    fprintf( PrintFile, "%s /lastx %.2f def /lasty %.2f def /shade %.2f def /LW %d def\n",
	     PrintPrefix, x_origin/100.0, y_origin/100.0,
	     shade, zipprint_Printing_Line_Width( self ) );
    nchunks = npoints / 200;
...
zipprint__Draw_Line( self, x1, y1, x2, y2 )
  ...
  if ( PostScriptLanguage )
    ...
    if ( zipprint_Printing_Line_Width( self ) )
      fprintf( PrintFile, "%s %.2f %.2f %.2f %.2f %d  zip_Line\n", PrintPrefix,
			x2/100.0, y2/100.0, x1/100.0, y1/100.0,
			zipprint_Printing_Line_Width( self ) );
    }
...
zipprint__Draw_Rectangle( self, left, top, right, bottom )
  ...
  {
  static char				  format[] =
"%s %.2f %.2f %.2f %.2f %.2f %d  zip_Rectangle\n";
  ...
  if ( PostScriptLanguage )
    ...
    fprintf( PrintFile, format, PrintPrefix,
	     ...
	     zipprint_Printing_Shade( self ), zipprint_Printing_Line_Width( self ) );
    }
...
zipprint__Draw_Round_Rectangle( self, left, top, right, bottom, x_radius, y_radius )
  ...
  {
  static char				  format[] =
"%s %.2f %.2f %.2f %.2f %.2f %.2f %.2f %d  zip_Round_Rectangle\n";
...
zipprint__Draw_Ellipse( self, x_center, y_center, x_radius, y_radius )
  ...
  if ( PostScriptLanguage )
    ...
    if ( zipprint_Printing_Line_Width( self ) )
      {
      fprintf( PrintFile, "%s %d setlinewidth\n", PrintPrefix,
		zipprint_Printing_Line_Width( self ) );
...
zipprint__Arc_To( self, x_center, y_center, x_radius,
  ...
  if ( PostScriptLanguage )
    ...
    if ( zipprint_Printing_Line_Width( self ) )
      ...
      fprintf( PrintFile, "%s %d setlinewidth\n", PrintPrefix, zipprint_Printing_Line_Width( self ) );
