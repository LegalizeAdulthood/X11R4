...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/fad/RCS/fadv.c,v 2.9 89/11/03 11:10:38 tpn Exp $ */
/* $ACIS:fadv.c 1.6$ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/fad/RCS/fadv.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/atk/fad/RCS/fadv.c,v 2.9 89/11/03 11:10:38 tpn Exp $";
#endif /* lint */
...
struct fadview *self;
{
	...
	for(i = 0; i < self->framecount; i++){
		...
		/* first test if more than a second has already past,
		    if not, then the loop can work without testing tv_sec 
			if so, then we don't have to loop, since mtm can't be more
			    then a second
			    */
		if(!(now.tv_sec > then.tv_sec && now.tv_usec >= then.tv_usec)){
		    do{
			if(now.tv_usec < then.tv_usec) {
			    then.tv_usec += -1000000;
			}
			if(now.tv_usec - then.tv_usec >= self->mtm) break;
			gettimeofday(&now,NULL);
		    }while (1) ;
		    gettimeofday(&then,NULL);
		}
...
struct classheader *classID;
{
    ...
    for(c = "cnlramIfuEdDsRbBN",buf[1] = '\0'; *c; c++){
	*buf = *c;
