...
#ifndef	lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/oda/isode/RCS/UPEPY-01.c,v 1.1 89/09/21 05:17:36 mss Exp $";
#endif
...
PEPYPARM parm;
{
    ...
    if ((p0_len = len) == 0)
        p0_len = strlen (p0);
    if (p0 == NULLCP) {
        advise (NULLCP, "IA5String initialization fails");
        return NOTOK;
    }
    if (((*pe) = str2prim (p0, p0_len, PE_CLASS_UNIV, 22)) == NULLPE) {
        advise (NULLCP, "IA5String: out of memory");
        return NOTOK;
    }
