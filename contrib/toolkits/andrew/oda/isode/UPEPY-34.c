...
#ifndef	lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/oda/isode/RCS/UPEPY-34.c,v 1.1 89/09/21 05:19:02 mss Exp $";
#endif
...
PEPYPARM parm;
{
    ...
    if ((p9_len = len) == 0)
        p9_len = strlen (p9);
    if (p9 == NULLCP) {
        advise (NULLCP, "GraphicString initialization fails");
        return NOTOK;
    }
    if (((*pe) = str2prim (p9, p9_len, PE_CLASS_UNIV, 25)) == NULLPE) {
        advise (NULLCP, "GraphicString: out of memory");
        return NOTOK;
    }
