...
/*****************************************************************
 * flgifc.c: FBM Library 0.9 (Beta test) 07-Mar-89  Michael Mauldin
 *
...
#define char_init   chinit
#define char_out    chrout
#define cl_block    clblck
#define cl_hash	    clhash
#define flush_char  flchar
#define ClearCode   clcode
#define a_count	    acont
#define clear_flg   clrflg
#define codetab	    codtab
#define cur_accum   curacm
#define cur_bits    curbts
#define free_ent    freent
#define g_init_bits ginbts
#define in_count    incont
#define maxbits	    mxbits
#define maxcode	    mxcode
#define maxmaxcode  mxmxcd
#define out_count   outcnt

#define EOFCode	    eofcod
...
/*
 * To save much memory, we overlay the table used by compress()
 * with those used by decompress().  The tab_prefix table is the
 * same size and type as the codetab.  The tab_suffix table needs
 * 2**BITS characters.  We get this from the beginning of htab.
...
#ifdef SIGNED_COMPARE_SLOW
    while ( (c = GIFNextPixel( ReadValue )) != (unsigned) RG_EOF ) {
#else
    while ( (c = GIFNextPixel( ReadValue )) != RG_EOF ) {
#endif
...
static flush_char()
{
    ...
    ch = accum;
	if( a_count > 0 ) {
		(*fbm_PutByte)(fbm_Parameter, a_count);
		for (i = 0; i < a_count; i++) {
