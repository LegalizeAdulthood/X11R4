...
\* ********************************************************************** */
/*
$Header: cuilib.c,v 1.1 89/12/05 21:18:00 xguest Exp $
$Source: /xsrc/contrib/toolkits/andrew/ams/libs/cui/RCS/cuilib.c,v $
*/
#include <andyenv.h>
#include <cui.h>
#include <errprntf.h>
...
extern char *malloc (), *realloc (), *NextAddress(), *cvEng();
...
char *Vers;
{
    ...
    CUI_ClientVersion = malloc(strlen(Vers) + 25 + strlen(SnapVersionString));
    sprintf(CUI_ClientVersion, "%s.CUILIB.%d.%d.SNAP.%s", Vers, CUI_MAJORVERSION, CUI_MINORVERSION, SnapVersionString);
}
...
/* This next structure ought to be replaced by a hash table -- especially for
	people who read a zillion bboards */
...
struct CUIDirNode {
    ...
    int NeedsPurging;
    unsigned int WasDeleted:1, HasVerified:1;
} *CUIDirCacheStart = (struct CUIDirNode *)NULL;
       ...
       *bodydir; /* Ignored 4/22/88 */
{
    ...
    ++slash;
    if ((*slash == '+') || index(slash, ' ') || index(slash, '.')) {
...
char   *arg;
{
    ...
    SpecFile[0] = '\0';
    if (secondarg = index(cleanarg, ' ')) { /* should be more flexible in parsing */
	*secondarg++ = '\0';
...
char *shortname, *longname;
{
    ...
    return(CUI_SetDirNode(shortname, longname, &Node));
}
...
struct CUIDirNode **Node;
{
    ...
    for (DNtmp = CUIDirCacheStart; DNtmp != NULL; DNtmp = DNtmp->Next) {
	if (!strcmp(DNtmp->NickName, myname) || !strcmp(DNtmp->FullName, myname)) {
	    if (DNtmp->WasDeleted) {
		continue;
	    }
	    if (!DNtmp->HasVerified) {
		if (longname) {
		    return(0); /* Already set */
		} else {
		    mserrcode = MS_DisambiguateFile(DNtmp->FullName, NameBuf, AMS_DISAMB_DIREXISTS);
		    if (mserrcode) return (mserrcode);
		    if (strcmp(DNtmp->FullName, NameBuf)) {
			s = malloc(1+strlen(NameBuf));
			if (! s) {
    ...
    if (longname) {
	strcpy(NameBuf, longname);
    } else {
	if ((mserrcode = MS_DisambiguateFile(myname, NameBuf, AMS_DISAMB_DIREXISTS)) != 0) {
	    return(mserrcode);
	}
    } 
...
char *Dname;
{
    ...
    for (DNtmp = CUIDirCacheStart; DNtmp != NULL; DNtmp = DNtmp->Next) {
	if (!strcmp(Dname, DNtmp->FullName)) {
...
CUI_DirectoriesToPurge() {
    ...
    return(total);		/* Number of directories in need of
				   purging */
}
...
int change;
{
    ...
    for (DNtmp = CUIDirCacheStart; DNtmp != NULL; DNtmp = DNtmp->Next) {
	if (!strcmp(DNtmp->FullName, dirname)) {
...
char *OrigDirName;
{
	...
	newpart = rindex(DirName, '.');
	newpart2 = rindex(DirName, '/');
	if (newpart2 > newpart) {
	    newpart = newpart2;
	}
	if (newpart) {
...
char *old, *new;
{
	...
	} else if (!strcmp(OldName, NewName)) {
    ...
    for (DNtmp = CUIDirCacheStart; DNtmp != NULL; DNtmp = DNtmp->Next) {
	if (!strcmp(DNtmp->FullName, OldName)) {
	    /* Here we leak a little core, because the application might
	       be keeping a pointer to the name and the name might have
	       grown so that we can not just copy over it.  So we throw
	       away the old storage for FullName and NickName.  This is
	       not too horrible because renames are extremely rare. */
	    DNtmp->FullName = malloc(1+strlen(NewName));
	    DNtmp->NickName = malloc(1+strlen(new));
	    if (!DNtmp->FullName || !DNtmp->NickName) {
		AMS_RETURN_ERRCODE(EMSNOMEM, EIN_MALLOC, EVIA_RENAMEDIR);
	    }
	    strcpy(DNtmp->FullName, NewName);
	    strcpy(DNtmp->NickName, new);
	    break;
	}
    }
...
CUI_FreeCaches() {
    ...
    cdp = CUIDirCacheStart;
    while (cdp) {
	CUIDirCacheStart = cdp->Next;
	free(cdp->NickName);
	free(cdp->FullName);
	free(cdp);
	cdp = CUIDirCacheStart;
...
char *Snapshot;
{
    ...
    NewDir[0] = '\0';
    if (CUI_GetHeaderContents(cuid,(char *) NULL, HP_DIRECTORYCREATION, NewDir, MAXPATHLEN) != NULL) {
...
char *Snapshot;
{
    ...
    if (CUI_GetHeaderContents(cuid,(char *) NULL, HP_VOTEREQUEST, VoteRequest, sizeof(VoteRequest) - 1) != NULL) {
    /* error already reported */
	return(-1);
    }
    if (VoteRequest[0]) {
	...
	if (FindQuotedString(VoteRequest, &VoteVector[0], &s) || FindQuotedString(s, &VoteVector[2], &dumstr) || VoteVector[0] == NULL || VoteVector[2] == NULL) {
	    ReportSuccess("Badly-quoted string in vote header");
	    return(-1);
	}
	if (CUI_GetHeaderContents(cuid,(char *) NULL, HP_VOTETO, VoteTo, sizeof(VoteTo) - 1) != NULL) {
	    /* error already reported */
	    return(-1);
	}
	VoteVector[1] = VoteTo;
	if (CUI_GetHeaderContents(cuid,(char *) NULL, HP_VOTECHOICES, VoteChoices, sizeof(VoteChoices) - 1) != NULL) {
	    /* error already reported */
	    return(-1);
	}
	s = VoteChoices;
	...
	if (CUI_GetHeaderContents(cuid,(char *) NULL, HP_OLDVOTE, VoteChoices, sizeof(VoteChoices) - 1) != NULL) {
	    /* error already reported */
	    return(-1);
	}
...
char *Snapshot;
{
    ...
    if (CUI_GetHeaderContents(cuid,(char *) NULL, HP_ACKTO, AckHeader, sizeof(AckHeader) - 1) != NULL) {
	/* error already reported */
	return(-1);
    }
...
char *Snapshot;
{
	...
	if (CUI_GetHeaderContents(cuid,(char *) NULL, HP_ENCLOSURE, Separator, sizeof(Separator) - 1) != NULL) {
	    /* error already reported */
	    return(-1);
	}
	if (!Separator[0]) {
...
static char *ExtVec[] = {
    ...
    "Trust the delivery system to remove it as needed", /* 4 -new */
    NULL
};
...
char *dirname, **result;
{
    ...
    *result = malloc((len > MAXPATHLEN+2)? len+2 : MAXPATHLEN+2);
    ...
    if ((index(dirname, ',') != NIL) || (strlen(dirname) > MAXPATHLEN)) {
	...
	mserrcode = CUI_DisambiguateDir(dirname, result);
