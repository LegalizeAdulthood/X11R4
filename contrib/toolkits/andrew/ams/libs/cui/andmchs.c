...
\* ********************************************************************** */
/*
$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/cui/RCS/andmchs.c,v 2.20 89/08/03 13:22:09 ghoti Exp $
$Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/cui/RCS/andmchs.c,v $
*/
...
extern char *getenv(), *malloc(), *getprofile();
...
int *len, *type, IsRecon;
{
	...
	*ThisUser = UserNameBuf;
	...
	if (HostNameBuf[0] == '?') {
	    if (!IsRecon) {
		GetStringFromUser("Name of host to run message server for you", HostNameBuf, sizeof HostNameBuf);
		sprintf(ErrorText, "User id on host %s", HostNameBuf);
		GetStringFromUser(ErrorText, UserNameBuf, sizeof UserNameBuf);
	    }
	    *ThisUser = UserNameBuf;
	}
