...
\* ********************************************************************** */
/*
$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/msprint.c,v 2.22 89/09/06 10:24:48 cfe Exp $
$Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/msprint.c,v $
*/
#include <ms.h>
#include <errprntf.h>
#include <hdrparse.h>
...
extern char *malloc(), *StripWhiteEnds(), *index(), *rindex(), *getenv();
extern double getla();
...
int flags;
{
    ...
    GetBodyFileName(DirName, id, RawFileName);
    sprintf(PrintQueueFileName, "%s/%s", home, PRINTPREFIX);
    if (printer) {
...
Boolean MustPrint;
{
	...
	if (!strncmp(dirent->d_name, PRINTPREFIX, prefixsize)) {
	    ...
	    thisprinter = index(bashable_name, '_');
	    if (thisprinter) {
		*thisprinter++ = '\0';
		s = index(thisprinter, '_'); /* future compatibility */
		if (s) *s++ = '\0';
	    }
	    if (thisprinter) {
