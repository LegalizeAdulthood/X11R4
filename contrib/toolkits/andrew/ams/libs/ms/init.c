...
\* ********************************************************************** */
/*
$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/init.c,v 2.35 89/10/24 16:10:03 cfe Exp $
$Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/init.c,v $
*/
#include <andyenv.h>
#include <andrewos.h>
#include <ms.h>
...
extern char **environ;
extern char *getprofile(), *index(), *rindex(), *StripWhiteEnds(), *malloc();
extern FILE *fopen();
...
Boolean UsingSnap;
{
	...
	if (RC > 0) {
	    ...
	    else sprintf("Can't read server config file for AMSHome domain %s: %s; can't configure to use that domain!", MyMailDomain, UnixError(errno));
	    FatalError(NameBuf);
	    safeexit(-1);
	}
    }
	...
	strcpy(NameBuf, Me);
	if (homeUsesAMSDelivery > 0) strcat(NameBuf, "+");
	strcat(NameBuf, "@");
	strcat(NameBuf, MyMailDomain);
    }
    ...
    if (MS_NameSep > 0) {	/* Now make MyPrettyAddress */
...
GetNameFromGecos(fullname, userid, domain, newname)
char *fullname, *userid, *domain;
char **newname;
{
	...
	strncpy(NameCopy, fullname, sizeof(NameCopy));
	NameCopy[sizeof(NameCopy)-1] = '\0';
	strt = NameCopy;
	if (*strt == '*') ++strt;
	for (othr = strt; *othr != '\0'; ++othr)	{   /* look for sysV gecos format */
	    if (*othr == '-' && othr[1] != '\0') {	/* Aha!  digits, hyphen: call it sysV. */
		++othr;
		strt = othr;
		break;
	    }
	...
	*newname = malloc(len);
	if (*newname != NULL) {
	    if (ampersand != NULL) {
		strncpy(*newname, strt, ampersand - strt + 1);
		othr = (*newname) + (ampersand - strt);
		strcpy(othr, userid);
	...
	*newname = malloc(1+strlen(fullname));
	if (*newname != NULL) strcpy(*newname, fullname);
    }
    return(0);
}
