...
\* ********************************************************************** */
/*
$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/subs.c,v 2.25 89/09/07 16:47:39 cfe Exp $
$Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/subs.c,v $
*/
#include <ms.h>
#include <mailconf.h>
#include <stdio.h>
...
extern char *malloc(), *index(), *realloc(), *StripWhiteEnds(), *getprofile(), *convlongto64(), *fixDate();
...
{
    int i, top, bottom, split, code, WhichTry;
    ...
    subs.key = name;
    for (WhichTry = 0; WhichTry < 2; ++WhichTry) {
	subs.pathelt = -1;
	...
	if (code) {
	    if (WhichTry == 0) {	/* make sure a bogus name not given */
		(void) DeSymLink(name, Full1, 0);
		if (abspath(Full1, Full2) == 0 && strcmp(name, Full2) != 0) {
		    subs.key = Full2;
		    continue;
		}
	    }
...
ReadSubs() 
{
    ...
    Boolean GoodParse;
    static Boolean HasCheckedPathChanges = FALSE;
...
CheckPathChanges() {
    ...
    if (strcmp(oldlocal, LOCALSEARCHPATHTEMPLATE)) LocalPathChanged = TRUE;
    if (strcmp(oldexternal, EXTERNALSEARCHPATHTEMPLATE)) ExtPathChanged = TRUE;
    if (strcmp(oldofficial, OFFICIALSEARCHPATHTEMPLATE)) OffPathChanged = TRUE;
...
long newdate;
{
    ...
    SetFullProfileEntry(FALSE, FullName, NULL, 0, TRUE, newvalue, newdate, FALSE);
}
...
char *SubsFileName;
{
	...
	cp = rindex(SubsNewName, '/');
	cp2 = rindex(SubsFileName, '/');
