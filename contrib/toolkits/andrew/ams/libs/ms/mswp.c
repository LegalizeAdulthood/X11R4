...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/mswp.c,v 2.39 89/11/07 12:49:40 cfe Exp $ */
/* $ACIS: $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/mswp.c,v $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/mswp.c,v 2.39 89/11/07 12:49:40 cfe Exp $ ";
#endif /* lint */
...
extern char *malloc(), *index(), *rindex(), *realloc(), *StripWhiteEnds();
...
{/* Generate MSPath elements from the current user's list plus, if cellName is non-null and not MyMailDomain, the default mspath for that cell.
    ...
    *SPEix = -1;
    while (*genPos < MS_NumDirsInSearchPath) {
...
Boolean WouldCreate;
{
	...
	if (mfp == NULL) {
		...
		*rcode = MSWP_CRAP;
	    ...
	    while (subst = index(MapName, '/')) {
		*subst = '.';
	    }
	    subst = index(RealRecip, '*');
...
static wp_FieldIndex idxN = -1, idxID, idxAf;
...
static int RefreshAliasFile()
{
	...
	while (*s != '\0') {
		...
		if (!strncmp(firstpart, "$forceformat", 12)) {
		    ...
		    while (*s != '\0' && *s != '\n') ++s;
		    if (*s != '\0') *s++ = '\0';
		    sprintf(ErrorText, "Unparsable $force line '%s': ignored", firstpart);
		...
		if (*s == '\0' || *s == '\n') {
		    if (*s != '\0') *s++ = '\0';
		    sprintf(ErrorText, "Unparsable alias line '%s': ignored (length %d)", tempalias->nick, strlen(tempalias->nick));
		    NonfatalBizarreError(ErrorText);
		    free(tempalias);
		    continue;
		}
...
{/* Do the White Pages lookup here; result in the Addr structure and MSWP_code to WpCode. */
    ...
    wp_ErrorCode wpErr;
    wp_PrimeKeySet *PKs;
    int MinMatch, OutMatch, size, i, code, WasFuzzy;
	...
	*WpCode = WasFuzzy ? MSWP_MATCHTOOFUZZY : (CheckAMSFmtOK(Domain) <= 0 ? MSWP_GOODNETMAIL : MSWP_GOODUSER);
	...
	if (CanonName == NULL || NameSep <= 0 || (laType == latype_LocalID && *PostID)) {
		...
		if (laType == latype_LocalID)
			...
			sprintf(NewName, UnderAMSDelivery > 0 ? "%s+" : "%s", CanonID);
	    }
	    CanonName = NULL;
		...
		*WpCode = MSWP_TEMPFAIL;
	    ...
	    free(PKey);
	    if (CanonName != NULL) {
		CommText = NewString(CanonName);
		if (CommText != NULL) {
		    if (Addr->RoutePhrase != NULL) free(Addr->RoutePhrase);
		    Addr->RoutePhrase = CommText;
		}
	    }
	} else {
	...
	*WpCode = WasFuzzy ? MSWP_PROBABLYFUZZYAMBIG : MSWP_AMBIGWITHERRORS;
	...
	if (CanonName != NULL && NameSep > 0) {
		...
		*WpCode = MSWP_TEMPFAIL;
	    ...
	    CPtr = &CommText[strlen(CommText)];
	    strcpy(CPtr, (CanonID ? CanonID : CanonName));
	    if (CanonID == NULL)		/* spaces to underscores */
		for (; *CPtr != '\0'; ++CPtr)
		    if (*CPtr == ' ') *CPtr = '_';
	    strcat(CommText, (CanonID && UnderAMSDelivery > 0) ? "+@" : "@");
...
int MaxNameMatches, AllowHeuristics, UnderAMSDelivery, NameSep, *Answered;
{
	...
	*WpCode = (CheckAMSFmtOK(Domain) <= 0 ? MSWP_GOODNETMAIL : MSWP_GOODUSER);
	...
	if (NameSep <= 0 || !RealName || (laType == latype_LocalID && *PostID)) {
		...
		sprintf(NewName, (UnderAMSDelivery > 0 ? "%s+" : "%s"), p->pw_name);
	    }
	    if (RealName) {
		if (Addr->RoutePhrase) free(Addr->RoutePhrase);
...
static int LookupInLocalDatabase(Addr, laType, IDpart, PostID, IsGoodLocalName, Domain, UnderAMSDelivery, NameSep, Answered)
...
int *IsGoodLocalName; /* OUT.  The Boolean answer about if this is a good name */
...
{
    *Answered = 0; /* By default, ignore this routine */
    return(0); /* But don't indicate any errors, either */
}
...
int MaxNameMatches, AllowHeuristics, UnderAMSDelivery, NameSep, *IsVacuous;
{
    ...
    *WpCode = MSWP_CRAP; /* A default in case no validation succeeds */
	...
	isnext = FALSE;
	if (curr == LocalDBVal) {
	    int IsGoodLocalName = 0;

	    if (LookupInLocalDatabase(Addr, laType, IDpart, PostID, &IsGoodLocalName, Domain, UnderAMSDelivery, NameSep, &Answered)) {
		AMS_RETURN_ERRCODE(errno, EIN_LOCALNAMEDB, EVIA_REWRITEADDRESS);
	    }
	    if (Answered) {
		*WpCode = IsGoodLocalName ? (CheckAMSFmtOK(Domain) <= 0 ? MSWP_GOODNETMAIL : MSWP_GOODUSER) : MSWP_CRAP;
		return(0);
	    }
...
/* Add back the default host name if there is none; record whether there was one. */
	...
	case latype_DirInsert:
		...
		if (outType == latype_DistList && recdepth < 13 && GetCellFromFileName(FileBuf, FileCell, sizeof(FileCell)) == 0 && ULstrcmp(FileCell, Domain) != 0 && CheckAMSDelivery(FileCell) < 0) {
		    sprintf(ErrMsg, "Distribution list %s is in cell %s, which does not support the AMS delivery system.", ap_Shorten(FileBuf), FileCell);
...
/* The CheckPersonalAlias case, as well as the .MS.DirectPost file case, put their results into SideBuf, not into the address list.  Those two cases are the only such ones: all other cases put their results into the address list. */
    ...
    if (AMSDel >= 0 && outType == latype_LocalID) {
	char *newloc;
	...
	if (pass) { /* Is a valid user id */
	    int PostCode;
	    if (pass->pw_gecos && *pass->pw_gecos) {
		char *realname;

		GetNameFromGecos(pass->pw_gecos, pass->pw_name, Domain, &realname);
		if (!realname) {
		    if (IDpart) free(IDpart);
		    AMS_RETURN_ERRCODE(ENOMEM, EIN_MALLOC, EVIA_REWRITEADDRESS);
		}
		if (AMSNameSep <= 0 || (PostID && *PostID)) {
		    if (Addr->RoutePhrase != NULL) free(Addr->RoutePhrase);
		    Addr->RoutePhrase = realname;
		} else {	/* Try to write as firstname.lastname. */
		    char *S;
		    for (S = realname; *S != '\0'; ++S) if (*S == ' ') *S = AMSNameSep;
		    if (Addr->LocalPart != NULL) free(Addr->LocalPart);
		    Addr->LocalPart = realname;
		}
	    }
	    PostCode = -1;
...
/* We've read the address (recip) from some .MS.DirectPost file.  We need to validate the .MS.DirectPost file address according to the cell in which we found that file, if for no other reason than to get that cell's name appended to the address. */
	    ...
	    *ErrCode = (PostCode >= 0 ? PostCode : (CheckAMSFmtOK(Domain) <= 0 ? MSWP_GOODNETMAIL: MSWP_GOODUSER));
	    if (IDpart) free(IDpart);
	    return(0);
	} 
    }
    /* Invalid user id will pass through to here for a wp check (in LookupLocalName) */
...
/* Names with spaces or tabs in them aren't bboards, but be kind on the ends. */
    ...
    PostCell = NULL;
    if (index(Addr->LocalPart, ' ') || index(Addr->LocalPart, '\t')) {
	*ErrCode = MSWP_CRAP;
    } else {
...
int codein;
{
    ...
    if (codein != MSWP_GOODNETMAIL && codein != MSWP_GOODEXTMSDIR && codein != 
MSWP_GOODUSER && codein != MSWP_GOODMSDIR) return(codein);
    if (RefreshAliasFile()) return (mserrcode);
    fetmp = ForceExtRoot;
    domainindex = index(ExtAddress, '@');
    while (fetmp) {
