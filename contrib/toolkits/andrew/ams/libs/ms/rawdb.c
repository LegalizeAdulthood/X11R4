...
\* ********************************************************************** */
/*
$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/rawdb.c,v 2.50 89/11/07 10:50:15 cfe Exp $
$Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/rawdb.c,v $
*/
/*
	rawdb.c -- Routines for accessing MS database files in the raw.
*/

char            ams_rawdb_rcsid[] = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/rawdb.c,v 2.50 89/11/07 10:50:15 cfe Exp $";
...
extern char    *malloc(), *index(), *rindex(), *permanentmalloc();
extern long     time();
...
int             HowMany;
{				/* Makes sure that HowMany entries are
				 * allocated in the IDs array hanging off
				 * Dir->IDs.  Returns 0 if OK, -1 if not. */
    int             Ix;
...
int             Overwrite;
{
    ...
    *NewDir = (struct MS_Directory *) permanentmalloc(sizeof(struct MS_Directory));
	...
	if (Overwrite) {
	    MarkInProgress(Dirname);	/* If we overwrite, wouldn't hurt to
...
struct MS_Directory *Dir;
{
    ...
    bzero(DirectoryHeader + i + 4 + ATTNAMESLEN, AMS_DIRHEADSIZE - (i + ATTNAMESLEN + 5));
...
int DoComplain;
{
    ...
    char            ConvBuf[PADSIZE + 1];	/* For conversion of ascii long ints */
    struct stat     statbuf;
	...
	if (DoComplain) AnnounceBadDirFormat(Dir->UNIXDir);
...
#endif /* AFS_ENV */
	AMS_RETURN_ERRCODE(errno, EIN_OPEN, EVIA_WRITEPUREFILE);
    }
...
#endif /* AFS_ENV */
	AMS_RETURN_ERRCODE(errno, EIN_VCLOSE, EVIA_WRITEPUREFILE);
    }
    return (0);
}
...
int             Code;
{
    ...
    if (Code == MD_OK) {
	close(fd);
	debug(128, ("Quickly closed dir %s\n", DirPath));
    }
    else {
	Dir->fd = fd;
	Dir->OpenMode = Code;
...
int             CloseMode;
{
    ...
    if ((CloseMode >= MD_APPEND) && Dir->BadScavenge == 0 && UnmarkInProgress(Dir->UNIXDir)) {
...
char           *fixDate(dPtr)
char           *dPtr;
{/* Fix up the base-64 value (size AMS_DATESIZE) at dPtr.  Return a pointer to it. */
    int             X;
    register char  *cp;
...
int            *msgnum;
{
	...
	if (lseek(Dir->fd, AMS_DIRHEADSIZE + (i * AMS_SNAPSHOTSIZE), L_SET) < 0) {
	    AMS_RETURN_ERRCODE(errno, EIN_LSEEK, EVIA_GETSNAPSHOTBYID);
	}
	readct = read(Dir->fd, snapshot, AMS_SNAPSHOTSIZE);
...
char           *snapshot;
{
    ...
    if (lseek(Dir->fd, AMS_DIRHEADSIZE + (num * AMS_SNAPSHOTSIZE), L_SET) < 0) {
...
char           *Dirname;
{
    ...
    strcat(UpdateFileName, "/");
    strcat(UpdateFileName, MS_MASTERDIR);
    if (access(UpdateFileName, F_OK) && errno == ENOENT) {
	return (0);		/* No MUF on this tree */
    }
	...
	*maybehere = '\0';
    ...
    close(fd);			/* It was already created, it is empty, who
				 * cares about a close error? */
    debug(2, ("OK: Left hint file '%s', things should be fine.\n", UpdateFileName));
    return (0);
}


#define NUMLOCKRETRIES 20
#define LOCKSLEEPPERIOD 10

static int      LockMUF(LockDirName, lockfd)
char           *LockDirName;
int            *lockfd;
{
...
char           *p1, *p2;
{
    ...
    elt1start = index(p1, '/');	/* Assuming slashes, not dots; hope that's OK */
    elt2start = index(p2, '/');
    while (elt1start && elt2start) {
	elt1end = index(++elt1start, '/');
	elt2end = index(++elt2start, '/');
	if (elt1end)
	    *elt1end = '\0';
	if (elt2end)
	    *elt2end = '\0';
	...
	return (elt1start != 0 ? 1 : -1);	/* (can't count on pointers
...
char           *PathElt;
{
	...
	FreeUpdates(FALSE);
	if (errno == ENOENT) {
	    return (0);		/* No update file means we don't create one */
	}
	AMS_RETURN_ERRCODE(errno, EIN_ACCESS, EVIA_UPDATEUPDATES);
    }
	...
	for (s = LineBuf; *s; ++s) {
		...
		NonfatalBizarreError(ErrorText);
		*s = 'X';	/* The error message should be kind to
				 * console */
		++mistakes;
	    }
	}
	...
	if (whichupdate >= UpdatesPending) {	/* finished with hint entries */
	    fputs(LineBuf, newW);
	    continue;
	}
	s = index(LineBuf, ' ');
	...
	if (code <= 0) {
		...
		/*
		 * Used to be fputs(LineBuf, newW) but this screwed up when
		 * two new things needed to be inserted consecutively 
		 */
	    }
...
struct CheckList {
    ...
    char          **ToCheck;
    int             Allocated, Used;
} FilesToCheck = {"file", 0, 0, 0},
   SnapsToCheck = {"snapshot", 0, 0, 0},
   Orphans = {"orphan", 0, 0, 0},
...
struct CheckList *CheckList;
{
	...
	if (CheckList->ToCheck) {
	    CheckList->ToCheck = (char **) realloc(CheckList->ToCheck, CheckList->Allocated * sizeof(char *));
	}
	else {
	    CheckList->ToCheck = (char **) malloc(CheckList->Allocated * sizeof(char *));
	}
	if (!CheckList->ToCheck)
    ...
    CheckList->ToCheck[CheckList->Used] = malloc(1 + strlen(name));
    if (!CheckList->ToCheck[CheckList->Used])
	AMS_RETURN_ERRCODE(ENOMEM, EIN_MALLOC, EVIA_MARKINPROGRESS);
    strcpy(CheckList->ToCheck[CheckList->Used], name);
    ++CheckList->Used;
    return (0);
}
...
char          **s1, **s2;
{
    return (conv64tolong(*s1) - conv64tolong(*s2));
}
...
int             Quiet, alien;
{
    char            FileName[1 + MAXPATHLEN], *partstart, ErrorText[200 + MAXPATHLEN];
    ...
    qsort(FilesToCheck.ToCheck, FilesToCheck.Used, sizeof(char *), CompareStrings);
    qsort(SnapsToCheck.ToCheck, SnapsToCheck.Used, sizeof(char *), CompareStrings);
    for (i = 0, j = 0; i < FilesToCheck.Used || j < SnapsToCheck.Used;) {
	if (j >= SnapsToCheck.Used) {
	    debug(2, ("Automatically using the file\n"));
	    code = -1;
	}
	else if (i >= FilesToCheck.Used) {
	    debug(2, ("Automatically using the snapshot\n"));
	    code = 1;
	}
	else {
	    code = strcmp(FilesToCheck.ToCheck[i], SnapsToCheck.ToCheck[j]);
	...
	if (code > 0) {
	    debug(2, ("Adding from snapshot: %s\n", SnapsToCheck.ToCheck[j]));
	    AddToCheckList(SnapsToCheck.ToCheck[j], &Missing);
	    ++j;
	}
	else {
	    debug(2, ("Adding from files: %s\n", FilesToCheck.ToCheck[i]));
	    AddToCheckList(FilesToCheck.ToCheck[i], &Orphans);
	    ++i;
	}
    }
    if (Orphans.Used > 0) {
	...
	for (i = 0; i < Missing.Used; ++i) {
	    ...
	    ++missingct;
	    debug(2, ("The directory %s has an orphaned snapshot in it -- %s!\n", Dir->UNIXDir, Missing.ToCheck[i]));
	    mserrcode = GetSnapshotByID(Dir, Missing.ToCheck[i], &msgnum, snapshot);
	    if (mserrcode) {
		++errs;
	    } else {
...
int *alien;
{
    ...
    FindTreeRoot(Dir->UNIXDir, Root, FALSE);
