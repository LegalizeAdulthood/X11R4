...
/*
$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/recon.c,v 2.17 89/10/26 11:02:48 cfe Exp $
$Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/recon.c,v $
*/
...
static char     ams_reconstruct_rcsid[] = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/libs/ms/RCS/recon.c,v 2.17 89/10/26 11:02:48 cfe Exp $";
...
static int      GetOldIDSomehow(Msg, FileName, Dir)
struct MS_Message *Msg;
char           *FileName;
    ...
    *NumGood = *NumBad = 0;
    ...
    if (oldDirReadable) {
        if (ReadOldMSDirectoryHead(oldMSDir)) {
            ...
            if (lseek(oldMSDir->fd, 0, L_SET) < 0) {
                oldDirReadable = FALSE;
                CloseMSDir(oldMSDir, MD_APPEND);
            }
            else {
    ...
    if (!oldDirReadable) {             /* BOGUS -- this is duplicated from
        ...
        bzero(headBuf + i + 4 + ATTNAMESLEN,
              AMS_DIRHEADSIZE - (i + ATTNAMESLEN + 5));
    }

    /* Defer writing of new header until we know newMaxChainVal */

    if (oldDirReadable) {
        if (IsDirAlien(oldMSDir, &alienDir)) {
            return (mserrcode);
        }
    }
    else
