...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/cui/RCS/cuifns.c,v 2.10 89/07/20 15:59:30 cfe Exp $ */
/* $ACIS: $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/cui/RCS/cuifns.c,v $ */

#ifndef NO_DUPLICATE_STATIC_CHARS
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/cui/RCS/cuifns.c,v 2.10 89/07/20 15:59:30 cfe Exp $ ";
...
extern char *index(), *StripWhiteEnds();
...
int def;
{
	...
	if (!myans) {
	    ...
	    len = strlen(ans);
	    for (i=1; QVec[i]; ++i) {
		if (!strnicmp(ans, QVec[i][0] == '\'' ? QVec[i]+1 : QVec[i], len)) {
		    lasthit = i;
		    ++hitcount;
		}
	    }
...
int len;
{
    ...
    if (len && *s1 && *s2) {
	return((isupper(*s2) ? tolower(*s2) : *s2) - (isupper(*s1) ? tolower(*s1) : *s1));
    } else {
	return(0);
    }
}
...
int code;
{
    ...
    next=index(arg,' ');   /* Allow further arguments */
    if (next) *next='\0';
