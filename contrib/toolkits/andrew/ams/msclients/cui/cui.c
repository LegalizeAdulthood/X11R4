...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/cui/RCS/cui.c,v 2.26 89/11/20 10:27:45 cfe Exp $ */
/* $ACIS: $ */
/* $Source: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/cui/RCS/cui.c,v $ */

#define CUIP_MAJ_VER 2
#define CUIP_MIN_VER 0

#ifndef NO_DUPLICATE_STATIC_CHARS
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/cui/RCS/cui.c,v 2.26 89/11/20 10:27:45 cfe Exp $ ";
...
extern	FILE * qopen ();
extern char *convlongto64 (), *StripWhiteEnds (), *index (), *malloc (), *realloc (),
	    *getenv (), *copy(), *GetLine(), *rindex();
...
void cmd_init()
{
    ...
    newcmd("epoch", Epoch, "'epoch <full path name> <date>' -- purge all messages in a tree before a given date", LEVEL_WIZARD);
...
char *arg;
{
	...
	i=strnicmp(t->header, arg, strlen(t->header));
	if (0<=i) break;
    }
    return((t==NULL || 0<i) != (TRUE==HeadKeep));
}
...
int	AliasCt;
{
	...
	FullName[0] = '\0';
	while(TRUE) {
	    if ((mserrcode = MS_GetNextSubsEntry(FullName, NickName, &substatus)) != 0) {
		ReportError("Cannot get your subscription information", ERR_CRITICAL, TRUE);
		return(-1);
	    }
	    if (FullName[0] == '\0') {
		return(0); /* no more subscriptions */
	    }
...
char   *arg;
{
	...
	*date++ = '\0';
    ...
    if (!*dname) {
	ReportError("You must supply a header specification", ERR_WARNING, FALSE);
	return(-1);
    }
    if (!date || !*date) {
...
char   *to;
{
	...
	if (CUI_NameReplyFile(cuid, code, TmpName) != 0)
	    return(-1);
	if (code == AMS_REPLY_REDRAFT) {
		...
		offset = FindBodyStartInViceFile(TmpName2);
...
char *strbuf;
{
	...
	while (InHeaders && UsefulStart) { /* Some bodies may not end in \n */
	    ...
	    if (*UsefulStart != ' ' && *UsefulStart != '\t' && !strnicmp(UsefulStart,"Subject",7)) {
...
FindBodyStartInViceFile(ViceFile)
char *ViceFile;
{
    char    Buf[MAXBODY], *s;
    int     bodylen;
