...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/vui/RCS/vui.c,v 2.29 89/07/21 17:45:55 cfe Exp $ */
...
#define DEFAULT_HEADERS "cc:date:from:subj:to"

extern char *index(), *getenv(), CUI_VersionString[], *GetSUser();
...
char *editor, heads[1024];
    ...
    sprintf(intro[1].pdata,"Version %d.%d",VUI_MAJORVERSION, VUI_MINORVERSION);
    ...
    RealWhenIs("today", intro[0].pdata);
    editor=index(intro[0].pdata,' ');
...
char *date = NIL, *matchstr = NIL, substatus, *savehelpfile, *reshowstr, matchbuf[81], dirnamebuf[MAXPATHLEN+1], *current_dir = dirnamebuf;
	...
	CheckForMail();
		...
		while (opt==(int)OPT_C_NEXT || opt==MENU_EMPTY) {
		    ...
		    if ((*current_dir==NULL) && (substatus == 'A'))
			substatus = '\0';
		    if (substatus=='A') {
