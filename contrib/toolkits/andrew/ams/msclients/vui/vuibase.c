...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/vui/RCS/vuibase.c,v 1.29 89/08/18 00:08:09 ghoti Exp $ */

#ifndef lint
static char *rcsid = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/vui/RCS/vuibase.c,v 1.29 89/08/18 00:08:09 ghoti Exp $ ";
#endif /* lint */
...
extern char *AndrewDir(), *GetSUser(), *getenv(), *index(), *copy();
extern char *malloc(), *realloc(); 
extern char VUI_editor[], *rindex(), *StripWhiteEnds(), CUI_VersionString[]; 
...
char *name;
{
    ...
    dfl->filename = malloc(1+strlen(name));
...
int freedata;
{
    ...
    result = malloc(headlen + datalen + 10);
    if (!result) {
...
Boolean erase_in_file, convert_tabs;
{
    ...
    do {
        ...
        if (bodylen <= 0) break;
        BodyBuf[bodylen+bytesleft] = '\0';
	if ((bytesunfetched <= 0) && (index(in_buf_p,'\n') == NIL)) {
