...
This package supports monochrome, color, and B/W displays via the standard PC BIOS Int 0x10 Video Interface.  Also supported are terminals with cursor movement capability via the curses/termcap interface on Unix systems.

Provides high level definition of input and output screens for easy user manipulation.  Also provides programmer-friendly mechanisms for defining hierarchic menus in the style of Lotus 1-2-3.
...
There are several useful definitions provided in the panel input/output package.  To use these your program must #include "panel.h".
...
A field is the fundamental input structure.  Each field consists of one or more extents for input and one prompt that will be shown when the field has the input focus.
...
A prompt is the text shown at the bottom of the screen when the user moves the cursor into a new field.
...
\example{PRMPT toprompt = \{0, "Enter the names of people to send the message to" \};
XTENT to_xtents[] = \{ 1, 4, TOCC_FIELD_LEN, 2, 4, TOCC_FIELD_LEN, 3, 4, TOCC_FIELD_LEN, 0, 0, 0 \};
...
PRMPT ccprompt = \{0, "Enter the names of people to receive copies of your message" \};
XTENT cc_xtents[] = \{ 5, 4, TOCC_FIELD_LEN, 6, 4, TOCC_FIELD_LEN, 7, 4, TOCC_FIELD_LEN, 0, 0, 0 \};
...
This structure allows you to build tables which define how to treat special keys.  The key codes are defined in keycodes.h which contains definitions for both the PC and terminals.
...
This structure holds the state of the system's video parameters.  Generally you will not need to change these.
...
The package provides data and constant definitions for the 1-2-3 style menus enhancements.  Your program must #include "lmenus.h".

\subheading{MKEYTAB
}
This structure is just like KEYTAB except that the handling functions return integers instead of field pointers.
...
This structure defines one menu option.  It consists of the one word description and a prompt to be displayed when the option is highlighted.
...
This structure provides the definition of the menu hierarchy.  The first element is an index into an array of menu options, the second is a pointer to a sub-menu.  (Currently the nesting has only been tested to a level of 3 deep.
...
The OPT_* constants can be used in the calling program to process the various options that are entered by the user.
 	
\heading{Function Calls for Paneling}

To use the functions listed in this section you must compile and link in the panelp.c module.

\subheading{InitPanels
}This procedure fills in the length fields in panel and field arrays to agree with the corresponding constant strings.  Some minimal consistency checking is also performed.
\example{
InitPanels (p, f)
PANEL *p[];
FIELD *f[];}
...
}This routine should be called early in your program to allow you to exit and return the machine to the state is was prior to running your program.
\example{
SaveVideoEnv (vp)
VIDEOPARMS *vp;}
...
}This routine allows you to put a text string on the screen.  Generally there are higher-level routines that are preferred.  Attr is one of the Field Attribute Values.
\example{
...
This routine redisplays the field currently with input focus.  (At one time this field was displayed in reverse video, but it was considered too harsh for data entry especially when red is used as the background color, plus it takes a while to paint the screen reverse over slow terminals.)
\example{
ShowCursor();}
...
}This routine displays a prompt.  Again, there are higher-level routines that are preferred.
...
This routine causes \italic{newfield} to become the current field.  The exit handler on the current field is called, its highlighting removed and the entry handler of the new field is called and it is highlighted.
\example{
...
This routine changes the current field, but does not update the screen or call any of the entry or exit handlers.
\example{
...
\subheading{GetSUser
}This routine gets a string from the user using the default locations for input and output.
...
\subheading{KeyIn
}This routine waits for the user to press a key and returns that value.  When possible use GetKey.
...
}This routine turns the flashing cursor off or moves it out of the way on terminals that can't turn off the cursor.

\example{CursorOff();
}
\subheading{CursorOn
}This routine turns the cursor on and displays it at the current input location.

\example{CursorOn();

}\heading{Function Calls for Menu bars
}
To use the functions listed in this section you must compile and link in the lmenus.c module.

\subheading{InitMenus
}Initialize a set of menus by determining the length of each prompt and inserting that value into the data structure.  This lets you avoid having to count the length of character strings.

\example{InitMenus(menu_opt)
MENU_OPTS *menu_opt;
}
\subheading{SetBaseMenu
}This lets the panelling code know what array you are using.  The menu trees contain indices into this array.
...
}Turn off the highlighting of a menu option.  Typically you will call this routine before beginning to process a menu option.

\example{EraseMenuCursor();
}
\subheading{ShowMenuCursor
}Call this routine to indicate that processing has finished and you are ready for input again.

\example{ShowMenuCursor();
}
\subheading{MenuInput
}Display a menu, handle keys according to the table and return a value.  The previous option is stored in last_opt_p.
...
Aside from the files mentioned above: (panel.h, panelp.c, lmenus.h, lmenus.c, keycodes.h) there is a file that contains screen manipulation routines that are machine specific, panelmach.c.
