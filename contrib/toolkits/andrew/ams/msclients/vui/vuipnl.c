...
\* ********************************************************************** */
/* $Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/msclients/vui/RCS/vuipnl.c,v 1.27 89/08/24 10:21:55 ghoti Exp $ */
...
extern char *malloc(), *index(), *GetSUser();
...
Boolean autoorman;
{
    ...
    if (DIR_PageBuf == NIL) {
	DIR_PageBuf = malloc(bar_row * (LINE_LENGTH + 2));
...
long startnum;
{
    ...
    *(++PageBufPtr)='\0';
    ...
    sprintf(mess_bp[1].pdata, "%ld of %ld.", msg_msgno[0]+1L, MSG_Total);
...
{
    long msgno,offset;
    char filename[MAXPATHLEN+1],tmpfile[MAXPATHLEN+1];
    ...
    if (GetHeaderFields(templatefile, tmpfile, &offset)) return(-1);
    if (DecompressFile(tmpfile, ENT_headerfile, &ENT_headlength, TRUE, TRUE)) return(-1);
...
int row,rc;
    ...
    if ((CurrentFile->PageBuf = malloc(CurrentFile->BodyLines * LINE_LENGTH)) == NIL) {
	ReportError(VUIMSG_NOMEM,ERR_FATAL,FALSE);
	return(-1);
    }
...
struct msg_page_list *this_page, **new_page_p;
{
    ...
    do {
        ...
        if (MS_GetPartialFile(CurrentFile->FileName,
			      headbuf,CurrentFile->BodyLines*LINE_LENGTH-1,
			      offset, &bytesunfetched, &bodylen) != 0) return(-1);
        debug((2,"Returned %d bytes and %ld are left\n",bodylen, bytesunfetched));
