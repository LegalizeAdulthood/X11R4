...
static char rcsid[] = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/delivery/trymail/RCS/strong.c,v 1.37 89/10/09 17:05:17 cfe Exp $";
...
/* First, look for the Content-Type: header. */
	if (Fmt == AlwaysOK || BracketField(BES->Header, ctFieldName,
	    ...
	    ctCode = -3;	/* There was such a header */
	...
	if (Fmt == NeverOK && ituCode == itu_send) ituCode = itu_alter;
...
/* Now look for the X-Andrew-ScribeFormat: header. */
 ...
 */
	...
	fprintf(RetF, "From: Mailer-daemon <%s>\n", PostmanAddress);
...
#define MOSTMATCHES 10
	...
	sprintf(MsgBf, "(On %s, local mail addressing at %s changed so that mail to a userid ", IDChangeDate, Dom);
	if (WasIDMatch) {
	    strcat(MsgBf, "(e.g. ");
	    strcat(MsgBf, Addr->LocalPart);
	    strcat(MsgBf, "@");
	...
	strcat(MsgBf, "is no longer unambiguously delivered to the person with that userid");
	if (WasIDMatch && NameCopy[0] != '\0') {
	    strcat(MsgBf, " (e.g. ");
	    strcat(MsgBf, NameCopy);
	    strcat(MsgBf, ")");
	}
	strcat(MsgBf, ".  To send to the person with a given userid, either suffix the userid in the mail address with a plus sign, giving userid+@");
	strcat(MsgBf, Dom);
	if (WasIDMatch) {
	    strcat(MsgBf, " (e.g., ");
	    strcat(MsgBf, Addr->LocalPart);
	...
	strcat(MsgBf, ", or address the mail to the user's real name, using periods or underscores in place of spaces");
	if (WasIDMatch && NameCopy[0] != '\0') {
...
#define MAXRECIP	40	/* Max number of oldsendmail recipients */
#define MAXARGVLEN	10000	/* Max number of chars in oldsendmail argv */
...
/* Once other delivery is finished, either print out or deliver to the remote addresses. */
	...
	int NumRecip, Recip, NMax, CharsRead, ArgVLen, ArgCCount;
...
/* Attempt to do cross-cell delivery. */
	if ((GlobalOptions & tmopt_CrossCellDelivery) != 0) {
	  ...
	  for (Hunk = 0; Hunk < DRCount; ++Hunk) {
	    ...
	    if (PGrp == 0) {
		if (IsSomeExpired) {	/* time it out early */
		    PGrp = -1;
		} else {
		    FCode1 = 14;
		}
		...
		if (FCode1 != 0) {FCode2 = FCode1; FCode1 = 1;}
			...
			if (! ReallyDeliver) {
			    ...
			    else if (FCode1 == Q_TEMP_FAIL) FCode1 = 15;
			    else {FCode2 = FCode1; FCode1 = 16;}
			}
			close(fd);
		    }
		    unlink(fName);
...
/* Invoke sendmail once for each distinct ForString value */
	...
	for (DR = RemoteRoot; DR != NULL; DR = NextDR) {
		...
		case 3:
		    if (DR->FailCode2 > 0) {
			if (tfail(DR->FailCode2) || ULstrcmp(GlobalDom, WorkstationCell) != 0) {
		...
		case 13:
		    ...
		    break;
		case 14:
		    AddrResult(tmltf_MailerTempFailure, DR->RmtAddr, "Temp failure checking AMS delivery in remote cell");
		    break;
		case 15:
		    AddrResult(tmltf_MailerTempFailure, DR->RmtAddr, "Temp failure in cross-cell dropoff");
		    break;
		case 16:
		    AddrResult(tmltf_MailerExecFailure, DR->RmtAddr, "Persistent failure in cross-cell dropoff (code %d)", DR->FailCode2);
		    break;
		default:
		    AddrResult(tmlpf_DropoffFailure, DR->RmtAddr,
			       "internal error in mail dropoff: %d/%d",
