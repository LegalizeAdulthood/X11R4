...
static char rcsid[] = "$Header: /afs/andrew.cmu.edu/itc/sm/releases/X.V11R4/andrew/ams/delivery/trymail/RCS/trymail.c,v 1.38 89/10/09 17:12:58 cfe Exp $";
...
int ReallyDeliver, IsExpired, IsExtraExpired, IsALittleExpired, IsSomeExpired;
...
{/* This guy has no electronic mail box; suggest an alternate path for contact. */
	wp_ErrorCode wpErr;
	char *Val;
	static char Reachable[] = ", but may be reached via ";
	...
	if (ULstrcmp(Dom, WorkstationCell) != 0) {
	    ...
	    return (PermMessage(tmlpf_NoMailbox, Addr, "No electronic mailbox",
			    "The addressee has no electronic mailbox%s%s.",
			    (wpErr == wperr_NoSuchField ? Reachable :
			     (strcmp(Val, "NONE") == 0 ?
			      " and has no forwarding address" : Reachable)),
			    (wpErr == wperr_NoSuchField ? DefaultSurfaceAddress :
			     (strcmp(Val, "NONE") == 0 ? "" : Val))
			    ));
	}
}

...
*/
	...
	for (FDum = Fwd; FDum != NULL; FDum = FDum->Parent) {
		if (! IsItOnList(FDum->fAddr, LocalDelivered, TRUE)) {
			if (FDum->Rewritten) AppendAddrTo(FDum->fAddr, LocalDelivered);
		  ...
		  && IsSameLocalPart(Addr, FDum->fAddr->LocalPart)) {
			...
			if (! IsItOnList(Addr, LocalDelivered, FALSE)) {
				if (TMDebug) fprintf(stderr, "Adding loc %s to delvr'd list.\n",
						Addr->LocalPart);
				AppendAddrTo(Addr, LocalDelivered);
			}
			return TRUE;
		}
	    }
...
char *DefaultDomain;
{
    ...
    extern ADDRESS_HOST *MakeHost();
    int laErr, laType, fwdErr, fwdType;
	...
	case latype_DistList:
	    ...
	    if (! IsItOnList(Addr, LocalDelivered, TRUE) && (Fwd == NULL ? TRUE : Fwd->Rewritten)) AppendAddrTo(Addr, LocalDelivered);
	    free(laPrime);
	    if (Hdr != NULL) free(Hdr);
	    return;
...
/* Found a unique maximum-NPA entry!  Use it--fake the cwp_Lookup result. */
	...
	Adv = OpenPMAdvisory(
			     (OutMatch > MatchExPA ? "Undeliverable heuristic" : "Deliverable heuristic"),
    ...
    if (NewLen == 0 && NameSep > 0) NewLen = strlen(QWName) + 1;
    ...
    if (! IsItOnList(Addr, LocalDelivered, TRUE)) if ((Fwd == NULL ? TRUE : Fwd->Rewritten)) AppendAddrTo(Addr, LocalDelivered);
    if (KVal != NULL) free(KVal);
    if (WName != NULL) free(WName);
    if (MBox != NULL) free(MBox);
    if (Hdr != NULL) free(Hdr);
...
{/* Set IsExpired and IsExtraExpired flags based on the EnqueueDate and the current time. */
	...
	IsExpired = IsExtraExpired = IsALittleExpired = IsSomeExpired = FALSE;
...
/* Sanity checks on the time value.  EnqueueDate values only appear too old if they're between AMS_ViceQueueLifetime and RidiculousAge before now.  Otherwise we assume a clock change (a time warp).  RidiculousAge is set to be the greater of AMS_ViceQueueLifetimeCap and twice the sum of AMS_ViceQueueLifetime and AMS_ExtraViceQueueLifetime. */
	...
	if (CheckAMSConfiguration() == 0 && osi_GetTimes(&TV) == 0) {
	    ...
	    if (TMDebug) fprintf(stderr,
		"Age: %d (%d,%d,%d): IsExpired=%d, IsExtraExpired=%d, IsALittleExpired=%d, IsSomeExpired=%d.\n", age,
			AMS_ViceQueueLifetime,
			AMS_ExtraViceQueueLifetime + AMS_ViceQueueLifetime,
			RidiculousAge, IsExpired, IsExtraExpired, IsALittleExpired, IsSomeExpired);
	}
}

...
/* Now initialize the exported globals */
	...
	SetREErr = -1;
	euid = geteuid();
	if (euid >= 0 && getuid() != euid) {	/* Make real and effective UIDs the same */
