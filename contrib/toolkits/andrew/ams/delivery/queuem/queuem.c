...
/*
	...
	OPTIONS:
		...
		-m	Mail errors back to sender (uses -r value
			or uid of invoker)
...
extern int errno;
extern long time();
extern char *malloc(), *realloc(), *arpadate();
extern char *sprintf();
...
static char MyPrimaryCell[2*MAXCELLCHARS+1] = { '\0' };
static int GotMyPrimaryCell = FALSE;
...
{/* Store the fact that a file is locked. */
    ...
    ErrorToPostmaster(sprintf(ErrM, "SetLocked(%s, %d): can't remember lock", name, where), NIL, FALSE);
}
...
{/* Store the fact that a file is being unlocked. */
    ...
    if (inthere == 0) ErrorToPostmaster(sprintf(ErrM, "unlock(%s, %d): file was never locked", name, where), NIL, FALSE);
}
...
{/* Put sanitized return-path in ReturnPath.  Return 0 if it couldn't be done.  This routine compensates for some of the strangely-formatted return paths that arrive over the net, such as route-addrs that are surrounded by multiple pairs of angle brackets. */
	...
	if (RPAt != NULL) {
		if (arg[extraBkts] == '<') {
		    ...
		    ReturnPath[0] = '<';
		    strcpy(ReturnPath+1, arg+extraBkts, argLen);
...

static int InitPrimaryCell()
{/* Sets up MyPrimaryCell if it can.  Returns -1 if can't get it, 0 for unauth or no primary, 1 for got primary. */
	int RC;
	struct CellAuth *ca;
	...
	if (! GotAMSHomeCell) {
		...
		if (RC == 2) {AMSHomeCell[0] = '\0'; GotAMSHomeCell = TRUE;}
			...
			{strcpy(AMSHomeCell, ca->CellName); GotAMSHomeCell = TRUE;}
	}
	if (! GotMyPrimaryCell) {
		errno = 0;
		RC = GetCurrentUserPrimaryCell(MyPrimaryCell, sizeof(MyPrimaryCell));
		if (RC == 0) {
			UserPrimaryCell = MyPrimaryCell;
			GotMyPrimaryCell = TRUE;
		}
	} else {
		RC = 0;
	}
	if (AMSHomeCell != NULL && *AMSHomeCell != '\0') {
		PrevailingCell = AMSHomeCell;
	} else {
		PrevailingCell = ThisDomain;
	}
	if (RC == 0) {
		if (MyPrimaryCell[0] == '\0') return 0;
		else return 1;
	} else {errno = RC; return -1;}
}

...
/* Process the arguments right away.  Keep track of things that require magic-ness. */
	...
	if (RC != 0 || ca == NULL) {
		...
		FatalError(sprintf(ErrorText,
				"Can't find desired home cell %s: code %d",
				homeCell, RC),
			NIL, FALSE, EX_TEMPFAIL);
	}
    ...
    FindPrimaryCell(&ca);
    if (ca != NULL) {
	strcpy(MyPrimaryCell, ca->CellName);
	UserPrimaryCell = MyPrimaryCell;
	GotMyPrimaryCell = TRUE;
    }
...
{/* Returns: 0 for all-sent, 1 for load-too-high, 2 for leaving-in-queue */
	...
	sprintf(ErrorText,
		"No recipients found in shadow file: \"%s\"",
...
char *srcDir;
{/* Set up more globals based on the source queue name.  Returns -1 if it can't do it, 0 if it can.  Return 1 if the source cell doesn't match my primary cell. */
	int RC;
...
static void InterjectDir(dirname, tokens, toklen, newFmt)
    char *dirname;
    ...
    int toklen, newFmt;
{
	...
	/* Sets up globals */
	Rslt = GetMessageFromQueue(LockFileName, &LockFD, &age, FALSE);
	if (Rslt != 0 && Rslt != 1) {
#ifdef LPPlumb
...
#ifdef AFS_ENV
    auto ClearToken ctoken;
    auto SecretToken stoken;
    struct CellAuth *ca;
    char *Read;
    auto char StrCellName[MAXCELLCHARS+1];
    int IsLocal, IsPrimary, Remaining, Expd, VID, RC;

    Read = tokens;
    Remaining = toklen;
    while (GenTokens(&Read, &Remaining, &ctoken, &stoken, &Expd, &VID, StrCellName, &IsPrimary, &IsLocal, 0) > 0) {
	if (IsPrimary) {
	    ca = NULL;
    ...
    int len;
{
    ...
    char *oldUserPrimCell; int oldGotMyPrimCell;
    ...
    NumTokenPairs = 0;
    pw = NULL;
    if (len > DROPOFF_PKT_LEN && ((ntohl(*(long *) packet) & DF_NEW_FMT) != 0)) {
	/* Third format */
	isNewFormat = 1;
    ...
    } else if (len == DROPOFF_PKT_LEN) {
	/* Second format */
	DropoffFlags = ntohl(*(long *) packet);
	uid = ntohl(*(long *) (packet+4));
	tokens = packet + 8;
	toklen = len - 8;
    } else if (len == DROPOFF_PACKET_LENGTH) {
	/* First format */
	DropoffFlags = 0;
	uid = ntohl(*(long *) packet);
	tokens = packet + 4;
	toklen = len - 4;
    } else {
	errprintf(Qname, ERR_CRITICAL, NULL, NULL,
    ...
    oldUserPrimCell = UserPrimaryCell;
    oldGotMyPrimCell = GotMyPrimaryCell;
#ifdef AFS_ENV
    ...
    GotMyPrimaryCell = FALSE;
    GotAMSHomeCell = FALSE;
    Res = InitPrimaryCell();
    if (Res <= 0) {
	errprintf(Qname, ERR_WARNING, NULL, NULL,
		  "No primary cell: %d/%d/%s", Res, errno, UnixError(errno));
	/* Unset the pag */
	blob.in_size = 0;
	blob.out_size = 0;
	...
	GotMyPrimaryCell = oldGotMyPrimCell;
	UserPrimaryCell = oldUserPrimCell;
    ...
    InterjectDir(newname, tokens, toklen, isNewFormat);
...
#define BIGPKTLEN (1500+DROPOFF_PKT_LEN)
	...
	int fromlen, readfds, writefds, exceptfds;
	...
	timeout.tv_sec = 0;
	timeout.tv_usec = 0;
	nfds = select(32, &readfds, &writefds, &exceptfds, &timeout);
	if (nfds < 0) {
...
static void waitforaction()
{
    ...
    timeout.tv_usec = 0;
    readfds = (portlisten ? 1<<mailport : 0);
    writefds = 0;
    exceptfds = 0;
    nfds = select(32, &readfds, &writefds, &exceptfds, &timeout);
    if (nfds < 0) {
    ...
    register int *nfiles, *err;
{
	...
	*err = -1;
	...
	if (stat(f, &buf) < 0) {
	    ...
	    *err = errno;
	    sprintf(ErrorText, "Can't stat \"%s\": %s",
		    f, UnixError(*err));
	...
	files[i].name = (char *) malloc(ent->d_namlen+1);
	if (files[i].name == NIL) {
...
static CheckOutgoings()
{
	...
	if (Pass == 1 && StartAt != '\0') {
	    ...
	    for (pwd=getvpwent(); pwd->pw_name[0] < StartAt; pwd=getvpwent());
	}
...
static LookAtQueue()
{
...
#endif /* AFS_ENV */
	/* Sets up globals */
	if (GetMessageFromQueue(LockFileName, &LockFD, &age, TRUE) != 0) break;
#ifdef LPPlumb
	...
	Ptr = NULL; XPtr = rindex(LockFileName, '/');
	if (XPtr != NULL) {
		*XPtr = '\0';
		Ptr = rindex(LockFileName, '/');
		*XPtr = '/';
	}
	...
	Ptr = index(MailSourceAuth, ';');	/* Now scan MailSourceAuth */
	if (Ptr != NULL) {
		*Ptr++ = '\0';
		XPtr = index(Ptr+1, ';');
		if (XPtr != NULL) *XPtr++ = '\0';
	...
	fputc('\n', stdout);
	if (atoi(MailSourceAuth) != 0) {
		fputs(XPtr, stdout);
#ifdef AFS_ENV
		if (ULstrcmp(Ptr, SourceQueueCell) != 0) {	/* indicate other-cell-ness */
...
static DaemonMain()
{
	...
	sprintf(ErrorText,
		"Setreuid (2) failed: %s; exiting", UnixError(errno));
	...
	if (SourceInVice < 0) {
	    ...
	    /* Sets up globals */
	    GetRslt = GetMessageFromQueue(LockFileName, &LockFD, &age, FALSE);
	}
#else /* AFS_ENV */
	GetRslt = GetMessageFromQueue(LockFileName, &LockFD, &age, FALSE);
#endif /* AFS_ENV */
...
#define No_Owner -1
    ...
    if (stat(name, buf) < 0) {
	sprintf(ErrorText,
		"Can't stat file \"%s\": %s", name, UnixError(errno));
	...
	sprintf(ErrorText,
		"Incompatible owners for \"%s\": %d, %d, %d",
		name, owner1, owner2, owner3);
	ErrorToPostmaster(ErrorText, NIL, FALSE);
	return No_Owner;
    }
...
static int GetMessageFromQueue(LockFile, refFD, refAge, JustLook)
    char *LockFile;
	...
	*refAge = time(0) - MailSourceStat.st_mtime;
	...
	if (SourceQueueCell[0] == '\0' || OwnerInCell == ANONYMOUSID) {
		...
		if (IsTempWPErr(cpw_error)) {
			...
			sprintf(MailSourceAuth, "%d;%s;Anonymous",
					OwnerInCell, SourceQueueCell);
...
{/* Returns 0 for all OK, +1 for retouched, -1 for retouch error */
	...
	XRes = 1;
	if (EMsg != NULL)
	    sprintf(ErrorText, "Old queued file %s (retouch failed in %s: %s)",
					MailSourceFile, EMsg, UnixError(errno));
	...
	if (EMsg != NULL) {
	    ...
	    ErrorToPostmaster(sprintf(ErrorText,
				      "Queue file %s is too old and has been retouched",
				      MailSourceFile),
			      NIL, TRUE);
	}
...
int UseAllQueues, UseSlowQueue;
{
	...
	case Q_NO_PERM:
	case Q_DIR_ERR:
	    sprintf(ErrorText, "Bad return (%d) from tryvicequeues: %s",
		    rc, Qmail_ErrMsg);
	    ErrorToPostmaster(ErrorText, NIL, FALSE);
	    return 1;
	default:
	    sprintf(ErrorText, "Impossible value from tryvicequeues: %d", rc);
	    return 1;
    }
}

    ...
    Boolean SendBody;
{
    ...
    argv[1] = PMMailbox;
    argv[2] = NULL;
    f = qopen(vicemail, argv, "w");
    if (f == NULL) {
	debug(512, ("Horrible bailout: can't run vicemail\n"));
	bailout(sprintf(Text,
			"Can not run vicemail to give error to postmaster: %s",
			Msg),
		AuxMsg, SendBody);
	return -1;
    }
...
#define HowMuchMore 80
	...
	if (fread(cbuf, sizeof(char), 3, f) == 0) {
	    if (ferror(f) != 0) {
		sprintf(ErrorText, "Can't get code from %s: %d (%s)",
			pgmname, errno, UnixError(errno));
	...
	for (Ix = 0; Ix < 3; ++Ix) {
		...
		sprintf(ErrorText, "Bad prefix from %s: ``%c%c%c%c'' (then ``%s'')",
				pgmname, cbuf[0], cbuf[1], cbuf[2], cont, MoreText);
		debug(32768, (NL, ErrorText));
		ErrorToPostmaster(ErrorText, NIL, FALSE);
		return -1;
	    }
...
static int TryLocalDelivery()
{
	...
	killpg(pgrp, SIGKILL);
	ErrorToPostmaster(sprintf(ErrorText,
				  "Had to kill %s.  Timeout %d, file %s",
				  argv[0], WillWait, MailSourceFile),
			  NIL, FALSE);
	return -1;
    }
	...
	switch (c=getc(fp)) {
		...
		for (;;) {
			...
			case EOF: debug(8, ("Unexpected EOF in GetNextToken"));
				  sprintf(ErrorText,
					 "EOF in bracketed token in shadow file \"%s\"",
					 MailSourceFileShadow);
				  ErrorToPostmaster(ErrorText, NIL, TRUE);
				  return NULL;
			case '|': c = getc(fp);
...
static int ParseShadowFile()
{
	...
	if (err != ENOENT && err != ENFILE && !vdown(err) && !RepeatError(EACCES, err))
	    ErrorToPostmaster(sprintf(ErrorText,
				      "Can not open shadow file %s: %s",
				      MailSourceFileShadow, UnixError(err)),
			      NIL, FALSE);
	errno = err;
	return -1;
    }
	...
	if (s[0] == '#' && bol) {
		...
		sprintf(ErrorText, "Shadow %s contains unknown field %s",
				MailSourceFileShadow, s);
    ...
    char *mesg;
{
    ...
    sprintf(ErrorText, "User (\"%s\"/%d/%s) not authorized to %s",
			WhoIAm, MyUID, PrevailingCell, mesg);
    FatalError(ErrorText, NIL, FALSE, EX_USAGE);
}

...
static CreateMessageTemp()
{
    ...
    if (i >= MAXTEMPTRIES) {
	sprintf(ErrorText, "Can't create temporary message file \"%s\": %s",
			MailSourceFile, UnixError(errn));
	debug(128, (NL, ErrorText));
...
static int GetMessageText()
{
	...
	if (MailSourceFD < 0) {
	    ...
	    FatalError(sprintf(ErrorText,
			       ...
			       MailSourceFile, UnixError(olderrno)),
		       NIL, FALSE, CheckRepeatErrors ? EX_TEMPFAIL : MapErrno(olderrno, EX_DATAERR));
	}
	...
	CleanupMessageText();
	sprintf(ErrorText, "Message on standard input is too small: %d (min is %d)",
		size, MIN_MESSAGE_SIZE);
