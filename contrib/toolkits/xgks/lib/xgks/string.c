...
static Bool XgksCreateDefString(WS_STATE_ENTRY *ws, Gint dev, INPUT_DEV **idevp)
{
    ...
    idev->gc = XCreateGC(ws->dpy, ws->win,
        GCFunction | GCForeground | GCBackground | GCFillStyle | GCFont, &gcvalues);
    ...
    PromptStatus pstate, XKeyPressedEvent *xev, int event_id)
{
    ...
    case PROMPTMOVE:
        ...
        else if (count > 0)
        {
            ...
            if (((StrX + MFontInfo->max_bounds.width + PADH) > xpt.x) || (StrPos == StrBufSiz))
            { /* check if input exceeds BufSiz */
                XBell(ws->dpy, 0);
            }
            else
            {
