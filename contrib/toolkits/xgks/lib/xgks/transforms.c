 ...
 *			XgksFindNTransNpts()
 * 
 * $Header: transforms.c,v 4.0 89/08/31 16:25:28 amy Exp $
 *
 * $Source: /andrew/Xgks/source/xgks.bld/src/RCS/transforms.c,v $
 *
 * $Log:	transforms.c,v $
 * Revision 4.0  89/08/31  16:25:28  amy
 * Changed IBM copyright for MIT distribution.
 * 
 * Revision 3.28  89/06/15  17:19:23  bruce
 * PTR# c1175:	Reworked code that calculates transformation matrices.
 * 
 * Revision 3.27  89/06/05  10:18:02  bruce
 * DCR# d1:	Changed include file name from gks_implement.h
 * 		to gks_implem.h for AIX compiler.
 * 
 * Revision 3.26  89/02/24  13:48:01  amy
 * PTR c2016	XgksInitWssTrans:  initialize deferral mode to GASAP.
 * 
 * Revision 3.25  89/02/20  11:52:53  amy
 * PTR c1008	gsetviewportinputpri, gselntran:  fix check for valid
 * 		transformation number.
 * 
 * Revision 3.24  89/02/04  15:30:43  amy
 * PTR c1147	Make global vars. and functions private, and static where possible.
 * 
 * Revision 3.23  88/12/29  13:13:19  amy
 * PTR c1139	XgksInitWssTrans, gsetwswindow, gsetwsviewport:  do not
 * 		attempt to set ws window or viewport on an MO workstation.
 * 
 * Revision 3.22  88/12/22  13:58:12  amy
 * PTR c1144	gsetwindow:  set pattern ref. pt and pattern size if MO open.
 * 		gsetviewport:  set pattern ref. pt. and pattern size if
 * 		MO is open.
 * 		gselntran:  set pattern ref. pt. and pattern size if MO open.
 * 
 * Revision 3.21  88/12/08  13:31:59  amy
 * PTR c1130	GKSERROR calls:  changed function name parameter from character
 * 		string to enum. type value.
 * 
 * Revision 3.20  88/10/12  13:56:10  amy
 ...
 * 
 * Revision 3.19  88/09/26  09:28:16  amy
 * MIT	Declare local and global functions where used.
 * 	XgksInitWssTrans:  to eliminate runtime error of request.w and request.v not 
 *      initialized, initialize these to the same values as current.w and current.v, 
 *	respectively.  Also, call XgksSetWsWindow only after these initializations 
 *	are complete.
 * 	Remove parameters in calls to XgksMoSetCharUp.
 * 	XgksComputeTrans:  define static void.
 * 	XgksSetClip, XgksSetWsWindow, XgksSetWsViewport:  define static void.
 * 
 * Revision 3.18  88/08/17  10:08:35  amy
 * AUG  gsetwindow:  call XgksMoSetCharUp with no parameters now.
 * 
 * Revision 3.17  88/08/11  09:15:48  amy
 * PTR c1012  gsetwsviewport & gsetwswindow:  added call to VALID_WSID
 * to check for error 20.
 * 
 * Revision 1.3  88/07/28  15:10:40  owens
 * added check for error 20 for PTR c1012
 * 
 * Revision 1.2  88/07/26  17:54:25  owens
 * renamed/replaced VALID_WSID macro for PTR c1012 (DWO)
 * 
 * Revision 1.1  88/07/21  10:50:34  david
 * Initial revision
 *  
 *
 */

static char *rcsid = "$Header: transforms.c,v 4.0 89/08/31 16:25:28 amy Exp $";
	...
	WS_STATE_PTR ws;
{
	 ...
	 */
	static void XgksSetWsWindow();		/*MIT*/
	static void XgksSetWsViewport();	/*MIT*/
	...
	Glimit *window;
{
	 ...
	 * declare local functions
	 */
	static void XgksComputeTrans();		/*MIT*/
	static void XgksSetClip();		/*MIT*/
...
static void XgksComputeTrans(trans)		/*MIT*/
	Gint trans;
{
	NORM_TRANS *nt;
...
Bool XgksFindNTransNpts( num, ndcpts, ntrans, wcpts)		/* c1014 */
...
static void XgksSetClip (ind)		/*MIT*/
Gclip ind;
{
	if (ind == GCLIP) {
...
static void XgksSetWsWindow (ws, window)		/*MIT*/
WS_STATE_PTR ws;
Glimit *window;
{
...
static void XgksSetWsViewport (ws, viewport)		/*MIT*/
WS_STATE_PTR ws;
Glimit *viewport;
{
