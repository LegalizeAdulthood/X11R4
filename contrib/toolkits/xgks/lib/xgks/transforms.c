...
Gint gsetviewport(Gint trans, Glimit *viewport)
{
    ...
    GKSERROR((viewport->xmin < 0.0 || viewport->xmin > 1.0 || viewport->xmax < 0.0 || viewport->xmax > 1.0 || viewport->ymin < 0.0 || viewport->ymin > 1.0 || viewport->ymax < 0.0 || viewport->ymax > 1.0), 52, errgsetviewport);
...
Gint gsetwswindow(Gint ws_id, Glimit *window)
{
    ...
    /* check all within 0..1 */
    GKSERROR((window->xmin < 0.0 || window->xmin > 1.0 || window->xmax < 0.0 || window->xmax > 1.0 || window->ymin < 0.0 || window->ymin > 1.0 || window->ymax < 0.0 || window->ymax > 1.0), 53, errgsetwswindow);
...
Bool XgksFindNTransNpts(int num, Gpoint *ndcpts, Gint *ntrans, Gpoint *wcpts)
{
    ...
    for (pl = 0; pl <= MAX_TRANS; pl++)
    {
        *ntrans = xgks_state.ntrans_priority[pl];        /* transform of pri pl */
        ...
        if (cnt == num
            && wcpt->x >= win->xmin && wcpt->y >= win->ymin
            && wcpt->x <= win->xmax && wcpt->y <= win->ymax) /* all the points transformed ok */
...
void XgksWsWinInterset(WS_STATE_PTR ws, Glimit *v, Glimit *clip)
{
    ...
    if ((v->xmax <= w->xmin) || (v->ymax <= v->ymin) || (v->xmin >= w->xmax) || (v->ymin >= w->ymax))
    {
        /* This probably means two are disjoint */
        clip->xmin = clip->xmax = clip->ymin = clip->ymax = 0.0;
        return;
    }
    if ((w->xmax >= v->xmax) && (w->xmin <= v->xmin) && (w->ymax >= v->ymax) && (w->ymin <= v->ymin))
    {
        /* This means ws-window encloses NDC-viewport */
        *(clip) = *v;
        return;
    }
    if ((v->xmax >= w->xmax) && (v->xmin <= w->xmin) && (v->ymax >= w->ymax) && (v->ymin <= w->ymin))
    {
...
Gint gaccumtran(Gfloat segtran[2][3], Gpoint *ppoint, Gpoint *pshift, Gfloat angle, Gpoint *pscale, Gcsw coord, Gfloat result[2][3])
{
    ...
    result[0][2] = (ndc_ref.x + ndc_shift.x) + segtran[0][2] * pscale->x * c - segtran[1][2] * pscale->y * s + ndc_ref.y * pscale->y * s - ndc_ref.x * pscale->x * c;
    result[1][2] = (ndc_ref.y + ndc_shift.y) + segtran[0][2] * pscale->x * s + segtran[1][2] * pscale->y * c - ndc_ref.x * pscale->x * s - ndc_ref.y * pscale->y * c;
    return (OK);
}
