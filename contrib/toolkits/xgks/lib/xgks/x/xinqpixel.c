...
Gint xXgksInqPixelarray(WS_STATE_PTR ws, Gpoint *point, Gipoint *dim, Gpxarray *pxarr)
{
    ...
    maxx = ((wsattr.x + ws->wbound.x + wsattr.border_width + 1) > rootattr.width) ? (rootattr.width - wsattr.x - wsattr.border_width - 1) : ws->wbound.x;
    maxy = ((wsattr.y + ws->wbound.y + wsattr.border_width + 1) > rootattr.height) ? (rootattr.height - wsattr.y - wsattr.border_width - 1) : ws->wbound.y;
