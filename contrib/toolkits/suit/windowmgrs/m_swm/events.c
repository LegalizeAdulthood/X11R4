/*
 *	Original Source: ./contrib/windowmgrs/twm/events.c
 *
 * Changes
...
static SwmWindow *tmp_win;	/* the current swm window */
static Window w;		/* the window that caused the event */
static SwmWindow *ButtonWindow;	/* button press window structure */
...
static double dl;
static double Dist = 70;
static long time0, time1;
static int  Dtime = 2;
static SwmWindow *DWindow;
...
HandleEvents()
{
	...
	else
	{
	    ...
	    w = event.xany.window;
	    if (XFindContext(dpy, w, SwmContext, &tmp_win) == XCNOENT)
		tmp_win = NULL;
...
HandleKeyPress()
{
    ...
    if (w == RootDesktop->frame)
        Context = C_ROOT;
    if ((tmp_win) && (tmp_win != RootDesktop))
    {
	    ...
	    (key->cont == Context || key->cont == C_NAME))
	{
		...
		for (tmp_win = SwmRoot.next; tmp_win != NULL;
		    tmp_win = tmp_win->next)
		{
		    if (!strncmp(key->win_name, tmp_win->window_name, len) ||
...
HandlePropertyNotify()
{
    ...
    default:
#ifdef DEBUG
	fprintf(stderr, "SWM Not handling property %d\n",event.xproperty.atom);
#endif
	break;
    }
}
...
HandleExpose()
{
	...
	if (tmp_win->icon_w == w)
	{
        ...
        if (tmp_win->dummyicon_w == w)
        {
            XDrawImageString(dpy, tmp_win->dummyicon_label,
                IconNormalGC,
                tmp_win->icon_x, tmp_win->icon_y,
                tmp_win->icon_name, strlen(tmp_win->icon_name));
            return;
...
SwmWindow       *tmp_win;
{
    ...
    XDeleteContext(dpy, tmp_win->w, SwmContext);
    XDeleteContext(dpy, tmp_win->move_edge, SwmContext);
    XDeleteContext(dpy, tmp_win->resize_rt_edge, SwmContext);
    XDeleteContext(dpy, tmp_win->resize_rb_edge, SwmContext);
    XDeleteContext(dpy, tmp_win->resize_lt_edge, SwmContext);
    XDeleteContext(dpy, tmp_win->resize_lb_edge, SwmContext);
    XDeleteContext(dpy, tmp_win->frame, SwmContext);
    XDeleteContext(dpy, tmp_win->title_w, SwmContext);
    XDeleteContext(dpy, tmp_win->iconify_w, SwmContext);
    XDeleteContext(dpy, tmp_win->icon_w, SwmContext);
    XDeleteContext(dpy, tmp_win->dummyicon_w, SwmContext);

    XDestroyWindow(dpy, tmp_win->frame);
    XDestroyWindow(dpy, tmp_win->icon_w);
    XDestroyWindow(dpy, tmp_win->dummyicon_w);
...
HandleDestroyNotify()
{
    ...
    XDestroyWindow(dpy, tmp_win->frame);
    XDestroyWindow(dpy, tmp_win->dummyicon_w);
    XGetGeometry(dpy, tmp_win->icon_w, &tmproot, &prex, &prey, &iw, &ih, &bd, &dp);
    tablex = (prex+(IconWidth/2))/IconTableWidth;
    tabley = (prey+(IconHeight/2))/IconTableWidth;
    junkDesktop = GetDesktopId();
    junkDesktop->iconTable[tablex][tabley] = 0;
    XDestroyWindow(dpy, tmp_win->icon_w);
    XFreeGC(dpy, tmp_win->title_gc);
    tmp_win->prev->next = tmp_win->next;
    if (tmp_win->next != NULL)
...
HandleMapRequest()
{
    ...
    XFetchName(dpy,event.xmaprequest.window,&windowName);

    w = event.xmaprequest.window;
    stat = XFindContext(dpy, w, SwmContext, &tmp_win);
    if (stat == XCNOENT)
	tmp_win = NULL;
	...
	if (Transient(w) && !DecorateTransients)
	{
        ...
        XGetNormalHints(dpy,w,&sizeHints);
        XAddToSaveSet(dpy, w);
        XReparentWindow(dpy,w,TempDesktop->w,sizeHints.x,sizeHints.y);
        newParent = TempDesktop->w;

        NewParent2 = TempDesktop->w;
        tmp_win = SuitWindow(w);
...
HandleMotionNotify()
{
	...
	if (ConstMoveDir != MOVE_NONE)
	{
	    ...
	    xl = ConstMoveX;
	    yt = ConstMoveY;
	    w = DragWidth + 1 * BorderWidth;
	    h = DragHeight + 1 * BorderWidth;
...
HandleButtonRelease()
{
	...
	XFindContext(dpy, DragWindow, SwmContext, &tmp_win);
	if (DragWindow == tmp_win->frame)
            {
             ...
             addX = xl-rx-BorderWidth;
             addY = yt-ry-BorderWidth;
             if (tmp_win->parent != 0)
                {
                if (addX < 0) 
                    addX = 0 ;
                if (addY < 0) 
                    addY = 0 ;
                if (addX >= TempDesktop->attr.width)
                   addX = tmp_win->frame_x ;
                if (addY > TempDesktop->attr.height)
                   addY = tmp_win->frame_y ;
                }
             if (tmp_win->parent == 1)
             {
                SetupWindow(tmp_win, addX, addY-TITLE_BAR_HEIGHT,
                tmp_win->frame_width, tmp_win->frame_height);
             }
             else
             SetupWindow(tmp_win, addX, addY, 
                tmp_win->frame_width, tmp_win->frame_height);
            }
	else
            {
              SetIconTable(tmp_win, &xl, &yt);
	      XMoveWindow(dpy, DragWindow, xl, yt);
              XMoveWindow(dpy, tmp_win->dummyicon_w, xl, yt);
             }
        ...
        if (Context == C_ICON)
          {
             if (DoubleClick)
                {
                  DoubleClick = 0;
                  ClickTime0 = ClickTime1;
                  XQueryPointer(dpy, Root, &JunkRoot, &JunkChild,
                               &JunkX, &JunkY, &cx1, &cy1, &JunkMask);
                  dl = sqrt((double)((cx1-cx0)*(cx1-cx0) + (cy1-cy0)*(cy1-cy0)));
                  if((dt <= Dtime)&&(dl<=Dist)&&(DWindow == tmp_win))
                    {
              ...
              {
               if (Mouse[event.xbutton.button][Context][modifier].func == F_ICONIFY)
                  {
                  ClickTime0 = time(NULL);
                  DoubleClick=1;
                  XQueryPointer(dpy, Root, &JunkRoot, &JunkChild,
                               &JunkX, &JunkY, &cx0, &cy0, &JunkMask);
                  DWindow = tmp_win;
                  return;
                  }
               else
...
HandleButtonPress()
{
    ...
    if (tmp_win && w == tmp_win->move_edge)
    {
        ExecuteFunction(F_MOVE, NULL, w, tmp_win, event, C_TITLE, FALSE);
        return;
    }

    if (tmp_win && w == tmp_win->resize_rt_edge)
    {
        ClampRight=ClampTop=1;
        ClampLeft=ClampBottom=0;
        ExecuteFunction(F_RESIZE, NULL, w, tmp_win, event, C_TITLE, FALSE);
        return;
    }
    if (tmp_win && w == tmp_win->resize_rb_edge)
    {
        ClampRight=ClampBottom=1;
        ClampLeft=ClampTop=0;
        ExecuteFunction(F_RESIZE, NULL, w, tmp_win, event, C_TITLE, FALSE);
        return;
    }
    if (tmp_win && w == tmp_win->resize_lt_edge)
    {
        ClampLeft=ClampTop=1;
        ClampRight=ClampBottom=0;
        ExecuteFunction(F_RESIZE, NULL, w, tmp_win, event, C_TITLE, FALSE);
        return;
    }
    if (tmp_win && w == tmp_win->resize_lb_edge)
    {
        ClampLeft=ClampBottom=1;
        ClampRight=ClampTop=0;
        ExecuteFunction(F_RESIZE, NULL, w, tmp_win, event, C_TITLE, FALSE);
        return;
    }
	...
	if (w == Root)
	{
	    ...
	    if (w == 0 ||
		(XFindContext(dpy, w, SwmContext, &tmp_win) == XCNOENT))
	    {
		RootFunction = NULL;
		XBell(dpy, screen);
		return;
