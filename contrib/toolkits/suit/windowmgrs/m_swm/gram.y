/*
 *	Original Source: ./contrib/windowmgrs/twm/gram.y
 *
 * Changes
...
%token <num> F_MENU F_UNFOCUS F_REFRESH F_FILE F_SWMRC F_CIRCLEUP F_QUIT
...
%token <num> F_DESKTOP F_HIDELIST NO_BACKINGSTORE NO_SAVEUNDER
...
%token <num> WARPCURSOR NUMBER EDGEWIDTH TITLE_FONT REVERSE_VIDEO
...
stmt		: error
		...
		| EDGEWIDTH number	{ if (FirstTime) EdgeWidth = $2; }
...
action		: F_NOP			{ $$ = F_NOP; }
		...
		| F_SOURCE string	{ Action = $2; $$ = F_SWMRC; }
		| F_MOVE		{ $$ = F_MOVE; }
...
yyerror(s) char *s;
{
    fprintf(stderr, "swm: syntax error, line %d\n", yylineno);
    ParseError = 1;
}
RemoveDQuote(str)
char *str;
