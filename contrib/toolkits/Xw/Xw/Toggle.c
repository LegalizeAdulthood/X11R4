...

{
   ...
   tw -> toggle.select_GC = 
      XtGetGC (tw, GCForeground | GCBackground | GCFillStyle, &values);
   ...
   Boolean all;         /* redo all, or just center? */
{
   ...
   if (w->toggle.square)
   {
      ...
      DrawDiamondButton (w, x-1, y-1, edge+2,
                         w -> button.normal_GC,
                         ((w->button.set) ? 
                           w -> toggle.select_GC :
                           w -> button.inverse_GC));
...
static void ClassInitialize()
{
   XwtoggleClassRec.primitive_class.select_proc = (XtWidgetProc) Select;
   XwtoggleClassRec.primitive_class.release_proc = (XtWidgetProc) Unselect;
   XwtoggleClassRec.primitive_class.toggle_proc = (XtWidgetProc) Toggle;
}
