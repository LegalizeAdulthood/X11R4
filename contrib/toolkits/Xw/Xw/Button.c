 ...
 /* Inherit procedures for handling traversal and selection */
   {
	...
	(XtWidgetProc) XtInheritSelectProc,
	(XtWidgetProc) XtInheritReleaseProc,
	(XtWidgetProc) XtInheritToggleProc,
   },
...
#define NewInverse 2
    ...
    ********************************************************************/
	...
	newcbox->button.sensitive_tile > XwSLANT_LEFT)
      {
    ...
    XwButtonWidget aButton;
{
    ...
    if (aButton->core.sensitive && aButton->core.ancestor_sensitive)
      {
          values.line_width   = 1;
     ...
     else
       {
         ...
         values.tile       = XwCreateTile (XtScreen((Widget)(aButton)),
				      values.foreground, values.background,
				      aButton->button.sensitive_tile);
...

{
    ...
    */
    ...
    if (aButton->button.sensitive_tile < XwFOREGROUND ||
	  aButton->button.sensitive_tile > XwSLANT_LEFT)
      {
         aButton->button.sensitive_tile = Xw75_FOREGROUND;
