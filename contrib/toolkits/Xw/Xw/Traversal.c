   ...
   if (direction == XwTRAVERSE_NEXT_TOP)
     {
       if (parent_is_manager)
       {
		  ...
		  manager_class.traversal_handler))
 		     (parent, newUl, newLr, direction);
       }
       else
       {
	    XtCallCallbacks(mgr, XtNnextTop, NULL);
       }
       return;
     }
...

{
   ...
   if ((action == XwADD_TO_CHOSEN_LIST) || (action == XwADD_TO_ACTIVE_LIST))
   {
      ...
      {
         _XwCheckListSize += 20;
         _XwCheckList = (_XwCheckListEntry *)XtRealloc(_XwCheckList, 
...
{
   while (w && XtIsSubclass(w, XwmanagerWidgetClass))
   {
