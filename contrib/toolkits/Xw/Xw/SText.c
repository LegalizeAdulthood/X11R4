	...
	XwStaticTextWidget	stw;
{
	...
	{
		XtWarning("XwStaticTextWidget: internal_height was less than zero, set to zero\n");
		stp->internal_height = 0;
	}
	if (stp->internal_width < 0)
	{
		XtWarning("XwStaticTextWidget: internal_width was less than zero, set to zero\n");
	...
	XSetWindowAttributes	*attributes;
{
	...
	XtCreateWindow(stw,InputOutput,(Visual *) CopyFromParent,
	...
	XwStaticTextWidget	stw;
{
	...
	if (stw->core.width < 0)
	{
		XtWarning("XwStaticTextWidget: Cannot resize to a negative width.  Resizing with width set to zero instead.");
		stw->core.width = 0;
	}
	if (stw->core.height < 0)
	{
		XtWarning("XwStaticTextWidget: Cannot resize to a negative height.  Resizing with height set to zero instead.");
		stw->core.height = 0;
	}
